...
let {
  ...
  cos: s,
  round: a
} = Math, r = e[0] | e[1] << 8 | e[2] << 16, i = e[3] | e[4] << 8, o = (63 & r) / 63, c = (r >> 6 & 63) / 31.5 - 1, u = (r >> 12 & 63) / 31.5 - 1, d = r >> 23, m = i >> 15, E = l(3, m ? d ? 5 : 7 : 7 & i), _ = l(3, m ? 7 & i : d ? 5 : 7), S = d ? (15 & e[5]) / 15 : 1, g = (e[5] >> 4) / 15, C = d ? 6 : 5, h = 0, Z = (n, t, l) => {
  let s = [];
  for (let a = 0; a < t; a++)
    for (let r = a ? 0 : 1; r * t < n * (t - a); r++)
      s.push(((e[C + (h >> 1)] >> ((1 & h++) << 2) & 15) / 7.5 - 1) * l);
  return s;
}, N = Z(E, _, (r >> 18 & 31) / 31), I = Z(3, 3, (i >> 3 & 63) / 63 * 1.25), x = Z(3, 3, (i >> 9 & 63) / 63 * 1.25), f = d && Z(5, 5, g), R = function(e) {
  let n = e[3],
    t = 128 & e[2],
    l = 128 & e[4];
...
for (let e = 0, a = 0; e < T; e++)
  for (let r = 0; r < A; r++, a += 4) {
    ...
    let h = i - 2 / 3 * m,
      Z = (3 * i - h + g) / 2,
      R = Z - g;
    v[a] = l(0, 255 * t(1, Z)), v[a + 1] = l(0, 255 * t(1, R)), v[a + 2] = l(0, 255 * t(1, h)), v[a + 3] = l(0, 255 * t(1, C));
  }
return {
  w: A,
  h: T,
