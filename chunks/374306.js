n.d(t, {
    Z: function () {
        return _;
    }
});
var i = n(200651);
n(192379);
var s = n(442837),
    a = n(481060),
    l = n(961675),
    r = n(883429),
    o = n(238349),
    c = n(109434),
    u = n(456269),
    d = n(432771),
    h = n(920303),
    p = n(665149),
    m = n(689938);
function _(e) {
    var t, n;
    let { channel: _ } = e,
        f = (0, u.r_)(_),
        E = (0, s.e7)([l.Z], () => l.Z.hasHidden(_.id)),
        g = (0, d.c)(_.id),
        { sortOrder: C, tagFilter: I } = (0, c.H)(_.id),
        T = (0, s.e7)([o.Z, h.Z], () => !!(o.Z.getThreadIds(_.id, C, I).length > 0) || !!(h.Z.getThreads(_.id, C, I).length > 0) || !1, [_.id, C, I]),
        x = _.isMediaChannel();
    if (!f || g || (x && T)) return null;
    let v = ((t = E), (n = x), t ? (n ? m.Z.Messages.MEDIA_CHANNEL_ADMIN_EDUCATION_SHOW : m.Z.Messages.FORUM_CHANNEL_ONBOARDING_SHOW) : n ? m.Z.Messages.MEDIA_CHANNEL_ADMIN_EDUCATION_HIDE : m.Z.Messages.FORUM_CHANNEL_ONBOARDING_HIDE);
    return (0, i.jsx)(p.JO, {
        tooltip: v,
        icon: a.CircleInformationIcon,
        onClick: () => r.Z.hideAdminOnboarding(_.id, !E),
        selected: !E
    });
}
