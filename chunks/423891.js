...
if (16 == c)
  ...
  tabs: {},
  frames: []
}, s = new Uint8Array(r.length), l = 0, c, u = 0, d = [
  137,
  80,
  78,
  71,
...
t += h, n.readUint(r, t), t += 4;
  ...
  for (var t = r.width, n = r.height, a = i.decode._getBPP(r), o = a >> 3, f = Math.ceil(t * a / 8), s = new Uint8Array(n * f), l = 0, c = [
  0,
  0,
  4,
  0,
...
if (0 == d)
  ...
  if (3 == d) {
    ...
    for (var v = o; v < f; v++)
      e[c + v] = e[u + v] + (e[c + v - f] + e[c + v - o] >> 1) & 255;
  }
  if (4 == d) {
    for (var v = 0; v < o; v++)
      e[c + v] = e[u + v] + s(0, e[c + v - f], 0) & 255;
    for (var v = o; v < f; v++)
      e[c + v] = e[u + v] + s(e[c + v - o], e[c + v - f], e[c + v - o - f]) & 255;
  }
}
  }
  return e;
...
if (v(f, c += 3 * y, d(f, c - 3 * y - 4, 3 * y + 4)), c += 4, g.gotAlpha) {
  ...
  for (var l = 0; l < y; l++)
    f[c + l] = g.plte[l] >> 24 & 255;
  v(f, c += y, d(f, c - y - 4, y + 4)), c += 4;
}
  }
  for (var R = 0, _ = 0; _ < g.frames.length; _++) {
var S = g.frames[_];
b && (v(f, c, 26), p(f, c += 4, 'fcTL'), v(f, c += 4, R++), v(f, c += 4, S.rect.width), v(f, c += 4, S.rect.height), v(f, c += 4, S.rect.x), v(f, c += 4, S.rect.y), h(f, c += 4, a[_]), h(f, c += 2, 1000), f[c += 2] = S.dispose, f[++c] = S.blend, v(f, ++c, d(f, c - 30, 30)), c += 4);
var N = S.cimg,
  y = N.length;
v(f, c, y + (0 == _ ? 0 : 4));
var T = c += 4;
...
if (0 != u && !A) {
  ...
  new Uint32Array(H.buffer), i._copyTile(O, r, t, H, T, M, -S, -N, 0), i._copyTile(R, r, t, H, T, M, -S, -N, 3) ? (i._copyTile(R, r, t, H, T, M, -S, -N, 2), q = 1) : (i._copyTile(R, r, t, H, T, M, -S, -N, 0), q = 0), _ = new Uint32Array((R = H).buffer);
}
var j = 4 * T;
if (U <= 256 && !1 == o) {
...
if (!(r * n > 500000) || 2 != s && 3 != s && 4 != s) {
  ...
  for (var c, u = 1000000000, d = 0; d < f.length; d++)
...
if (3 == o) {
  ...
  for (var c = a; c < n; c++)
    e[s + c] = r[f + c] + 256 - (r[f + c - n] + r[f + c - a] >> 1) & 255;
}
if (4 == o) {
  for (var c = 0; c < a; c++)
    e[s + c] = r[f + c] + 256 - l(0, r[f + c - n], 0) & 255;
  for (var c = a; c < n; c++)
    e[s + c] = r[f + c] + 256 - l(r[f + c - a], r[f + c - n], r[f + c - a - n]) & 255;
}
  }
}, i.crc = {
  table: function() {
...
for (var c = n[o], u = c.length, d = 0; d < u; d++)
  f[l + d] = c[d];
l += u;
  }
  var v = {
...
for (var U = i.quantize.planeDst, I = new Uint8Array(n[A].buffer), R = new Uint32Array(n[A].buffer), _ = I.length, o = 0; o < _; o += 4) {
  for (var S = I[o] * (1 / 255), N = I[o + 1] * (1 / 255), T = I[o + 2] * (1 / 255), M = I[o + 3] * (1 / 255), q = v; q.left;)
