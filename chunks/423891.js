...
if (m && (d = m[0], v = m[1], g = m[2]), 8 == l)
  for (var p = 0; p < i; p++) {
    var y = p << 2,
      b = 3 * p;
    s[y] = t[b], s[y + 1] = t[b + 1], s[y + 2] = t[b + 2], s[y + 3] = 255, -1 != d && t[b] == d && t[b + 1] == v && t[b + 2] == g && (s[y + 3] = 0);
  }
if (16 == l)
  for (var p = 0; p < i; p++) {
    var y = p << 2,
      b = 6 * p;
    s[y] = t[b], s[y + 1] = t[b + 2], s[y + 2] = t[b + 4], s[y + 3] = 255, -1 != d && h(t, b) == d && h(t, b + 2) == v && h(t, b + 4) == g && (s[y + 3] = 0);
  }
  } else if (3 == c) {
var E = n.tabs.PLTE,
  T = n.tabs.tRNS,
  ...
  for (var A = 0; A < r; A++) {
    for (var I = A * a, S = A * e, p = 0; p < e; p++) {
      var y = S + p << 2,
        H = t[I + (p >> 3)] >> 7 - ((7 & p) << 0) & 1,
        L = 3 * H;
      s[y] = E[L], s[y + 1] = E[L + 1], s[y + 2] = E[L + 2], s[y + 3] = H < _ ? T[H] : 255;
    }
  }
if (2 == l)
  for (var A = 0; A < r; A++) {
...
if (16 == l)
  ...
  tabs: {},
  frames: []
}, u = new Uint8Array(e.length), c = 0, l, h = 0, f = [
  137,
  80,
  78,
  71,
...
r += m, n.readUint(e, r), r += 4;
  ...
  for (var r = e.width, n = e.height, i = o.decode._getBPP(e), a = i >> 3, s = Math.ceil(r * i / 8), u = new Uint8Array(n * s), c = 0, l = [
  0,
  0,
  4,
  0,
...
for (var _ = 0, A = l[m]; A < n;) {
  for (var I = h[m], S = c + _ * T << 3; I < r;) {
    if (1 == i) {
      var H = t[S >> 3];
      H = H >> 7 - (7 & S) & 1, u[A * s + (I >> 3)] |= H << 7 - ((3 & I) << 0);
    }
    if (2 == i) {
      var H = t[S >> 3];
      H = H >> 6 - (7 & S) & 3, u[A * s + (I >> 2)] |= H << 6 - ((3 & I) << 1);
    }
    if (4 == i) {
      var H = t[S >> 3];
      H = H >> 4 - (7 & S) & 15, u[A * s + (I >> 1)] |= H << 4 - ((1 & I) << 2);
    }
    if (i >= 8) {
      for (var L = A * s + I * a, P = 0; P < a; P++)
        u[L + P] = t[(S >> 3) + P];
    }
    S += i, I += v;
  }
...
if (0 == f)
  ...
  if (3 == f) {
    ...
    for (var p = a; p < s; p++)
      t[l + p] = t[h + p] + (t[l + p - s] + t[l + p - a] >> 1) & 255;
  }
  if (4 == f) {
    for (var p = 0; p < a; p++)
      t[l + p] = t[h + p] + u(0, t[l + p - s], 0) & 255;
    for (var p = a; p < s; p++)
      t[l + p] = t[h + p] + u(t[l + p - a], t[l + p - s], t[l + p - a - s]) & 255;
  }
}
  }
  return t;
...
for (var m = 0; m < c; m++)
  if (a >= 0 && s >= 0 ? (h = p * e + m << 2, f = (s + p) * i + a + m << 2) : (h = (-s + p) * e - a + m << 2, f = p * i + m << 2), 0 == u)
    n[f] = t[h], n[f + 1] = t[h + 1], n[f + 2] = t[h + 2], n[f + 3] = t[h + 3];
  else if (1 == u) {
var d = t[h + 3] * (1 / 255),
  v = t[h] * d,
  g = t[h + 1] * d,
...
var d = t[h + 3],
  ...
  T = n[f + 1],
  _ = n[f + 2];
d == b && v == E && g == T && y == _ ? (n[f] = 0, n[f + 1] = 0, n[f + 2] = 0, n[f + 3] = 0) : (n[f] = v, n[f + 1] = g, n[f + 2] = y, n[f + 3] = d);
  } else if (3 == u) {
var d = t[h + 3],
  v = t[h],
  g = t[h + 1],
...
v = o.encode.compressPNG(t, e, r, n, a);
  p(s, l, 13), d(s, l += 4, 'IHDR'), p(s, l += 4, e), p(s, l += 4, r), s[l += 4] = v.depth, s[++l] = v.ctype, s[++l] = 0, s[++l] = 0, s[++l] = 0, p(s, ++l, f(s, l - 17, 17)), p(s, l += 4, 1), d(s, l += 4, 'sRGB'), s[l += 4] = 1, p(s, ++l, f(s, l - 5, 5)), l += 4;
  var g = t.length > 1;
  if (g && (p(s, l, 8), d(s, l += 4, 'acTL'), p(s, l += 4, t.length), p(s, l += 4, 0), p(s, l += 4, f(s, l - 12, 12)), l += 4), 3 == v.ctype) {
var y = v.plte.length;
...
if (p(s, l += 3 * y, f(s, l - 3 * y - 4, 3 * y + 4)), l += 4, v.gotAlpha) {
  ...
  for (var c = 0; c < y; c++)
    s[l + c] = v.plte[c] >> 24 & 255;
  p(s, l += y, f(s, l - y - 4, y + 4)), l += 4;
}
  }
  for (var I = 0, S = 0; S < v.frames.length; S++) {
var H = v.frames[S];
g && (p(s, l, 26), d(s, l += 4, 'fcTL'), p(s, l += 4, I++), p(s, l += 4, H.rect.width), p(s, l += 4, H.rect.height), p(s, l += 4, H.rect.x), p(s, l += 4, H.rect.y), m(s, l += 4, i[S]), m(s, l += 2, 1000), s[l += 2] = H.dispose, s[++l] = H.blend, p(s, ++l, f(s, l - 30, 30)), l += 4);
var L = H.cimg,
  y = L.length;
p(s, l, y + (0 == S ? 0 : 4));
var P = l += 4;
d(s, l, 0 == S ? 'IDAT' : 'fdAT'), l += 4, 0 != S && (p(s, l, I++), l += 4);
for (var c = 0; c < y; c++)
  s[l + c] = L[c];
p(s, l += y, f(s, P, l - P)), l += 4;
  }
  return p(s, l, 0), d(s, l += 4, 'IEND'), p(s, l += 4, f(s, l - 4, 4)), l += 4, s.buffer.slice(0, l);
...
for (var f = new Uint8Array(t[h]), p = f.length, m = 0; m < p; m += 4)
  l &= f[m + 3];
  }
  var d = 255 != l,
v = {},
...
for (var h = 0; h < t.length; h++) {
  for (var E = new Uint32Array(t[h]), p = E.length, m = 0; m < p; m++) {
    var b = E[m];
    if ((m < e || b != E[m - 1] && b != E[m - e]) && null == v[b] && (v[b] = g.length, g.push(b), g.length >= 300))
      break;
  }
}
  var T = !!d && i,
_ = g.length;
  _ <= 256 && !1 == a && (u = _ <= 2 ? 1 : _ <= 4 ? 2 : _ <= 16 ? 4 : 8, i && (u = 8), d = !0);
...
var I = new Uint8Array(t[h]),
  ...
  N = 0;
if (0 != h && !T) {
  for (var w = i || 1 == h || 2 == A[A.length - 2].dispose ? 1 : 2, R = 0, M = 1000000000, C = 0; C < w; C++) {
    for (var O = new Uint8Array(t[h - 1 - C]), D = new Uint32Array(t[h - 1 - C]), U = e, F = r, G = -1, k = -1, x = 0; x < r; x++)
      for (var j = 0; j < e; j++) {
        var m = x * e + j;
        S[m] != D[m] && (j < U && (U = j), j > G && (G = j), x < F && (F = x), x > k && (k = x));
      }
    var V = -1 == G ? 1 : (G - U + 1) * (k - F + 1);
    V < M && (M = V, R = C, -1 == G ? (H = L = 0, P = B = 1) : (H = U, L = F, P = G - U + 1, B = k - F + 1));
  }
  var O = new Uint8Array(t[h - 1 - R]);
  1 == R && (A[A.length - 1].dispose = 2);
  var Z = new Uint8Array(P * B * 4);
  new Uint32Array(Z.buffer), o._copyTile(O, e, r, Z, P, B, -H, -L, 0), o._copyTile(I, e, r, Z, P, B, -H, -L, 3) ? (o._copyTile(I, e, r, Z, P, B, -H, -L, 2), N = 1) : (o._copyTile(I, e, r, Z, P, B, -H, -L, 0), N = 0), S = new Uint32Array((I = Z).buffer);
}
var K = 4 * P;
if (_ <= 256 && !1 == a) {
  ...
  for (var Z = new Uint8Array(P * B * 3), q = P * B, m = 0; m < q; m++) {
    var Y = 3 * m,
      z = 4 * m;
...
if (!(e * n > 500000) || 2 != u && 3 != u && 4 != u) {
  ...
  for (var l, h = 1000000000, f = 0; f < s.length; f++)
...
if (3 == a) {
  ...
  for (var l = i; l < n; l++)
    t[u + l] = e[s + l] + 256 - (e[s + l - n] + e[s + l - i] >> 1) & 255;
}
if (4 == a) {
  for (var l = 0; l < i; l++)
    t[u + l] = e[s + l] + 256 - c(0, e[s + l - n], 0) & 255;
  for (var l = i; l < n; l++)
    t[u + l] = e[s + l] + 256 - c(e[s + l - i], e[s + l - n], e[s + l - i - n]) & 255;
}
  }
}, o.crc = {
  table: function() {
...
for (var l = n[a], h = l.length, f = 0; f < h; f++)
  s[c + f] = l[f];
c += h;
  }
  var p = {
...
for (var a = 0; a < 16; a++)
  E.bst.R[a] = g.bst.R[a] - b.bst.R[a];
for (var a = 0; a < 4; a++)
  E.bst.m[a] = g.bst.m[a] - b.bst.m[a];
E.est = o.quantize.estats(E.bst), g.left = b, g.right = E, m[v] = b, m.push(E);
  }
  m.sort(function(t, e) {
...
for (var _ = o.quantize.planeDst, A = new Uint8Array(n[T].buffer), I = new Uint32Array(n[T].buffer), S = A.length, a = 0; a < S; a += 4) {
  for (var H = A[a] * (1 / 255), L = A[a + 1] * (1 / 255), P = A[a + 2] * (1 / 255), B = A[a + 3] * (1 / 255), N = p; N.left;)
    N = 0 >= _(N.est, H, L, P, B) ? N.left : N.right;
  I[a >> 2] = N.est.rgba;
}
n[T] = I.buffer;
  }
...
m = 0;
  if (0 != n)
for (var d = 0; d < 10 && (f = h.multVec(l, f), m = Math.sqrt(h.dot(f, f)), f = h.sml(1 / m, f), !(1e-9 > Math.abs(m - p))); d++) {
  ;
  p = m;
}
  var v = [
