...
if (0 == s ? i = p : 0 == u.blend ? o._copyTile(p, h, f, i, e, r, c, l, 0) : 1 == u.blend && o._copyTile(p, h, f, i, e, r, c, l, 1), n.push(i.buffer), i = i.slice(0), 0 == u.dispose);
  ...
  for (var d = s - 1; 2 == t.frames[d].dispose;)
    d--;
...
if (r += 4, 'IHDR' == m)
  ...
  s.tabs[m] = [];
  for (var p = 0; p < 8; p++)
    s.tabs[m].push(n.readUint(e, r + 4 * p));
} else if ('tEXt' == m) {
  ...
  s.tabs[m] = n.readBytes(e, r, d);
else if ('hIST' == m) {
  var I = s.tabs.PLTE.length / 3;
  s.tabs[m] = [];
  for (var p = 0; p < I; p++)
    s.tabs[m].push(i(e, r + 2 * p));
} else if ('tRNS' == m)
  3 == s.ctype ? s.tabs[m] = n.readBytes(e, r, d) : 0 == s.ctype ? s.tabs[m] = i(e, r) : 2 == s.ctype && (s.tabs[m] = [
    i(e, r),
    i(e, r + 2),
    i(e, r + 4)
  ]);
else if ('gAMA' == m)
  s.tabs[m] = n.readUint(e, r) / 100000;
else if ('sRGB' == m)
  s.tabs[m] = e[r];
else if ('bKGD' == m)
  0 == s.ctype || 4 == s.ctype ? s.tabs[m] = [i(e, r)] : 2 == s.ctype || 6 == s.ctype ? s.tabs[m] = [
    i(e, r),
    i(e, r + 2),
    i(e, r + 4)
  ] : 3 == s.ctype && (s.tabs[m] = e[r]);
else if ('IEND' == m) {
  if (0 != h) {
    var v = s.frames[s.frames.length - 1];
    v.data = o.decode._decompress(s, l.slice(0, h), v.rect.width, v.rect.height), h = 0;
...
for (var m = f[d], v = p[d], g = 0, y = 0, b = l[d]; b < n;)
  b += m, y++;
for (var E = h[d]; E < r;)
  E += v, g++;
...
for (var d = 0; d < c; d++)
  if (a >= 0 && s >= 0 ? (h = p * e + d << 2, f = (s + p) * i + a + d << 2) : (h = (-s + p) * e - a + d << 2, f = p * i + d << 2), 0 == u)
...
for (var f = new Uint8Array(t[h]), p = f.length, d = 0; d < p; d += 4)
  l &= f[d + 3];
  }
  var m = 255 != l,
v = {},
...
for (var h = 0; h < t.length; h++) {
  for (var E = new Uint32Array(t[h]), p = E.length, d = 0; d < p; d++) {
    var b = E[d];
    if ((d < e || b != E[d - 1] && b != E[d - e]) && null == v[b] && (v[b] = g.length, g.push(b), g.length >= 300))
      break;
  }
}
  var T = !!m && i,
...
if (_ <= 256 && !1 == a) {
  for (var Z = new Uint8Array((K = Math.ceil(u * P / 8)) * B), x = 0; x < B; x++) {
    ...
    if (8 == u)
      ...
      for (var j = 0; j < P; j++)
        Z[d + (j >> 1)] |= v[S[X + j]] << 4 - (1 & j) * 4;
    else if (2 == u)
      for (var j = 0; j < P; j++)
        Z[d + (j >> 2)] |= v[S[X + j]] << 6 - (3 & j) * 2;
    else if (1 == u)
      for (var j = 0; j < P; j++)
        Z[d + (j >> 3)] |= v[S[X + j]] << 7 - (7 & j) * 1;
  }
  I = Z, s = 3, c = 1;
...
for (var _ = o.quantize.planeDst, A = new Uint8Array(n[T].buffer), I = new Uint32Array(n[T].buffer), S = A.length, a = 0; a < S; a += 4) {
  for (var L = A[a] * (1 / 255), H = A[a + 1] * (1 / 255), P = A[a + 2] * (1 / 255), B = A[a + 3] * (1 / 255), w = p; w.left;)
