var r = n(814033), i = n(685053), a = i.getNAF, o = i.getJSF, s = i.assert;
function l(e, t) {
    this.type = e, this.p = new r(t.p, 16), this.red = t.prime ? r.red(t.prime) : r.mont(this.p), this.zero = new r(0).toRed(this.red), this.one = new r(1).toRed(this.red), this.two = new r(2).toRed(this.red), this.n = t.n && new r(t.n, 16), this.g = t.g && this.pointFromJSON(t.g, t.gRed), this._wnafT1 = [
        ,
        ,
        ,
        ,
...
e.exports = l, l.prototype.point = function () {
    ...
    s(e.precomputed);
    var n, r, i = e._getDoubles(), o = a(t, 1, this._bitLength), l = (1 << i.step + 1) - (i.step % 2 == 0 ? 2 : 1);
    l /= 3;
    var u = [];
    ...
    var s, l, u, c = this._wnafT1, d = this._wnafT2, _ = this._wnafT3, E = 0;
    for (s = 0; s < r; s++) {
    ...
    for (s = r - 1; s >= 1; s -= 2) {
        ...
        for (l = 0, E = Math.max(T[0].length, E), _[h] = Array(E), _[p] = Array(E); l < E; l++) {
            var g = 0 | T[0][l], S = 0 | T[1][l];
            _[h][l] = I[(g + 1) * 3 + (S + 1)], _[p][l] = 0, d[h] = m;
        }
    }
    var A = this.jpoint(null, null, null), N = this._wnafT4;
    for (s = E; s >= 0; s--) {
        for (var v = 0; s >= 0;) {
            var O = !0;
            for (l = 0; l < r; l++)
                N[l] = 0 | _[l][s], 0 !== N[l] && (O = !1);
            if (!O)
                break;
            v++, s--;
        }
        if (s >= 0 && v++, A = A.dblp(v), s < 0)
            break;
        for (l = 0; l < r; l++) {
            var R = N[l];
            if (0 !== R) {
    ...
    if ((4 === e[0] || 6 === e[0] || 7 === e[0]) && e.length - 1 == 2 * n)
        return 6 === e[0] ? s(e[e.length - 1] % 2 == 0) : 7 === e[0] && s(e[e.length - 1] % 2 == 1), this.point(e.slice(1, 1 + n), e.slice(1 + n, 1 + 2 * n));
