...
}), t(518263), t(970173), t(520712), t(268111), t(941497), t(32026), t(480839), t(744285), t(492257), t(873817), t(411104), t(47120), t(724458), t(242167), t(653041);
...
class u {
    appendBytes(e) {
        if (this._offset + e.length > this._buffer.length) {
            let n = this._offset + e.length, t = new Uint8Array(Math.pow(2, Math.ceil(Math.log2(Math.max(2 * this._buffer.length, n)))));
    ...
    addPage(e) {
        ...
        for (let [n, t] of e.segments.entries())
            if (t.length > 255)
                throw Error('Segment at index '.concat(n, ' too large (length ').concat(t.length, ' exceeds ').concat(255, ')'));
        let n = e.segments.reduce((e, n) => e + n.length, 0), t = 27 + e.segments.length + n;
        if (t > 65307)
            throw Error('Page too large (size '.concat(t, ' exceeds ').concat(65307, ')'));
        let l = this._offset, a = 0;
        switch (e.pageType) {
        case 1:
            a = 1;
            break;
        ...
        let i = this._offset;
        for (let n of (this.appendBytes([
                0,
                0,
                0,
                0,
                e.segments.length
            ]), this.appendBytes(e.segments.map(e => e.length)), e.segments))
            this.appendBytes(n);
        let s = function (e) {
    ...
    finalize(e) {
        ...
        let n = this._buffer, t = this._offset;
...
function o(e, n) {
    ...
    for (let n of (a.addPage({
        ...
        let e = function (e) {
            let n = Math.floor(e.length / 255), t = [];
            for (let l = 0; l <= n; l++) {
                let a = 0 === l ? 0 : 255 * l, i = l === n ? e.length : (l + 1) * 255;
                t.push(e.slice(a, i));
            }
            return t;
        }(n.buffer);
        i += n.numSamples, a.addPage({
            pageType: 0,
            granulePosition: i,
            segments: e
        });
