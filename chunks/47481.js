"use strict";
n.r(t), n.d(t, {
  default: function() {
    return d
  }
}), n("653041"), n("47120");
var a = n("275726"),
  l = n("135938"),
  s = n("55935"),
  i = n("709054"),
  r = n("554838"),
  o = n("493892"),
  u = n("981631");

function d(e) {
  let t, n, d, {
      channel: c,
      messages: f,
      oldestUnreadMessageId: h,
      treatSpam: m,
      summaries: p,
      selectedSummary: E
    } = e,
    g = [],
    C = !1,
    S = null != h ? i.default.extractTimestamp(h) : null,
    _ = null;
  return f.forEach(e => {
    var l, T, I, A, v, x, N, M;
    if (null != p && p.length > 0) {
      ;
      let t = i.default.extractTimestamp(e.id);
      for (let e = 0; l = e < (null == p ? void 0 : p.length), l; e++) {
        if (null == p[e]) continue;
        let n = i.default.extractTimestamp(p[e].startId),
          a = i.default.extractTimestamp(p[e].endId);
        if (t >= n && t <= a) {
          if (_ === p[e].id) break;
          g.push({
            type: u.ChannelStreamTypes.DIVIDER,
            content: p[e].topic,
            contentKey: p[e].id
          }), _ = p[e].id;
          break
        }
      }
    }
    let y = (0, s.dateFormat)(e.timestamp, "LL");
    y !== t && null == _ && (g.push({
      type: u.ChannelStreamTypes.DIVIDER,
      content: y,
      contentKey: y
    }), t = y);
    let R = g[g.length - 1],
      L = null,
      O = (0, o.isSpam)(e);
    C = C || O;
    let j = function(e, t, n) {
      if (a.MessageTypesSets.NON_COLLAPSIBLE.has(t.type));
      else if (t.blocked) return u.ChannelStreamTypes.MESSAGE_GROUP_BLOCKED;
      else if ((0, o.isSpamSupported)(e) && n) return u.ChannelStreamTypes.MESSAGE_GROUP_SPAMMER;
      return null
    }(c, e, O && m);
    if (null !== j) {
      ;
      let t, n;
      [L, R] = (T = g, I = e, A = j, n = v = R, null == v || v.type !== A ? (t = {
        type: A,
        content: [],
        key: I.id
      }, T.push(t)) : n = (t = v).content[t.content.length - 1], [t, n])
    }
    if (h === e.id && null != S) {
      if (null != R && R.type === u.ChannelStreamTypes.DIVIDER) R.unreadId = e.id, S = null;
      else if (null !== L) {
        ;
        x = L, N = c, (M = e).isFirstMessageInForumPost(N) || x.content.push({
          type: u.ChannelStreamTypes.DIVIDER,
          unreadId: M.id
        }), x.hasUnread = !0, S = null
      } else !e.isFirstMessageInForumPost(c) && g.push({
        type: u.ChannelStreamTypes.DIVIDER,
        unreadId: e.id
      }), S = null
    } else null != S && i.default.extractTimestamp(e.id) > S && (!e.isFirstMessageInForumPost(c) && g.push({
      type: u.ChannelStreamTypes.DIVIDER,
      unreadId: e.id
    }), S = null);
    let P = (null == R ? void 0 : R.type) === u.ChannelStreamTypes.MESSAGE ? d : R;
    (0, r.isNewGroupItem)(c, P, e) && (n = e.id);
    let D = {
      type: e.type === u.MessageTypes.THREAD_STARTER_MESSAGE ? u.ChannelStreamTypes.THREAD_STARTER_MESSAGE : u.ChannelStreamTypes.MESSAGE,
      content: e,
      groupId: n
    };
    n === e.id && (d = D);
    let {
      jumpSequenceId: b,
      jumpFlash: U,
      jumpTargetId: F
    } = f;
    U && e.id === F && null != b && (D.flashKey = b), f.jumpTargetId === e.id && (D.jumpTarget = !0), null != E && e.id === E.startId && E.count > 1 && g.push({
      type: u.ChannelStreamTypes.DIVIDER,
      content: E.topic,
      contentKey: E.startId,
      isSummaryDivider: !0
    }), null !== L ? (L.content.push(D), D.jumpTarget && (L.hasJumpTarget = !0)) : g.push(D), e.isFirstMessageInForumPost(c) && g.push({
      type: u.ChannelStreamTypes.FORUM_POST_ACTION_BAR
    }), null != E && e.id === E.endId && E.count > 1 && g.push({
      type: u.ChannelStreamTypes.DIVIDER,
      contentKey: E.endId,
      isSummaryDivider: !0
    })
  }), C && (0, o.isSpamSupported)(c) && l.default.trackExposure({
    location: "416cc9_1"
  }), g
}