t.d(n, {
    Z: function () {
        return v;
    }
}),
    t(47120);
var i = t(735250),
    a = t(470079),
    l = t(120356),
    o = t.n(l),
    r = t(442837),
    s = t(481060),
    c = t(115130),
    d = t(147865),
    u = t(427996),
    m = t(812236),
    p = t(70097),
    _ = t(240991),
    C = t(783097),
    h = t(695676),
    f = t(520315),
    A = t(29380),
    E = t(689938),
    N = t(655863),
    x = t(910212);
function v(e) {
    let { channel: n, application: t, videoUrl: l, imageCoverUrl: s, sectionName: d, hasCommands: m } = e,
        _ = a.useMemo(() => {
            var e;
            return null !== (e = (0, C.jD)(t)) && void 0 !== e ? e : '';
        }, [t]),
        f = (0, r.e7)([c.Z], () => c.Z.inDevModeForApplication(t.id)),
        A = null != l || null != s,
        { isSlideReady: x } = (0, h.hH)(),
        [v, S] = a.useState(!1);
    return (
        a.useEffect(() => {
            x && S(!0);
        }, [x]),
        (0, i.jsxs)('div', {
            className: N.container,
            children: [
                (0, i.jsxs)('div', {
                    className: N.profileAndVideoContainer,
                    children: [
                        A
                            ? (0, i.jsxs)('div', {
                                  className: N.videoContainer,
                                  children: [
                                      v
                                          ? (0, i.jsx)(p.Z, {
                                                className: o()(N.videoCover, N.video),
                                                loop: !0,
                                                muted: !0,
                                                autoPlay: !0,
                                                src: l,
                                                poster: s
                                            })
                                          : null,
                                      (0, i.jsx)('img', {
                                          className: N.videoCover,
                                          src: s,
                                          'aria-label': E.Z.Messages.IMAGE
                                      })
                                  ]
                              })
                            : null,
                        (0, i.jsxs)('div', {
                            className: A ? N.overviewContainerWithVideo : N.overviewContainerNoVideo,
                            children: [
                                (0, i.jsx)(g, { application: t }),
                                (0, i.jsx)(P, { application: t }),
                                _.length > 0 ? (0, i.jsx)(b, { description: _ }) : null,
                                f
                                    ? (0, i.jsx)('div', {
                                          className: N.developerShelfControlsContainer,
                                          children: (0, i.jsx)(u.W, {
                                              hideSearch: !0,
                                              className: N.developerShelfControls
                                          })
                                      })
                                    : null,
                                (0, i.jsx)(L, {
                                    channel: n,
                                    application: t,
                                    sectionName: d,
                                    isDeveloperOfThisApp: f,
                                    hasCommands: m
                                })
                            ]
                        })
                    ]
                }),
                (0, i.jsx)(I, { application: t })
            ]
        })
    );
}
function I(e) {
    let { application: n } = e,
        t = (0, C.Cb)(n),
        a = (0, C.Hu)(n);
    return t || a
        ? (0, i.jsxs)('div', {
              className: N.monetizationDisclosureContainerStyle,
              children: [
                  t
                      ? (0, i.jsxs)('div', {
                            className: N.monetizationDisclosureStyle,
                            children: [
                                (0, i.jsx)(s.ShopIcon, {
                                    size: 'sm',
                                    color: s.tokens.colors.INTERACTIVE_MUTED
                                }),
                                (0, i.jsx)(s.Text, {
                                    variant: 'text-sm/normal',
                                    color: 'text-secondary',
                                    children: E.Z.Messages.APP_LAUNCHER_IAP_DISCLOSURE
                                })
                            ]
                        })
                      : null,
                  a
                      ? (0, i.jsxs)('div', {
                            className: N.monetizationDisclosureStyle,
                            children: [
                                (0, i.jsx)(s.BillIcon, {
                                    size: 'sm',
                                    color: s.tokens.colors.INTERACTIVE_MUTED
                                }),
                                (0, i.jsx)(s.Text, {
                                    variant: 'text-sm/normal',
                                    color: 'text-secondary',
                                    children: E.Z.Messages.APP_LAUNCHER_ADS_DISCLOSURE
                                })
                            ]
                        })
                      : null
              ]
          })
        : null;
}
function g(e) {
    var n;
    let { application: t } = e,
        a = (0, C.BQ)(t) ? t.name : null !== (n = (0, C.$d)(t)) && void 0 !== n ? n : '',
        l = (0, C.vJ)(t);
    return (0, i.jsxs)('div', {
        className: N.titleContainer,
        children: [
            (0, i.jsx)(s.Heading, {
                variant: 'heading-xl/extrabold',
                lineClamp: 1,
                children: a
            }),
            l
                ? (0, i.jsx)('div', {
                      className: N.partnerLabelContainer,
                      children: (0, i.jsx)(s.Text, {
                          variant: 'text-xs/medium',
                          color: 'text-normal',
                          children: E.Z.Messages.APP_DIRECTORY_PARTNER
                      })
                  })
                : null
        ]
    });
}
function P(e) {
    let { application: n } = e,
        t = a.useMemo(() => {
            var e;
            return (0, C.BQ)(n) && null !== (e = null == n ? void 0 : n.tags) && void 0 !== e ? e : [];
        }, [n]);
    return (0, C.ye)(n)
        ? (0, i.jsxs)('div', {
              className: N.tagsContainer,
              children: [
                  (0, i.jsx)(S, { application: n }),
                  t.map((e, n) =>
                      (0, i.jsx)(
                          'div',
                          {
                              className: N.tagContainer,
                              children: (0, i.jsx)(s.Text, {
                                  variant: 'text-sm/semibold',
                                  color: 'interactive-normal',
                                  children: e
                              })
                          },
                          e + n
                      )
                  )
              ]
          })
        : null;
}
function S(e) {
    var n;
    let { application: t } = e;
    if (!(0, C.ye)(t)) return null;
    let a = null !== (n = 'maxParticipants' in t ? t.maxParticipants : 'max_participants' in t ? t.max_participants : 0) && void 0 !== n ? n : 0;
    return (0, i.jsxs)('div', {
        className: N.tagContainer,
        children: [
            (0, i.jsx)(s.GroupIcon, {
                size: 'xs',
                color: s.tokens.colors.INTERACTIVE_NORMAL
            }),
            (0, i.jsx)(s.Text, {
                variant: 'text-sm/semibold',
                color: 'interactive-normal',
                children: (0, d.lY)(a)
            })
        ]
    });
}
function L(e) {
    let { channel: n, application: t, sectionName: a, hasCommands: l, isDeveloperOfThisApp: o } = e,
        { commands: r } = (0, m.SY)(n, t.id),
        c = r[0];
    return (0, C.BQ)(t)
        ? null != c
            ? (0, i.jsx)('div', {
                  className: N.entrypointContainer,
                  children: (0, i.jsx)(A.Z, {
                      channel: n,
                      application: t,
                      sectionName: a,
                      primaryEntryPointCommand: c,
                      buttonSize: s.ButtonSizes.MAX
                  })
              })
            : o && !l && (0, C.ye)(t)
              ? (0, i.jsx)(s.HelpMessage, {
                    className: N.primaryEntryPointWarningMessage,
                    messageType: s.HelpMessageTypes.WARNING,
                    children: E.Z.Messages.APPLICATION_COMMAND_PRIMARY_ENTRY_POINT_MISSING_WARNING.format()
                })
              : null
        : null;
}
function b(e) {
    let { description: n } = e,
        [t, l] = a.useState(!0);
    a.useLayoutEffect(() => l(!1), []);
    let o = a.useMemo(() => (0, _.YP)(n), [n]),
        {
            ref: r,
            lineHeight: c,
            lineCount: d
        } = (function () {
            let e = a.useRef(null),
                [n, t] = a.useState(null),
                [i, l] = a.useState(null);
            return (
                a.useLayoutEffect(() => {
                    let n = e.current;
                    if (null === n || 0 === n.clientHeight) return;
                    let i = parseInt(getComputedStyle(n).lineHeight);
                    if (!isNaN(i)) t(i), l(Math.floor(n.clientHeight / i));
                }, []),
                {
                    ref: e,
                    lineHeight: n,
                    lineCount: i
                }
            );
        })(),
        u = a.useMemo(() => {
            if (null == c || null == d) return { key: 0 };
            let e = c * d;
            return {
                key: 1,
                minHeightOverride: Math.min(e, 1 * c),
                maxHeightOverride: e
            };
        }, [d, c]),
        {
            ref: m,
            isTransitioning: p,
            onTransitionEnd: C
        } = (0, f.Z)({
            isExpanded: t,
            ...u
        }),
        h = t || p;
    return (0, i.jsxs)('div', {
        className: N.descriptionContainer,
        children: [
            (0, i.jsx)('div', {
                ref: m,
                className: N.overflowHidden,
                onTransitionEnd: C,
                children: (0, i.jsx)(s.Text, {
                    ref: r,
                    className: x.markup,
                    variant: 'text-sm/medium',
                    lineClamp: h ? void 0 : 1,
                    style: { maxHeight: h ? void 0 : u.minHeightOverride },
                    children: o
                })
            }),
            null != d && d > 1
                ? (0, i.jsxs)(s.Clickable, {
                      className: N.expandableDescriptionClickable,
                      onClick: () => l((e) => !e),
                      children: [
                          (0, i.jsx)(s.Text, {
                              variant: 'text-sm/semibold',
                              color: 'text-brand',
                              children: h ? E.Z.Messages.APP_LAUNCHER_SHOW_LESS : E.Z.Messages.APP_LAUNCHER_SHOW_MORE
                          }),
                          h
                              ? (0, i.jsx)(s.ChevronSmallUpIcon, {
                                    size: 'sm',
                                    color: s.tokens.colors.TEXT_BRAND
                                })
                              : (0, i.jsx)(s.ChevronSmallDownIcon, {
                                    size: 'sm',
                                    color: s.tokens.colors.TEXT_BRAND
                                })
                      ]
                  })
                : null
        ]
    });
}
