...
function Y(e, t, n) {
    ...
    if (null == i) {
        G.add(e), h.Z.restored(e), C.log('load returned null; early returning (guild: '.concat(e, ', database: ').concat(r, ')'));
        return;
    }
    let [a, s] = i;
    for (let n of ((0, m.ZP)(a), 0 !== t && (j += 1), G.add(e), h.Z.restored(e), u.Z.mark('\u2757', 'loaded guild channels (guild: '.concat(e, ')'), s), a)) !Object.hasOwn(O, n.id) && $((0, b._H)(n));
...
function ee(e) {
    if (null != e.channels) for (let t of (C.fileOnly('GuildCreate contained full channels for '.concat(e.id, ' #:').concat(e.channels.length)), K(e.id), G.add(e.id), h.Z.restored(e.id), e.channels)) $(t);
...
class eu extends (r = c.ZP.Store) {
    ...
    getChannelIds(e) {
        var t, n;
        return (Y(e, 0, 'getChannelIds'), null == e) ? T.default.keys(L) : T.default.keys(null !== (n = null !== (t = h.Z.getGuildBasicChannels(e)) && void 0 !== t ? t : D[e]) && void 0 !== n ? n : R);
    }
...
(s = 'ChannelStore'),
    ...
    (t.Z = new eu(d.Z, {
        BACKGROUND_SYNC: function (e) {
            ...
            (O = {}),
                ...
                t.forEach((e) => {
                    ...
                    } else C.fileOnly('BG sync contained full channels for '.concat(e.id, ' #:').concat(e.channels.length)), K(e.id), G.add(e.id), h.Z.restored(e.id), e.channels.forEach((t) => $((0, b.q_)(t, e.id)));
                });
        },
        CACHE_LOADED_LAZY: function (e) {
            for (let [t, n] of e.guildChannels) for (let e of (C.fileOnly('Lazy cache contained full guild channels for '.concat(t, ' #:').concat(n.length)), G.add(t), n)) Q((0, b._H)(e));
        },
        ...
        GUILD_DELETE: function (e) {
            C.fileOnly('GuildDelete of '.concat(e.guild.id)), K(e.guild.id), G.delete(e.guild.id), h.Z.invalidate(e.guild.id);
        },
        LOAD_ARCHIVED_THREADS_SUCCESS: en,
        LOAD_CHANNELS: function (e) {
            for (let { guildId: t, channels: n } of e.channels) for (let e of (C.fileOnly('Lazy loaded guild channels for '.concat(t)), (0, m.ZP)(n), G.add(t), h.Z.restored(t), n)) !Object.hasOwn(O, e.id) && $((0, b._H)(e));
            return !1;
        },
