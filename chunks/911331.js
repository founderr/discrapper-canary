...
var o = function (e, t) {
    ...
    for (var I = n.visualViewport ? n.visualViewport.width : innerWidth, m = n.visualViewport ? n.visualViewport.height : innerHeight, T = window.scrollX || pageXOffset, g = window.scrollY || pageYOffset, S = e.getBoundingClientRect(), A = S.height, N = S.width, O = S.top, R = S.right, v = S.bottom, C = S.left, y = 'start' === o || 'nearest' === o ? O : 'end' === o ? v : O + A / 2, L = 'center' === l ? C + N / 2 : 'end' === l ? R : C, D = [], b = 0; b < h.length; b++) {
        ...
        if (f === M) (j = 'start' === o ? y : 'end' === o ? y - m : 'nearest' === o ? s(g, g + m, m, H, Y, g + y, g + y + A, A) : y - m / 2), (W = 'start' === l ? L : 'center' === l ? L - I / 2 : 'end' === l ? L - I : s(T, T + I, I, V, Z, T + L, T + L + N, N)), (j = Math.max(0, j + g)), (W = Math.max(0, W + T));
        else {
            (j = 'start' === o ? y - x - H : 'end' === o ? y - k + Y + z : 'nearest' === o ? s(x, k, U, H, Y + z, y, y + A, A) : y - (x + U / 2) + z / 2), (W = 'start' === l ? L - B - V : 'center' === l ? L - (B + w / 2) + K / 2 : 'end' === l ? L - G + Z + K : s(B, G, w, V, Z + K, L, L + N, N));
            var X = M.scrollLeft,
                $ = M.scrollTop;
