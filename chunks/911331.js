...
var s = function (e, t) {
    var n = window, i = t.scrollMode, s = t.block, l = t.inline, u = t.boundary, c = t.skipOverflowHiddenElements, d = 'function' == typeof u ? u : function (e) {
            return e !== u;
        };
    if (!r(e))
    ...
    for (var m = n.visualViewport ? n.visualViewport.width : innerWidth, I = n.visualViewport ? n.visualViewport.height : innerHeight, T = window.scrollX || pageXOffset, g = window.scrollY || pageYOffset, S = e.getBoundingClientRect(), A = S.height, N = S.width, v = S.top, O = S.right, R = S.bottom, C = S.left, y = 'start' === s || 'nearest' === s ? v : 'end' === s ? R : v + A / 2, D = 'center' === l ? C + N / 2 : 'end' === l ? O : C, L = [], b = 0; b < h.length; b++) {
        var M = h[b], P = M.getBoundingClientRect(), U = P.height, w = P.width, x = P.top, G = P.right, k = P.bottom, B = P.left;
        if ('if-needed' === i && v >= 0 && C >= 0 && R <= I && O <= m && v >= x && R <= k && C >= B && O <= G)
            break;
        var F = getComputedStyle(M), V = parseInt(F.borderLeftWidth, 10), H = parseInt(F.borderTopWidth, 10), Z = parseInt(F.borderRightWidth, 10), Y = parseInt(F.borderBottomWidth, 10), j = 0, W = 0, K = 'offsetWidth' in M ? M.offsetWidth - M.clientWidth - V - Z : 0, z = 'offsetHeight' in M ? M.offsetHeight - M.clientHeight - H - Y : 0, q = 'offsetWidth' in M ? 0 === M.offsetWidth ? 0 : w / M.offsetWidth : 0, Q = 'offsetHeight' in M ? 0 === M.offsetHeight ? 0 : U / M.offsetHeight : 0;
        if (f === M)
            ...
            var X = M.scrollLeft, $ = M.scrollTop;
