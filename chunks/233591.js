var r = n(581031),
    ...
    L = function (e) {
        ...
        for (p && v(r, _); I < l; ) {
            ...
            var N = I + 1;
            if (S - u > T((i - E) / N)) throw m(h);
            for (E += (S - u) * N, u = S, t = 0; t < e.length; t++) {
                if ((n = e[t]) < u && ++E > i) throw m(h);
                if (n === u) {
                    for (var O = E, R = a; ; ) {
                        var L = R <= f ? o : R >= f + s ? s : R - f;
                        if (O < L) break;
                        var D = O - L,
                            M = a - L;
                        v(r, g(y(L + (D % M)))), (O = T(D / M)), (R += a);
                    }
                    v(r, g(y(O))), (f = b(E, N, I === p)), (E = 0), I++;
                }
            }
            E++, u++;
        }
...
e.exports = function (e) {
    var t,
        ...
        r = [],
        i = O(N(R(e), f, '.'), '.');
    for (t = 0; t < i.length; t++) v(r, I(E, (n = i[t])) ? 'xn--' + L(n) : n);
    return A(r, '.');
};
