...
function h(e) {
  ...
  }, [E, T, h]), p = s.useCallback(() => {
    T(g.L.DISMISS)
  }, [T]);
  s.useEffect(() => ((null == f || null == M) && T(g.L.AUTO_DISMISS), () => {}), [M, f, h, T]);
  let Z = (0, _.nP)(null == L ? void 0 : null === (a = L.branding) || void 0 === a ? void 0 : a.primaryColor, [i.tokens.colors.BG_BRAND, .7]),
  ...
  let U = R ? A.Z.Messages.CLAN_USER_ADOPT_TAG_MODAL_NEW_IDENTITY_HEADING.format({
      guildName: f.name
    }) : A.Z.Messages.CLAN_USER_ADOPT_TAG_MODAL_HEADING.format({
      guildName: f.name
    }),
    G = R ? A.Z.Messages.CLAN_USER_ADOPT_TAG_MODAL_NEW_IDENTITY_DESCRIPTION.format({
      guildName: f.name
