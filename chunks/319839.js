...
class Z {
  ...
  constructor(e, t) {
    ...
    for (let e in s) {
      let t = s[e];
      t.type === x.ChannelTypes.GUILD_CATEGORY && (a[t.id] = t, o[t.id] = [])
    }
    let u = [],
      c = [],
      C = [],
    ...
    for (let e in s) {
      ...
      if (t.type !== x.ChannelTypes.GUILD_CATEGORY) {
        ...
        eI(this, t, f) ? u.push(t) : (t.type === x.ChannelTypes.GUILD_VOICE || t.type === x.ChannelTypes.GUILD_STAGE_VOICE) && (null != t.parent_id && null != a[t.parent_id] && c.push(a[t.parent_id]), c.push(t)), null != t.parent_id && t.parent_id in o ? o[t.parent_id].push(t) : d.push(t)
      }
    }
...
class er extends ea {
  computeState(e) {
    ...
    return (a = eC(this.record, c, n, i, d.hideMutedChannels), d.optInEnabled && d.hideResourceChannels && this.record.hasFlag(k.ChannelFlags.IS_GUILD_RESOURCE_CHANNEL)) ? {
      ...
      renderLevel: 2,
      threadIds: a
    } : this.category.isCollapsed && (d.mutedChannelIds.has(this.id) || null != r && d.mutedChannelIds.has(r) || this.record.isGuildVocal() || this.record.type === x.ChannelTypes.GUILD_STORE || (0, N.isGuildReadableType)(this.record.type) && !O.default.hasUnread(this.record.id)) ? {
      renderLevel: 3,
      threadIds: a
    } : {
      renderLevel: 4,
