...
class $ {
  get initializationData() {
    return {
      ...
      activeJoinedRelevantThreads: w.default.getActiveJoinedRelevantThreadsForGuild(this.id),
      activeJoinedUnreadThreads: w.default.getActiveJoinedUnreadThreadsForGuild(this.id)
    }
  }
  invalidate() {
  ...
  isPlaceholderRow(e, t) {
    return r(e > J, "Invalid section"), e !== this.recentsSectionNumber && this.getRows()[e][t] === X
  }
...
class ei extends ee {
  ...
  constructor(e, t) {
    ...
    this.channels = h(null !== (n = U.default.getGuildFavorites(e.id)) && void 0 !== n ? n : []).map(e => R.default.getChannel(e)).filter(F.isNotNullish).map(e => new eo(this, e, t)).keyBy(e => e.id).value();
    let i = v.default.getSuggestedChannelId(e.id),
      s = R.default.getChannel(i);
...
class es extends ee {
  ...
  updateShownChannelIds(e) {
    ...
    if (e.lastMessageTimestamp > (null === (t = this.channels[this.shownChannelIds[0]]) || void 0 === t ? void 0 : t.lastMessageTimestamp)) {
      ...
      return t > -1 && this.shownChannelIds.splice(t, 1), this.shownChannelIds.splice(0, 0, e.id), this.shownChannelIds.length > es.MAX_RECENT_CHANNELS && (this.shownChannelIds = this.shownChannelIds.slice(0, es.MAX_RECENT_CHANNELS)), g.default.get("recent_channels_throttle") && (this.isThrottled = !0, setTimeout(() => {
        this.isThrottled = !1
  ...
  constructor(e, t, n) {
    if (super(e), this.enabled = !1, this.RECENTS_THROTTLE = 5 * P.default.Millis.SECOND, this.isThrottled = !1, this.isCollapsed = y.default.isCollapsed(e.id), this.enabled = function() {
        return !1
...
class el extends ee {
  updateAllChannels(e) {
    let t = !1;
    return H.default.keys(this.channels).forEach(n => {
...
class eh extends ed {
  computeState(e) {
    ...
    return (a = ef(this.record, c, n, i, d.hideMutedChannels), d.optInEnabled && d.hideResourceChannels && this.record.hasFlag(W.ChannelFlags.IS_GUILD_RESOURCE_CHANNEL)) ? {
      renderLevel: o ? 4 : 1,
      threadIds: a
    } : !d.optInEnabled || d.optedInChannels.has(this.id) || null != r && d.optedInChannels.has(r) ? o || u || !h.isEmpty(a) || b.default.getMentionCount(this.id) > 0 ? {
      renderLevel: 4,
      threadIds: a
    } : d.hideMutedChannels && d.mutedChannelIds.has(this.id) ? {
      renderLevel: 2,
      threadIds: a
    } : this.category.isCollapsed && (d.mutedChannelIds.has(this.id) || null != r && d.mutedChannelIds.has(r) || this.record.isGuildVocal() || this.record.type === k.ChannelTypes.GUILD_STORE || (0, T.isGuildReadableType)(this.record.type) && !b.default.hasUnread(this.record.id)) ? {
      renderLevel: 3,
      threadIds: a
    } : {
      renderLevel: 4,
...
class ec extends eh {
  computeState(e) {
    ...
    if (t > 1) {
      ...
      s.mutedChannelIds.has(this.id) || null != i && s.mutedChannelIds.has(i) ? t = 2 : 4 === t ? t = 3 : 2 === t && eS(this.category.guild, this.record, e) && (t = 3), 3 === t && eE(this, e) && (t = 4), n = h.sortBy(n, e => -b.default.lastMessageTimestamp(e))
    }
    return {
      renderLevel: t,
      threadIds: n
...
class eC extends eh {
  getRenderLevel(e) {
    ...
    return !D.default.can(z.Permissions.VIEW_CHANNEL, this.record) || 4 === e || 3 === e || eI(t, this.record) ? 1 : this.category.isCollapsed ? h.some(M.default.getVoiceStatesForChannel(this.record.id)) ? 4 : 3 : 4
  }
  computeState(e) {
...
function ef(e, t, n, i, s) {
  ...
  if (T.THREADED_CHANNEL_TYPES.has(e.type)) {
    ...
    else if (s) return e.filter(e => !N.default.isMuted(e));
    else return e
  }
  return []
}
...
function eS(e, t, n) {
  ...
  if (null != i && !(0, E.isInMainTabsExperiment)() && (i.id === t.id || i.isThread() && i.parent_id === t.id) || b.default.getMentionCount(t.id) > 0) return !0;
