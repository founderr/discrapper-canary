n.d(t, {
    Z: function () {
        return O;
    }
}),
    n(47120);
var r = n(200651),
    i = n(192379),
    a = n(120356),
    s = n.n(a),
    o = n(442837),
    l = n(481060),
    u = n(195302),
    c = n(206295),
    d = n(706454),
    f = n(314897),
    _ = n(158776),
    p = n(594174),
    h = n(302221),
    m = n(5192),
    g = n(358085),
    E = n(998502),
    v = n(528567),
    I = n(371583),
    b = n(246627),
    S = n(249268),
    T = n(864138),
    y = n(473188),
    A = n(169080),
    N = n(463031),
    C = n(388032),
    R = n(881894);
function O(e) {
    let { leaderboard: t, previewMode: n = !1 } = e,
        a = (0, o.e7)([f.default], () => f.default.getId()),
        s = (0, o.e7)([d.default], () => d.default.locale),
        { rankChanges: u } = (0, b.Z)({
            guildId: t.guild_id,
            leaderboardId: t.leaderboard_id,
            intervalStart: t.interval_start
        }),
        _ = i.useMemo(() => {
            let e = new Map();
            u.forEach((t) => {
                let { userId: n, previousRank: r } = t;
                e.set(n, r);
            });
            let { sort_by_statistic_id: n } = t.guild_settings;
            return t.users.map((t, r) => {
                var i, a;
                return {
                    userId: t.user_id,
                    value: null !== (a = null === (i = t.statistics[n]) || void 0 === i ? void 0 : i.value) && void 0 !== a ? a : 0,
                    rank: r + 1,
                    previousRank: e.get(t.user_id)
                };
            });
        }, [t, u]),
        p = (0, I.K)(t),
        { primaryColor: m } = (0, c.Z)(p),
        g = i.useMemo(() => {
            let e = (0, h.oo)(m),
                t = {
                    ...(0, h.pz)(e.r, e.g, e.b),
                    l: 15
                };
            return (0, h.ZJ)(t.h, t.s, t.l);
        }, [m]),
        E = i.useMemo(() => null == _.find((e) => e.userId === a) && _.length <= 10, [_, a]);
    if (0 === t.users.length) return (0, r.jsx)(D, { leaderboard: t });
    let { sort_by_statistic_id: T, sort_desc: y } = t.guild_settings,
        A = t.settings.statistics[T];
    return (0, r.jsxs)('div', {
        className: R.container,
        style: { background: g },
        children: [
            (0, r.jsx)(S.Z, {
                userId: _[0].userId,
                leaderboard: t
            }),
            n
                ? null
                : (0, r.jsx)(r.Fragment, {
                      children: (0, r.jsxs)('div', {
                          className: R.table,
                          children: [
                              (0, r.jsxs)('div', {
                                  className: R.headerRow,
                                  children: [
                                      (0, r.jsx)(l.Text, {
                                          variant: 'text-xxs/medium',
                                          className: R.headerRank,
                                          children: '#'
                                      }),
                                      (0, r.jsx)(l.Text, {
                                          variant: 'text-xxs/medium',
                                          className: R.headerPlayer,
                                          children: C.intl.string(C.t['6ii1+/'])
                                      }),
                                      (0, r.jsx)(l.Text, {
                                          variant: 'text-xxs/medium',
                                          className: R.headerValue,
                                          children: (0, v.s)({
                                              statisticId: T,
                                              sortDesc: y,
                                              aggregationType: null == A ? void 0 : A.aggregation_type
                                          })
                                      })
                                  ]
                              }),
                              (0, r.jsxs)(l.ScrollerNone, {
                                  className: R.rows,
                                  children: [
                                      _.map((e) =>
                                          (0, i.createElement)(L, {
                                              ...e,
                                              key: e.userId,
                                              guildId: t.guild_id,
                                              locale: s,
                                              leaderboard: t
                                          })
                                      ),
                                      E ? (0, r.jsx)(x, {}) : null
                                  ]
                              })
                          ]
                      })
                  })
        ]
    });
}
function D(e) {
    let { leaderboard: t } = e,
        n = (0, I.K)(t);
    return (0, r.jsx)(T.z, {
        imageUrl: n,
        children: (0, r.jsxs)('span', {
            className: R.emptyTitleContainer,
            children: [
                (0, r.jsx)(l.Text, {
                    tag: 'span',
                    variant: 'text-lg/normal',
                    className: R.emptyTitle,
                    children: C.intl.string(C.t.BfRGk5)
                }),
                (0, r.jsx)(l.LeagueOfLegendsBrandIcon, {
                    size: 'custom',
                    width: 16,
                    height: 16,
                    className: s()(R.leagueIcon, R.emptyCrownIcon)
                })
            ]
        })
    });
}
function L(e) {
    let { userId: t, rank: n, value: a, guildId: s, previousRank: c, locale: d, leaderboard: f } = e,
        h = (0, o.e7)([p.default], () => p.default.getUser(t)),
        v = m.ZP.getName(s, void 0, h),
        I = null != c && c > n,
        [b, S] = i.useState(!1),
        T = (0, o.e7)([_.Z], () => {
            let e = _.Z.getPrimaryActivity(t);
            return (null == e ? void 0 : e.application_id) != null && N.T.includes(e.application_id);
        }),
        { generatingImage: O, generateImage: D } = (0, u.d)({
            renderComponent: (e) =>
                (0, r.jsx)(A.Z, {
                    ...e,
                    userId: t,
                    leaderboard: f,
                    guildId: s
                }),
            imageOptions: { pixelRatio: 4 }
        }),
        L = async () => {
            let e = await D();
            await E.ZP.copyImage(e), S(!0);
        };
    return (0, r.jsxs)('div', {
        className: R.userRow,
        children: [
            I ? (0, r.jsx)('div', { className: R.rankChangeIndicator }) : null,
            (0, r.jsx)('div', {
                className: R.userRank,
                children:
                    n <= 3
                        ? (0, r.jsx)(y._, { rank: n })
                        : (0, r.jsx)(l.Text, {
                              tabularNumbers: !0,
                              color: 'text-muted',
                              variant: 'text-xs/medium',
                              children: n
                          })
            }),
            (0, r.jsxs)('div', {
                className: R.userPlayer,
                children: [
                    (0, r.jsx)(l.Avatar, {
                        src: null == h ? void 0 : h.getAvatarURL(s, 16),
                        size: l.AvatarSizes.SIZE_16,
                        'aria-label': 'avatar'
                    }),
                    (0, r.jsx)(l.Spacer, {
                        size: 6,
                        horizontal: !0
                    }),
                    (0, r.jsx)(l.Heading, {
                        variant: 'heading-sm/medium',
                        color: 'header-primary',
                        children: v
                    }),
                    1 === n
                        ? (0, r.jsxs)(r.Fragment, {
                              children: [
                                  (0, r.jsx)(l.Spacer, {
                                      size: 4,
                                      horizontal: !0
                                  }),
                                  (0, r.jsx)(l.LeagueOfLegendsBrandIcon, {
                                      size: 'custom',
                                      width: 14,
                                      height: 14
                                  })
                              ]
                          })
                        : null,
                    I ? (0, r.jsx)(w, { rankChange: c - n }) : null
                ]
            }),
            (0, r.jsxs)('div', {
                className: R.badgesContainer,
                children: [
                    T &&
                        (0, r.jsx)(l.GameControllerIcon, {
                            size: 'xs',
                            color: l.tokens.colors.TEXT_POSITIVE,
                            className: R.liveIcon
                        }),
                    g.isPlatformEmbedded &&
                        (0, r.jsx)(l.Tooltip, {
                            text: b ? C.intl.string(C.t.ZO0ku7) : C.intl.string(C.t.mikfoa),
                            shouldShow: !O,
                            onTooltipHide: () => S(!1),
                            children: (e) =>
                                (0, r.jsx)(l.Clickable, {
                                    ...e,
                                    onClick: O ? void 0 : L,
                                    className: R.copyLinkContainer,
                                    children: O
                                        ? (0, r.jsx)(l.Spinner, {
                                              type: l.Spinner.Type.SPINNING_CIRCLE_SIMPLE,
                                              className: R.copySpinner
                                          })
                                        : (0, r.jsx)(l.LinkIcon, {
                                              size: 'xs',
                                              className: R.copyLink
                                          })
                                })
                        })
                ]
            }),
            (0, r.jsx)(l.Heading, {
                variant: 'heading-sm/medium',
                color: 'header-primary',
                tabularNumbers: !0,
                children: a.toLocaleString(d)
            })
        ]
    });
}
function x() {
    return (0, r.jsxs)('div', {
        className: R.joinLeaderboardCTAContainer,
        children: [
            (0, r.jsx)(l.Text, {
                variant: 'text-xs/normal',
                className: R.joinLeaderboardCTA,
                children: C.intl.string(C.t.BfRGk5)
            }),
            (0, r.jsx)(l.LeagueOfLegendsBrandIcon, {
                size: 'custom',
                width: 14,
                height: 14,
                className: R.leagueIcon
            })
        ]
    });
}
function w(e) {
    let { rankChange: t } = e;
    return (0, r.jsxs)('div', {
        className: R.rankChangeBadge,
        children: [
            (0, r.jsx)(l.ArrowSmallUpIcon, {
                size: 'xxs',
                color: l.tokens.colors.TEXT_MUTED
            }),
            (0, r.jsx)(l.Text, {
                variant: 'text-xs/medium',
                color: 'text-muted',
                children: t
            })
        ]
    });
}
