...
let {
  ...
  let s = c.Endpoints.STICKER_ASSET(e.id, O(i));
  if ("development" !== T) {
    if (e.format_type === d.StickerFormat.LOTTIE) return "".concat(location.protocol).concat(f).concat(s);
    let i = e.format_type === d.StickerFormat.APNG && t && !(0, l.isAndroid)() ? "&passthrough=false" : "",
      a = Math.min(2, (0, r.getDevicePixelRatio)());
    return "".concat(location.protocol).concat(I).concat(s, "?size=").concat((0, r.getBestMediaProxySize)(n * a)).concat(i)
  }
