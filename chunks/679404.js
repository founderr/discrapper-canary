...
let L = "lastFrecencySavedTime",
  C = Math.random() * (5 * A.default.Millis.MINUTE),
  D = 1 * A.default.Millis.DAY + Math.floor(Math.random() * (1 * A.default.Millis.HOUR)),
  c = null,
  d = Date.now();
...
async function P() {
  d = Date.now(), m(!0), !R.default.hasLoaded(u.UserSettingsTypes.FRECENCY_AND_FAVORITES_SETTINGS) && (S.default.hasPendingUsage() || s.default.hasPendingUsage() || i.default.hasPendingUsage() || I.default.hasPendingUsage() || N.default.hasPendingUsage()) && await O.FrecencyUserSettingsActionCreators.loadIfNecessary(), o.forEach(O.UserSettingsActionCreatorsByType, e => {
