        ...
        if (t.action === c.AuditLogActions.MEMBER_ROLE_UPDATE ? u = function(e) {
          ...
          let {
            ...
            let {
              title: _,
              id: a
            } = s, u = e.role_ids.map(n).filter(g.isNotNullish), A = e.channel_ids.map(T).filter(g.isNotNullish), I = s.role_ids.map(n).filter(g.isNotNullish), L = s.channel_ids.map(T).filter(g.isNotNullish), o = i().difference(I, u), r = i().difference(u, I), d = i().difference(L, A), S = i().difference(A, L), N = [];
            return (!i().isEqual(e.title, s.title) && N.push("title"), !i().isEqual(e.description, s.description) && N.push("description"), 0 === o.length && 0 === r.length && 0 === d.length && 0 === S.length && 0 === N.length) ? null : (0, E.jsxs)("li", {
          ...
          }, A = i().keyBy(null != a ? a : [], "id"), I = i().keyBy(null != _ ? _ : [], "id"), L = i().difference(Object.keys(I), Object.keys(A)).map(e => I[e]), d = i().difference(Object.keys(A), Object.keys(I)).map(e => A[e]), S = i().intersection(Object.keys(I), Object.keys(A));
          return (0, E.jsxs)("ul", {
