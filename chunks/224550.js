...
n.Z = function(e) {
  ...
  } else null != T && (n = T.interval, a = T.intervalCount);
  let S = (0, s.K)({
    ...
    v = "";
  if (N) {
    if (null != E && (null == P ? void 0 : P.subscriptionPeriodEnd) != null) b = _.Z.Messages.EMBEDDED_ACTIVITIES_IAP_NON_REFUNDABLE_SUBSCRIPTION_RATE_AND_RENEWAL.format({
      rate: E,
      renewalDate: P.subscriptionPeriodEnd
    });
    else switch (n) {
      case d.rV.MONTH:
        b = 1 === a ? _.Z.Messages.EMBEDDED_ACTIVITIES_IAP_NON_REFUNDABLE_SUBSCRIPTION_MONTHLY : _.Z.Messages.EMBEDDED_ACTIVITIES_IAP_NON_REFUNDABLE_SUBSCRIPTION_MULTI_MONTH.format({
          intervalCount: a
        });
        break;
      case d.rV.YEAR:
  ...
  } else if (R === u.GZQ.ONE_TIME) v = _.Z.Messages.BILLING_LEGAL_MUMBO_JUMBO_TERMS_LABEL_V2.format({
    primaryText: S,
    paidURL: u.EYA.PAID_TERMS
  ...
  else if (null == T || A) switch (A && (v = _.Z.Messages.BILLING_LEGAL_MUMBO_JUMBO_TERMS_LABEL_V2.format({
      primaryText: S,
      paidURL: u.EYA.PAID_TERMS
    })), n) {
    case d.rV.MONTH:
      b = A ? _.Z.Messages.PREMIUM_PAYMENT_GIFT_SUBTEXT_MONTHLY : _.Z.Messages.SUBSCRIPTION_PAYMENT_LEGALESE_MONTHLY, b = A ? _.Z.Messages.PREMIUM_PAYMENT_GIFT_SUBTEXT_MONTHLY : 1 === a ? _.Z.Messages.SUBSCRIPTION_PAYMENT_LEGALESE_MONTHLY : _.Z.Messages.SUBSCRIPTION_PAYMENT_LEGALESE_MULTI_MONTH.format({
        intervalCount: a
      });
      break;
    case d.rV.YEAR:
