...
t.Z = {
    [p.Etm.SET_ACTIVITY]: {
        ...
        handler(e) {
            ...
            let { assets: I, party: C, secrets: v, timestamps: S, buttons: T, type: N } = h;
            if ((null == N && (h.type = p.IIU.PLAYING), null != v)) {
                let e = r()
                    .values(v)
                    .filter((e) => !!e);
                ...
                if (null != T) throw new d.Z({ errorCode: p.lTL.INVALID_ACTIVITY_SECRET }, 'secrets cannot currently be sent with buttons');
            }
            if ((null != T && ((h.metadata = { button_urls: T.map((e) => e.url) }), (h.buttons = T.map((e) => e.label))), null != S)) for (let e of Object.keys(S)) Date.now().toString().length - S[e].toString().length > 2 && (S[e] = Math.floor(S[e] * c.Z.Millis.SECOND));
