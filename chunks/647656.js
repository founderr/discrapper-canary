...
t.Z = {
    [E.Etm.SET_ACTIVITY]: {
        scope: {
            [h.Gp.ANY]: [s.x.RPC, s.x.RPC_ACTIVITIES_WRITE, h.lH]
        },
        validation: (e) =>
            (0, _.Z)(e)
                .required()
        ...
        handler(e) {
            ...
            if (![h.He.IPC, h.He.WEBSOCKET, h.He.POST_MESSAGE].includes(i.transport)) throw new d.Z({ errorCode: E.lTL.INVALID_COMMAND }, 'command not available from "'.concat(i.transport, '" transport'));
            if (null == s && h.He.IPC === i.transport) throw new d.Z({ errorCode: E.lTL.INVALID_COMMAND }, 'nonzero pid required');
            if (null == _)
                return (
                    r.Z.dispatch({
            ...
            let { assets: T, party: S, secrets: C, timestamps: f, buttons: N, type: A } = _;
            if ((null == A && (_.type = E.IIU.PLAYING), null != C)) {
                let e = a()
                    .values(C)
                    .filter((e) => !!e);
            ...
            if ((null != N && ((_.metadata = { button_urls: N.map((e) => e.url) }), (_.buttons = N.map((e) => e.label))), null != f)) for (let e of Object.keys(f)) Date.now().toString().length - f[e].toString().length > 2 && (f[e] = Math.floor(f[e] * c.Z.Millis.SECOND));
            ...
            return n.then((e) => {
                var t, n;
                let [a, o] = e;
                if ((null != T && (null != a ? (T.large_image = a) : delete T.large_image, null != o ? (T.small_image = o) : delete T.small_image), !I())) return;
                r.Z.dispatch({
                    type: 'LOCAL_ACTIVITY_UPDATE',
                    socketId: i.id,
                    pid: s,
