...
t.Z = {
  [h.Etm.SET_ACTIVITY]: {
    scope: {
      [E.Gp.ANY]: [a.x.RPC, a.x.RPC_ACTIVITIES_WRITE, E.lH]
    },
    ...
    handler(e) {
      ...
      if (![E.He.IPC, E.He.WEBSOCKET, E.He.POST_MESSAGE].includes(i.transport)) throw new d.Z({
        errorCode: h.lTL.INVALID_COMMAND
      }, 'command not available from "'.concat(i.transport, '" transport'));
      if (null == a && E.He.IPC === i.transport) throw new d.Z({
        errorCode: h.lTL.INVALID_COMMAND
      }, "nonzero pid required");
      if (null == _) return r.Z.dispatch({
      ...
      if ((null == A || A !== h.IIU.PLAYING && !p) && (_.type = h.IIU.PLAYING), null != f) {
        ...
        if (s().uniq(e).length < e.length) throw new d.Z({
          errorCode: h.lTL.INVALID_ACTIVITY_SECRET
