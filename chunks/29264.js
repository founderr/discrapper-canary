"use strict";
n.r(t), n.d(t, {
  default: function() {
    return _
  }
}), n("47120");
var a = n("735250");
n("470079");
var i = n("481060"),
  l = n("668781"),
  s = n("829883"),
  r = n("976853"),
  u = n("626135"),
  o = n("358085"),
  d = n("960048"),
  c = n("591759"),
  f = n("998502"),
  M = n("36998"),
  E = n("981631"),
  m = n("689938");
let g = "https://media.discordapp.net",
  v = /^.*\.discordapp\.net$/,
  I = "cdn.discordapp.com",
  h = "".concat(g, "/stickers"),
  p = new Set(["jpg", "jpeg", "png", "webp", "gif", "tiff", "bmp"]),
  S = new Set(["jpg", "jpeg", "png"]),
  A = e => {
    var t, n, a, i;
    return null === (i = c.default.toURLSafe(e)) || void 0 === i ? void 0 : null === (a = i.pathname) || void 0 === a ? void 0 : null === (n = a.split(".")) || void 0 === n ? void 0 : null === (t = n.pop()) || void 0 === t ? void 0 : t.toLowerCase()
  };

function C(e, t) {
  l.default.show({
    title: m.default.Messages.ERROR,
    body: e
  }), d.default.captureException(t)
}

function _(e, t, n) {
  if ((0, r.default)(null == t ? void 0 : t.getChannelId()) || (null == n ? void 0 : n.shouldHideMediaOptions) === !0 || !o.isPlatformEmbedded || null == e || ! function(e) {
      let t = c.default.toURLSafe(e);
      if (null == t) return !1;
      let n = A(e);
      return (v.test(t.hostname) || t.host === I) && !e.startsWith(h) && !(0, s.isRoleIconAssetUrl)(e) && null != n && p.has(n)
    }(e)) return null;
  let l = function(e) {
      let t = c.default.toURLSafe(e);
      return null == t || t.host === I ? e : t.origin === g ? (t.host = I, t.searchParams.delete("size"), t.searchParams.delete("width"), t.searchParams.delete("height"), t.searchParams.delete("quality"), t.searchParams.delete("format"), t.toString()) : (t.searchParams.delete("width"), t.searchParams.delete("height"), t.searchParams.set("quality", "lossless"), t.toString())
    }(e),
    d = async () => {
      try {
        await f.default.saveImage(l), u.default.track(E.AnalyticEvents.CONTEXT_MENU_IMAGE_SAVED, {
          ...(0, M.getNativeContextMenuChannelAnalytics)()
        })
      } catch (e) {
        u.default.track(E.AnalyticEvents.CONTEXT_MENU_IMAGE_SAVE_FAILED, {
          ...(0, M.getNativeContextMenuChannelAnalytics)()
        }), C(m.default.Messages.ERROR_SAVING_IMAGE, e)
      }
    }, _ = async () => {
      try {
        await f.default.copyImage(l), u.default.track(E.AnalyticEvents.CONTEXT_MENU_IMAGE_COPIED, {
          ...(0, M.getNativeContextMenuChannelAnalytics)()
        })
      } catch (e) {
        C(m.default.Messages.ERROR_COPYING_IMAGE, e), u.default.track(E.AnalyticEvents.CONTEXT_MENU_IMAGE_COPY_FAILED, {
          ...(0, M.getNativeContextMenuChannelAnalytics)()
        })
      }
    };
  return [f.default.canCopyImage() && function(e) {
    let t = A(e);
    return null != t && S.has(t)
  }(e) ? (0, a.jsx)(i.MenuItem, {
    id: "copy-image",
    label: m.default.Messages.COPY_IMAGE_MENU_ITEM,
    action: _
  }, "copy-image") : null, (0, a.jsx)(i.MenuItem, {
    id: "save-image",
    label: m.default.Messages.SAVE_IMAGE_MENU_ITEM,
    action: d
  }, "save-image")]
}