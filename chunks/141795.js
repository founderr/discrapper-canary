...
class C extends A.ZP {
  ...
  async prepareChunkUploadItem() {
    ...
    if (this.item.platform === A.ow.REACT_NATIVE) {
      ...
      if (e = null != i.mimeType && "" !== i.mimeType ? i.mimeType : "application/octet-stream", null == i.size || 0 === i.size || isNaN(i.size)) try {
        ...
        R.warn("Failed to peek content length for file id ".concat(this.id, ", reading whole file instead: ").concat(e)), t = (n = await (0, S.Lc)(i.uri)).size
      } else t = i.size
  ...
  uploadFileToCloud() {
    ...
    R.log("Uploading ".concat(this.id)), this.item.platform === A.ow.REACT_NATIVE ? t = null != (e = {
  ...
  async upload() {
    ...
    try {
      ...
      e !== m.evJ.ENTITY_TOO_LARGE && (R.error("Requesting upload url failed with code ".concat(null != e ? e : JSON.stringify(i.body), " for ").concat(this.id)), f.Z.captureException(i)), this.handleError(e);
      return
    }
    try {
      let e;
  ...
  async reactNativeCompressAndExtractData() {
    var e;
    if (!(0, N.F)(this.item.target).shouldReactNativeCompressUploads) return this.uploadAnalytics.compressAndExtractDisabled = !0, R.log("reactNativeCompressAndExtractData() disabled by upload target"), this;
