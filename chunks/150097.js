...
function y(e, t) {
	...
	if (S.Z.isBlockedForMessage(e) || (void 0 !== e.activity_instance && null != e.interaction && e.interaction.user.id === a.id)) return !1;
	if (T.Ec.has(u.type)) {
...
function w(e, t, n) {
	...
	let C = t.content;
	if ((0, d.Z)(t) && null == (C = M.Z.stringify(t, e))) throw (new l.Z('NotificationTextUtils').warn('SystemMessageUtils.stringify(...) could not convert', t), Error('failed to stringify system message'));
	...
	if ((0, u.s)(t)) c = L.Z.Messages.MESSAGE_FORWARDED_NOTIFICATION_BODY;
		...
		c =
			t.activity.type === R.mFx.JOIN
				? V(e, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GUILD_ACTIVITY_JOIN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GROUP_DM_ACTIVITY_JOIN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_DM_ACTIVITY_JOIN).format({
						user: T,
						game: t.application.name
					})
				: t.activity.type === R.mFx.JOIN_REQUEST
					? V(e, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_DM_ACTIVITY_JOIN_REQUEST, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_DM_ACTIVITY_JOIN_REQUEST, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_DM_ACTIVITY_JOIN_REQUEST).format({
							user: T,
							game: t.application.name
						})
					: '';
	else if (null != t.activity && t.activity.type === R.mFx.LISTEN) c = V(e, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GUILD_ACTIVITY_LISTEN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GROUP_DM_ACTIVITY_LISTEN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_DM_ACTIVITY_LISTEN).format({ user: T });
	else if (null != A && A.length > 0) c = L.Z.Messages.STICKER_NOTIFICATION_BODY.format({ stickerName: A[0].name });
	else if (t.type === R.uaV.PREMIUM_REFERRAL) c = L.Z.Messages.SHARE_NITRO_EMBEDDED_MESSAGE_BODY_SHORT.format({ username: p.ZP.getName(n) });
	...
	return (
		0 === c.length &&
			(c = (function (e) {
				...
				if ((0, D.yE)(null !== (t = e.flags) && void 0 !== t ? t : 0, R.iLy.IS_VOICE_MESSAGE)) return L.Z.Messages.VOICE_MESSAGES_SENT_NOTIFICATION;
