...
function w(e, t, n) {
  ...
  else if (null != t.activity && null != t.application) c = t.activity.type === R.mFx.JOIN ? V(e, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GUILD_ACTIVITY_JOIN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GROUP_DM_ACTIVITY_JOIN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_DM_ACTIVITY_JOIN).format({
    user: T,
  ...
  else if (null != t.activity && t.activity.type === R.mFx.LISTEN) c = V(e, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GUILD_ACTIVITY_LISTEN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_GROUP_DM_ACTIVITY_LISTEN, L.Z.Messages.NOTIFICATION_MESSAGE_CREATE_DM_ACTIVITY_LISTEN).format({
    user: T
  });
  else if (null != A && A.length > 0) c = L.Z.Messages.STICKER_NOTIFICATION_BODY.format({
    stickerName: A[0].name
  });
  else if (t.type === R.uaV.PREMIUM_REFERRAL) c = L.Z.Messages.SHARE_NITRO_EMBEDDED_MESSAGE_BODY_SHORT.format({
    username: p.ZP.getName(n)
  });
  else if (null != t.poll) c = L.Z.Messages.POLL_SENT_NOTIFICATION.format({
