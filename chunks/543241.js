"use strict";
n.d(t, {
  A3: function() {
    return v
  },
  C1: function() {
    return V
  },
  DV: function() {
    return x
  },
  Gn: function() {
    return y
  },
  IP: function() {
    return H
  },
  J1: function() {
    return b
  },
  MZ: function() {
    return Z
  },
  NJ: function() {
    return k
  },
  Nf: function() {
    return G
  },
  PC: function() {
    return L
  },
  Y1: function() {
    return w
  },
  fe: function() {
    return U
  },
  jA: function() {
    return D
  },
  kI: function() {
    return C
  },
  nV: function() {
    return F
  },
  to: function() {
    return P
  },
  wC: function() {
    return B
  },
  y$: function() {
    return M
  }
}), n(733860), n(653041), n(47120), n(724458), n(627341), n(757143);
var i = n(470079),
  r = n(392711),
  s = n(278074),
  o = n(442837),
  a = n(367907),
  l = n(339085);
n(353842);
var u = n(558217),
  _ = n(675478),
  c = n(430824),
  d = n(771845),
  E = n(594174),
  I = n(626135),
  T = n(70956),
  h = n(176354),
  f = n(111361);
n(709054);
var S = n(304852),
  A = n(199257),
  N = n(149203),
  m = n(981631),
  O = n(185923),
  p = n(957825),
  R = n(474936),
  g = n(689938);
let C = function(e, t) {
    let n = arguments.length > 2 && void 0 !== arguments[2] ? arguments[2] : null == t ? void 0 : t.getGuildId();
    (0, u.O)(n);
    let r = (0, O.Gt)(e),
      s = B(n),
      a = k(n),
      {
        topEmojis: _,
        newlyAddedEmojis: I
      } = (0, A.Z)(n, e),
      {
        allEmojis: T
      } = (0, S.Z)({
        topEmojis: _,
        newlyAddedEmojis: I
      }),
      m = (0, o.e7)([l.Z], () => l.Z.getDisambiguatedEmojiContext(n), [n]),
      p = (0, o.e7)([c.Z], () => {
        var e;
        return null === (e = c.Z.getGuild(n)) || void 0 === e ? void 0 : e.name
      }),
      R = (0, o.e7)([E.default], () => E.default.getCurrentUser()),
      C = (0, f.I5)(R);
    return i.useMemo(() => {
      let i = m.getGroupedCustomEmoji(),
        o = d.ZP.getFlattenedGuildIds(),
        u = [],
        _ = (i, r) => h.ZP.getEmojiUnavailableReasons({
          categoryEmojis: i,
          channel: t,
          guildId: n,
          intention: e,
          computeUnfiltered: r
        });
      return ((e, r) => {
        for (let s of e) {
          let e;
          if (r === N.En.GUILD && (e = c.Z.getGuild(s)), null == e) continue;
          let o = null == i ? void 0 : i[e.id];
          if (null == o || 0 === o.length) continue;
          let {
            emojisDisabled: a,
            emojisFilteredCount: l,
            emojisPremiumLockedCount: d,
            emojiNitroLocked: E
          } = _(o, !1);
          if (null != t && o.length === l) continue;
          let I = null;
          r === N.En.GUILD && (I = {
            type: N.En.GUILD,
            guild: e,
            isNitroLocked: !C && E && d === o.length,
            emojis: o,
            emojisDisabled: a
          }), null != I && (e.id === n ? u.unshift(I) : u.push(I))
        }
      })(o, N.En.GUILD), l.Z.categories.reduce((e, t) => {
        if (t === N.UX.TOP_GUILD_EMOJI) {
          let {
            emojisDisabled: n,
            emojisUnfiltered: i
          } = _(T, !0);
          if (null == i || 0 === i.length) return e;
          e.push({
            type: N.En.TOP_GUILD_EMOJI,
            id: t,
            name: g.Z.Messages.EMOJI_CATEGORY_TOP_GUILD_EMOJI.format({
              guildName: p
            }),
            isNitroLocked: !1,
            emojis: i,
            emojisDisabled: n
          })
        } else if (t === N.UX.RECENT) {
          let {
            emojisDisabled: n,
            emojisUnfiltered: i
          } = _(s, !0);
          if (null == i || 0 === i.length) return e;
          e.push({
            type: N.En.RECENT,
            id: t,
            name: g.Z.Messages.EMOJI_CATEGORY_RECENT,
            isNitroLocked: !1,
            emojis: i,
            emojisDisabled: n
          })
        } else if (t === N.UX.FAVORITES) {
          let {
            emojisDisabled: n,
            emojisUnfiltered: i
          } = _(a, !0);
          if (null == i || 0 === i.length) return e;
          e.push({
            type: N.En.FAVORITES,
            id: t,
            name: g.Z.Messages.CATEGORY_FAVORITE,
            isNitroLocked: !1,
            emojis: i,
            emojisDisabled: n
          })
        } else if (t === N.UX.CUSTOM) {
          let t = u;
          !r && (t = u.filter(e => e.type === N.En.GUILD && e.guild.id === n)), e.push(...t)
        } else e.push({
          type: N.En.UNICODE,
          id: t,
          name: t,
          isNitroLocked: !1
        });
        return e
      }, [])
    }, [m, t, n, e, T, p, s, a, r, C])
  },
  v = e => {
    let t = (null == e ? void 0 : e.getGuildId()) != null;
    I.default.track(m.rMx.PREMIUM_PROMOTION_OPENED, {
      location_page: t ? m.ZY5.GUILD_CHANNEL : m.ZY5.DM_CHANNEL,
      location_section: null != e ? m.jXE.EMOJI_PICKER_POPOUT : m.jXE.CUSTOM_STATUS_MODAL
    })
  },
  L = (e, t) => {
    a.ZP.trackWithMetadata(m.rMx.SEARCH_STARTED, {
      search_type: null != t && t === O.Hz.REACTION ? m.aib.EMOJI_REACTION : m.aib.EMOJI,
      location: e
    })
  },
  D = (e, t, n, i, r) => {
    a.ZP.trackWithMetadata(m.rMx.SEARCH_RESULT_VIEWED, {
      search_type: null != r && r === O.Hz.REACTION ? m.aib.EMOJI_REACTION : m.aib.EMOJI,
      total_results: e,
      num_results_locked: t,
      query: i,
      location: n
    })
  },
  M = e => {
    var t, n, i;
    let {
      emoji: r,
      emojiSuggestions: o,
      searchQuery: l,
      isLocked: u,
      location: _,
      intention: c,
      index: d
    } = e, E = null !== (i = r.uniqueName) && void 0 !== i ? i : r.name, I = (0, s.EQ)(c).with(O.Hz.REACTION, () => m.aib.EMOJI_REACTION).with(O.Hz.AUTO_SUGGESTION, () => m.aib.EMOJI_AUTO_SUGGESTION).otherwise(() => m.aib.EMOJI);
    a.ZP.trackWithMetadata(m.rMx.SEARCH_RESULT_SELECTED, {
      search_type: I,
      location: _,
      expression_guild_id: r.guildId,
      emoji_id: r.id,
      emoji_name: E,
      is_custom: null != r.id,
      is_animated: r.animated,
      is_locked: u,
      query: l,
      index_num: d,
      load_id: null == o ? void 0 : o.loadId,
      total_results: null == o ? void 0 : null === (t = o.results) || void 0 === t ? void 0 : t.length,
      emoji_suggestion_ids: null == o ? void 0 : null === (n = o.results) || void 0 === n ? void 0 : n.map(e => {
        let {
          emoji: t
        } = e;
        return t.id
      }).filter(e => null != e)
    })
  },
  P = (e, t) => {
    a.ZP.trackWithMetadata(m.rMx.SEARCH_RESULT_EMPTY, {
      search_type: m.aib.EMOJI,
      query: t,
      location: e
    })
  },
  y = e => {
    let {
      emoji: t,
      subCategory: n,
      position: i,
      newlyAddedHighlight: r
    } = e;
    a.ZP.trackWithMetadata(m.rMx.EXPRESSION_PICKER_EXPRESSION_FOCUS, {
      expression_section: null == n ? void 0 : n.toString(),
      newly_added_highlight: r,
      emoji_id: t.id,
      emoji_name: t.name,
      emoji_animated: t.animated,
      emoji_position: i
    })
  },
  U = e => {
    var t;
    let n, {
      emoji: i,
      location: r,
      pickerIntention: s,
      category: o,
      subCategory: l = N.t0.NONE,
      position: u,
      newlyAddedHighlight: _,
      isBurstReaction: c
    } = e;
    switch (s) {
      case O.Hz.REACTION:
        n = c ? R.cd.EMOJI_PICKER_SUPER_REACTION_EMOJI_CLICKED : R.cd.EMOJI_PICKER_REACTION_EMOJI_CLICKED;
        break;
      case O.Hz.STATUS:
        n = R.cd.EMOJI_PICKER_STATUS_EMOJI_CLICKED;
        break;
      default:
        n = R.cd.EMOJI_PICKER_EMOJI_CLICKED
    }
    let d = null !== (t = i.uniqueName) && void 0 !== t ? t : i.name;
    a.ZP.trackWithMetadata(m.rMx.EXPRESSION_PICKER_EXPRESSION_SELECTED, {
      type: n,
      location: r,
      expression_id: i.id,
      expression_name: d,
      expression_guild_id: i.guildId,
      is_custom: null != i.id,
      is_animated: i.animated,
      expression_picker_section: o,
      expression_section: null == l ? void 0 : l.toString(),
      emoji_position: u,
      newly_added_highlight: _,
      is_burst: c
    })
  },
  b = e => {
    var t;
    let {
      emoji: n,
      location: i
    } = e, r = null !== (t = n.uniqueName) && void 0 !== t ? t : n.name;
    a.ZP.trackWithMetadata(m.rMx.EXPRESSION_FAVORITED, {
      location: i,
      expression_type: p.X1.EMOJI,
      expression_id: n.id,
      expression_name: r,
      expression_guild_id: n.guildId,
      is_custom: null != n.id,
      is_animated: n.animated
    })
  };
(0, r.throttle)(e => {
  let {
    emojiSuggestions: t,
    analyticsLocation: n
  } = e;
  a.ZP.trackWithMetadata(m.rMx.AUTO_SUGGEST_DISPLAYED, {
    suggestion_type: m.Z9p.EMOJI,
    suggestion_quantity: t.results.length,
    custom_quantity: t.results.filter(e => {
      let {
        emoji: t
      } = e;
      return null != t.id
    }).length,
    load_id: t.loadId,
    location: n
  })
}, T.Z.Millis.HALF_SECOND, {
  leading: !1,
  trailing: !0
});
let G = (e, t) => {
    if (e.type === N.En.GUILD) return null != t ? t.toString() : "";
    return e.id
  },
  w = (e, t) => {
    switch (e) {
      case N.UX.TOP_GUILD_EMOJI:
        return g.Z.Messages.EMOJI_CATEGORY_TOP_GUILD_EMOJI.format({
          guildName: t
        });
      case N.UX.RECENT:
        return g.Z.Messages.EMOJI_CATEGORY_RECENT;
      case N.UX.FAVORITES:
        return g.Z.Messages.CATEGORY_FAVORITE;
      case N.UX.ACTIVITY:
        return g.Z.Messages.EMOJI_CATEGORY_ACTIVITY;
      case N.UX.FLAGS:
        return g.Z.Messages.EMOJI_CATEGORY_FLAGS;
      case N.UX.FOOD:
        return g.Z.Messages.EMOJI_CATEGORY_FOOD;
      case N.UX.NATURE:
        return g.Z.Messages.EMOJI_CATEGORY_NATURE;
      case N.UX.OBJECTS:
        return g.Z.Messages.EMOJI_CATEGORY_OBJECTS;
      case N.UX.PEOPLE:
        return g.Z.Messages.EMOJI_CATEGORY_PEOPLE;
      case N.UX.SYMBOLS:
        return g.Z.Messages.EMOJI_CATEGORY_SYMBOLS;
      case N.UX.TRAVEL:
        return g.Z.Messages.EMOJI_CATEGORY_TRAVEL;
      case N.UX.PREMIUM_UPSELL:
        return g.Z.Messages.EMOJI_CATEGORY_PREMIUM_UPSELL;
      default:
        return null != t ? t : e
    }
  };

function x(e, t, n) {
  i.useEffect(() => {
    _.DZ.loadIfNecessary()
  }, []);
  let r = (0, O.Gt)(n);
  return (0, o.e7)([l.Z], () => {
    let i = e.replace(/^:/, "").replace(/:$/, "");
    return "" === i ? null : l.Z.searchWithoutFetchingLatest({
      channel: t,
      query: i,
      count: 0,
      intention: n,
      includeExternalGuilds: r
    })
  }, [t, n, e, r], o.pF)
}

function B(e) {
  return i.useEffect(() => {
    _.DZ.loadIfNecessary()
  }, []), (0, o.Wu)([l.Z], () => l.Z.getDisambiguatedEmojiContext(e).getFrequentlyUsedEmojisWithoutFetchingLatest())
}

function k(e) {
  return i.useEffect(() => {
    _.DZ.loadIfNecessary()
  }, []), (0, o.Wu)([l.Z], () => l.Z.getDisambiguatedEmojiContext(e).favoriteEmojisWithoutFetchingLatest)
}

function V(e, t) {
  return i.useEffect(() => {
    _.DZ.loadIfNecessary()
  }, []), (0, o.e7)([l.Z], () => null != t && l.Z.getDisambiguatedEmojiContext(e).isFavoriteEmojiWithoutFetchingLatest(t))
}

function Z(e) {
  return i.useEffect(() => {
    _.DZ.loadIfNecessary()
  }, []), (0, o.Wu)([l.Z], () => l.Z.getDisambiguatedEmojiContext(e).getEmojiInPriorityOrderWithoutFetchingLatest())
}

function H(e, t, n) {
  if (null == n) return N.t0.NONE;
  let i = e.map(e => {
      var t, n;
      return null !== (n = null !== (t = e.id) && void 0 !== t ? t : e.uniqueName) && void 0 !== n ? n : e.name
    }),
    r = t.map(e => e.id);
  return i.includes(n) ? N.t0.TOP_GUILD_EMOJI : r.includes(n) ? N.t0.NEWLY_ADDED_EMOJI : N.t0.NONE
}

function F(e, t) {
  return e === O.Hz.REACTION ? t ? g.Z.Messages.SEARCH_FOR_SUPER_REACTION : g.Z.Messages.SEARCH_FOR_REACTION : g.Z.Messages.SEARCH_FOR_EMOJI
}