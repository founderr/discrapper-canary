...
function c(e, t) {
  if (e.length > t) throw Error(l.Z.Messages.GUILD_AUTOMOD_KEYWORD_ERROR_TOO_MANY_KEYWORDS.format({
    limit: t
  }));
  e.forEach(e => {
    if (e.length > s.hu || e.length < s.Vk) throw new E.V6(l.Z.Messages.GUILD_AUTOMOD_KEYWORD_ERROR_INVALID_KEYWORD_LENGTH.format({
      keyword: e,
      max: s.hu,
      min: s.Vk
    }))
...
function M(e) {
  if (I(e)) {
    ...
    if (0 === r.length && 0 === a.length) throw Error(l.Z.Messages.GUILD_AUTOMOD_KEYWORD_ERROR_NO_KEYWORDS_OR_REGEX);
    c(r, s.RH), ! function(e) {
      if (e.length > s.VW) throw Error(l.Z.Messages.GUILD_AUTOMOD_KEYWORD_ERROR_TOO_MANY_REGEX.format({
        limit: s.VW
      }));
      e.forEach(e => {
        if (e.length > s.aj || e.length < s.uE) throw new E.uS(l.Z.Messages.GUILD_AUTOMOD_REGEX_ERROR_INVALID_REGEX_LENGTH.format({
          regex: e,
          max: s.aj,
          min: s.uE
        }))
