t.d(n, {
  Z: function() {
return F;
  }
}), t(47120), t(653041), t(724458);
var i = t(735250),
  a = t(470079),
  l = t(392711),
  r = t.n(l),
  o = t(892814),
  s = t(79411),
  c = t(442837),
  u = t(481060),
  d = t(911969),
  m = t(115130),
  p = t(566620),
  _ = t(127255),
  C = t(880308),
  f = t(427996),
  h = t(367907),
  A = t(399654),
  E = t(844439),
  N = t(254711),
  v = t(213459),
  x = t(10718),
  I = t(895924),
  g = t(148958),
  P = t(424602),
  L = t(541099),
  R = t(827498),
  S = t(87005),
  T = t(695676),
  b = t(98880),
  M = t(804307),
  j = t(888617),
  y = t(561160),
  O = t(41558),
  Z = t(105862),
  H = t(28147),
  U = t(981631),
  B = t(689079),
  k = t(689938),
  D = t(678313);
let w = [],
  V = [, , , , ].fill(0).map((e, n) => n),
  W = [{
  cards: [, , , , ].fill(0).map((e, n) => n),
  look: b.U4.MEDIUM_BANNER
},
{
  cards: [, , , , ].fill(0).map((e, n) => n),
  look: b.U4.ROW
},
{
  cards: [, , , , ].fill(0).map((e, n) => n),
  look: b.U4.ROW
}
  ];

function F(e) {
  let {
channel: n,
entrypoint: t,
searchQuery: l,
setSearchQuery: r,
setScroller: o,
isScrollCloseToBottom: s
  } = e, d = (0, c.e7)([m.Z], () => m.Z.getIsEnabled(), []), _ = t === R._b.TEXT, C = t === R._b.TEXT && !n.isPrivate(), f = t === R._b.TEXT, h = t === R._b.VOICE, A = t === R._b.TEXT, [E, N] = K(_), [v, x] = K(C), [I, g] = K(f), [P, L] = K(h), S = E && v && I && P, T = (f || C) && !S, b = h && d;
  a.useEffect(() => {
(0, p.w1)({
  guildId: n.getGuildId(),
  force: !0
});
  }, [n]), a.useEffect(() => {
t === R._b.VOICE && p.ux();
  }, [t]);
  let M = l.length > 0;
  return (0, i.jsxs)('div', {
className: D.container,
children: [
  b ? (0, i.jsx)(Y, {}) : null,
  (0, i.jsx)(G, {
    searchQuery: l,
    setSearchQuery: r,
    placeholder: A ? k.Z.Messages.APP_LAUNCHER_SEARCH_PLACEHOLDER : k.Z.Messages.APP_LAUNCHER_SEARCH_ACTIVITIES_PLACEHOLDER
  }),
  (0, i.jsx)(u.Scroller, {
    ref: o,
    className: D.scrollableContent,
    fade: !0,
    children: M ? (0, i.jsx)(H.Z, {
      channel: n,
      query: l,
      entrypoint: t,
      enableGlobalSearch: t === R._b.TEXT,
      isScrollCloseToBottom: s
    }) : (0, i.jsxs)('div', {
      children: [
        _ && (0, i.jsx)(z, {
          channel: n,
          entrypoint: t,
          onEmptyState: N
        }),
        C && (0, i.jsx)(q, {
          channel: n,
          onEmptyState: x
        }),
        f && (0, i.jsx)(Q, {
          channel: n,
          onEmptyState: g
        }),
        h && (0, i.jsx)(X, {
          channel: n,
          onEmptyState: L
        }),
        S && (0, i.jsx)(y.A, {
          type: R.LG.HOME_EMPTY,
          textContent: t === R._b.TEXT ? k.Z.Messages.APP_LAUNCHER_HOME_EMPTY_STATE_SEARCH_SUGGEST_BODY : k.Z.Messages.APP_LAUNCHER_HOME_EMPTY_STATE_ACTIVITIES_BODY
        }),
        T && (0, i.jsx)(Z.Z, {})
      ]
    })
  })
]
  });
}

function Y() {
  return (0, i.jsxs)('div', {
className: D.developerShelfControlsContainer,
children: [
  (0, i.jsx)(u.Text, {
    className: D.developerShelfControlsLabel,
    variant: 'text-sm/normal',
    children: k.Z.Messages.EMBEDDED_ACTIVITIES_DEVELOPER_SHELF_SUBTITLE
  }),
  (0, i.jsx)(f.W, {
    hideSearch: !0
  })
]
  });
}

function G(e) {
  let {
searchQuery: n,
setSearchQuery: t,
placeholder: l
  } = e, o = a.useRef(null), [s, c] = a.useState(!1), d = a.useMemo(() => r().debounce(e => {
(0, h.yw)(U.rMx.APP_LAUNCHER_SEARCH_QUERY_TYPED, {
  query: e,
  source: L.Z.entrypoint(),
  location: I.Vh.APP_LAUNCHER_HOME
});
  }, 400, {
leading: !1,
trailing: !0
  }), []), m = a.useCallback(() => t(''), [t]), p = a.useCallback(() => {
c(!0), (0, h.yw)(U.rMx.APP_LAUNCHER_SEARCH_FOCUSED, {
  source: L.Z.entrypoint(),
  location: I.Vh.APP_LAUNCHER_HOME
});
  }, []), _ = a.useCallback(() => {
c(!1);
  }, []), C = a.useCallback(e => {
!s && p(), t(e), d(e);
  }, [
s,
t,
p,
d
  ]);
  return a.useEffect(() => {
let e = o.current;
if (null == e)
  return;
let n = () => {
  !s && p();
};
return e.addEventListener('click', n), () => {
  e.removeEventListener('click', n);
};
  }, [
s,
p
  ]), (0, i.jsx)('div', {
className: D.searchBarContainer,
children: (0, i.jsx)(u.SearchBar, {
  ref: e => {
    var n;
    o.current = null !== (n = null == e ? void 0 : e.containerRef.current) && void 0 !== n ? n : null;
  },
  placeholder: l,
  query: n,
  onChange: C,
  onClear: m,
  size: u.SearchBar.Sizes.MEDIUM,
  onFocus: _,
  autoFocus: !0
})
  });
}

function z(e) {
  let {
channel: n,
entrypoint: t,
onEmptyState: l
  } = e, {
frecentApps: r,
loading: o
  } = (0, S.f)(n), s = a.useMemo(() => {
let e = [];
for (let n of r)
  null != n.application && e.push({
    application: n.application,
    isPartner: !1
  });
return e;
  }, [r]), c = k.Z.Messages.APP_LAUNCHER_HOME_RECENT_APPS_HEADER, {
items: u,
handleViewMore: d
  } = $(c, t === R._b.VOICE ? b.U4.LARGE_BANNER : b.U4.ROW, s, 8, R.L3.RECENT_APPS);
  a.useEffect(() => {
if (!0 !== o.current)
  0 !== u.length && (0, h.yw)(U.rMx.APP_LAUNCHER_FRECENTS_SEEN, {
    num: u.length,
    section_name: R.L3.RECENT_APPS,
    location: R.G0.HOME,
    source: t
  });
  }, [
u.length,
t,
o
  ]);
  let m = o.current,
p = !m && 0 === u.length;
  return (a.useEffect(() => {
l(p);
  }, [
p,
l
  ]), m || p) ? null : (0, i.jsxs)('div', {
children: [
  (0, i.jsx)(O.Z, {
    title: c,
    buttonType: O.Z.buttonTypes.VIEW_MORE,
    onClickViewButton: d
  }),
  (0, i.jsx)('div', {
    className: D.sectionContentContainer,
    children: (0, i.jsx)('div', {
      className: D.frecentList,
      children: u.map((e, n) => {
        let {
          application: t,
          isPartner: a
        } = e;
        return (0, i.jsx)(b.kA, {
          application: t,
          look: b.U4.ICON,
          isPartner: a,
          location: R.G0.HOME,
          sectionName: R.L3.RECENT_APPS,
          resultsPosition: n
        }, t.id);
      })
    })
  })
]
  });
}

function X(e) {
  let {
channel: n,
onEmptyState: t
  } = e;
  (0, C.g)();
  let l = (0, _.Z)({
  guildId: n.getGuildId()
}),
r = b.U4.LARGE_BANNER,
{
  trackSectionImpressionRef: o
} = (0, j.Z)({
  sectionName: R.L3.ACTIVITIES,
  numItems: l.length,
  numVisibleItems: l.length
}),
{
  clickOnHomeActivityOpensAppDetail: s
} = P.m1.getCurrentConfig({
  location: 'ActivitiesShelfSection'
}, {
  autoTrackExposure: !1
}),
c = 0 === l.length;
  return (a.useEffect(() => {
t(c);
  }, [
t,
c
  ]), c) ? null : (0, i.jsxs)('div', {
children: [
  (0, i.jsx)('div', {
    ref: e => o.current = e,
    children: (0, i.jsx)(O.Z, {
      title: k.Z.Messages.EMBEDDED_ACTIVITIES_SHELF_TITLE
    })
  }),
  (0, i.jsx)('div', {
    className: D.sectionTwoColumnContentContainer,
    children: l.map((e, t) => {
      let {
        application: a
      } = e;
      return (0, i.jsx)(b.qR, {
        channel: n,
        application: a,
        look: r,
        location: I.Vh.APP_LAUNCHER_HOME,
        sectionName: R.L3.ACTIVITIES,
        resultsPosition: t,
        isOneClickCTA: !s
      }, a.id);
    })
  })
]
  });
}

function q(e) {
  var n;
  let {
channel: t,
onEmptyState: l
  } = e, o = (0, v.LD)(t.guild_id, !0), {
commandsByActiveSection: s,
loading: c
  } = x.wi(t, {
commandTypes: [d.yU.CHAT]
  }, {
placeholderCount: 0,
limit: B.tn,
includeFrecency: !0
  }), u = a.useMemo(() => s.reduce((e, n) => {
let {
  section: t,
  data: i
} = n;
return i.length > 0 && e.add(t.id), e;
  }, new Set()), [s]), m = a.useMemo(() => {
var e, n;
return Object.values(null !== (n = null === (e = o.result) || void 0 === e ? void 0 : e.sections) && void 0 !== n ? n : {}).map(e => {
  let {
    descriptor: n
  } = e;
  return n;
}).filter(e => !(e.id in N.Tm) && u.has(e.id));
  }, [
null === (n = o.result) || void 0 === n ? void 0 : n.sections,
u
  ]), p = (0, g.h)(m), _ = R.L3.APPS_IN_THIS_SERVER, C = a.useMemo(() => r().compact(p.map(e => {
let {
  application: n
} = e;
return n;
  })).map(e => ({
application: e
  })), [p]), {
items: f,
handleViewMore: h
  } = $(k.Z.Messages.APP_LAUNCHER_HOME_APPS_IN_SERVER_HEADER, b.U4.ROW, C, 4, _), {
trackSectionImpressionRef: A
  } = (0, j.Z)({
sectionName: _,
numItems: C.length,
numVisibleItems: f.length
  }), E = o.fetchState.fetching || c.current, P = !E && 0 === f.length;
  return (a.useEffect(() => {
l(P);
  }, [
P,
l
  ]), P) ? null : (0, i.jsxs)('div', {
children: [
  (0, i.jsx)('div', {
    ref: e => A.current = e,
    children: (0, i.jsx)(O.Z, {
      title: k.Z.Messages.APP_LAUNCHER_HOME_APPS_IN_SERVER_HEADER,
      buttonType: O.Z.buttonTypes.VIEW_MORE,
      onClickViewButton: h
    })
  }),
  (0, i.jsx)('div', {
    className: D.sectionRowsContentContainer,
    children: E ? V.map(e => (0, i.jsx)(M.Z, {
      look: b.U4.ROW
    }, e)) : f.map((e, n) => {
      let {
        application: t,
        isPartner: a
      } = e;
      return null != t ? (0, i.jsx)(b.kA, {
        application: t,
        isPartner: a,
        look: b.U4.ROW,
        sectionName: _,
        resultsPosition: n,
        location: I.Vh.APP_LAUNCHER_HOME
      }, t.id) : null;
    })
  })
]
  });
}

function Q(e) {
  let {
channel: n,
onEmptyState: t
  } = e, {
fetchState: l,
recommendationsSections: r,
isInstallOnDemand: s
  } = function(e) {
let {
  channel: n,
  location: t
} = e, i = n.id;
a.useEffect(() => {
  (0, A.a)({
    channelId: i,
    location: t,
    withCommands: !1
  });
}, [
  i,
  t
]);
let {
  sectionDescriptors: l
} = x.wi(n, {
  commandTypes: [d.yU.CHAT]
}, {
  placeholderCount: 0,
  limit: B.tn,
  includeFrecency: !0
}), r = a.useCallback(e => !(null != l.find(n => n.id === e.id)), [l]), {
  fetchState: o,
  recommendationsSections: s
} = (0, c.cj)([E.ZP], () => ({
  fetchState: E.ZP.getFetchState({
    channelId: i,
    location: t,
    withCommands: !1
  }),
  recommendationsSections: E.ZP.getRecommendations({
    channelId: i,
    location: t,
    withCommands: !1
  })
}));
return {
  fetchState: o,
  recommendationsSections: s,
  isInstallOnDemand: r
};
  }({
channel: n,
location: o.I.APP_LAUNCHER_TEXT
  }), u = function(e) {
let {
  channel: n,
  recommendationsSections: t
} = e;
(0, C.g)();
let i = (0, _.Z)({
  guildId: n.getGuildId()
});
return a.useMemo(() => {
  if (!t.some(e => e.appends_remaining_activities))
    return w;
  let e = new Set();
  return t.forEach(n => {
    n.items.forEach(n => {
      null != n.application && e.add(n.application.id);
    });
  }), i.filter(n => !e.has(n.application.id));
}, [
  t,
  i
]);
  }({
channel: n,
recommendationsSections: r
  }), m = l === E.M.FETCHING, p = !m && 0 === r.length;
  return (a.useEffect(() => {
t(p);
  }, [
p,
t
  ]), p) ? null : (0, i.jsx)(i.Fragment, {
children: m ? W.map((e, n) => {
  let {
    cards: t,
    look: a
  } = e;
  return (0, i.jsxs)('div', {
    children: [
      (0, i.jsx)(O.Z.Loading, {}),
      (0, i.jsx)('div', {
        className: a === b.U4.ROW ? D.sectionRowsContentContainer : D.sectionTwoColumnContentContainer,
        children: t.map(e => (0, i.jsx)(M.Z, {
          look: a
        }, e))
      })
    ]
  }, n);
}) : r.map(e => (0, i.jsx)(J, {
  recommendationsSection: e,
  remainingActivities: u,
  isInstallOnDemand: s
}, e.key))
  });
}

function J(e) {
  let n, t, {
  recommendationsSection: l,
  remainingActivities: r,
  isInstallOnDemand: o
} = e,
c = l.section_title;
  switch (l.type) {
case s.i.BANNER_CARDS:
  n = b.U4.LARGE_BANNER;
  break;
case s.i.SMALL_BANNER_CARDS:
  n = b.U4.MEDIUM_BANNER;
  break;
default:
  n = b.U4.ROW;
  }
  let u = a.useMemo(() => {
  let e = l.items.map(e => {
    let {
      application: n,
      is_partner: t
    } = e;
    return {
      application: n,
      isPartner: t,
      installOnDemand: o(n)
    };
  });
  return l.appends_remaining_activities && e.push(...r.map(e => {
    let {
      application: n
    } = e;
    return {
      application: n,
      isPartner: !1,
      installOnDemand: !0
    };
  })), e;
}, [
  l.items,
  l.appends_remaining_activities,
  o,
  r
]),
d = l.key;
  switch (l.type) {
case s.i.BANNER_CARDS:
case s.i.SMALL_BANNER_CARDS:
  t = 6;
  break;
case s.i.EXPANDABLE_LIST:
default:
  t = 4;
  }
  let {
items: m,
handleViewMore: p
  } = $(c, n, u, t, d), {
trackSectionImpressionRef: _
  } = (0, j.Z)({
sectionName: d,
numItems: u.length,
numVisibleItems: m.length
  }), C = l.type !== s.i.SMALL_BANNER_CARDS;
  return (0, i.jsxs)('div', {
children: [
  (0, i.jsx)('div', {
    ref: e => _.current = e,
    children: (0, i.jsx)(O.Z, {
      title: l.section_title,
      buttonType: O.Z.buttonTypes.VIEW_MORE,
      onClickViewButton: p
    })
  }),
  (0, i.jsx)('div', {
    className: n === b.U4.ROW ? D.sectionRowsContentContainer : D.sectionTwoColumnContentContainer,
    children: m.map((e, t) => {
      let {
        application: a,
        isPartner: l,
        installOnDemand: r
      } = e;
      return (0, i.jsx)(b.kA, {
        application: a,
        look: n,
        isPartner: l,
        sectionName: d,
        resultsPosition: t,
        location: I.Vh.APP_LAUNCHER_HOME,
        installOnDemand: r,
        enableVideoBanner: C
      }, a.id);
    })
  })
]
  });
}

function K(e) {
  let [n, t] = a.useState(!e);
  return [
n,
a.useCallback(function() {
  let e = !(arguments.length > 0) || void 0 === arguments[0] || arguments[0];
  t(e);
}, [])
  ];
}

function $(e, n, t, i, l) {
  let {
pushHistory: r
  } = (0, T.hH)();
  return a.useMemo(() => t.length <= i ? {
items: t,
handleViewMore: void 0
  } : {
items: t.slice(0, i),
handleViewMore: () => {
  (0, h.yw)(U.rMx.APP_LAUNCHER_SECTION_VIEW_MORE, {
    section_name: l,
    source: L.Z.entrypoint(),
    num: t.length
  }), r({
    type: T.gc.LIST,
    title: e,
    look: n,
    items: t,
    sectionName: l
  });
}
  }, [
t,
i,
l,
r,
e,
n
  ]);
}