t.d(n, {
    Z: function () {
        return F;
    }
}),
    t(47120),
    t(653041);
var i = t(200651),
    a = t(192379),
    l = t(392711),
    o = t.n(l),
    r = t(892814),
    s = t(79411),
    c = t(442837),
    d = t(481060),
    u = t(911969),
    m = t(115130),
    p = t(566620),
    _ = t(127255),
    C = t(880308),
    h = t(427996),
    f = t(367907),
    A = t(399654),
    E = t(844439),
    N = t(10718),
    v = t(895924),
    x = t(835473),
    I = t(541099),
    P = t(827498),
    g = t(87005),
    T = t(219066),
    L = t(106066),
    S = t(783097),
    b = t(695676),
    R = t(98880),
    M = t(804307),
    y = t(888617),
    j = t(561160),
    O = t(41558),
    Z = t(105862),
    H = t(28147),
    U = t(981631),
    D = t(689079),
    k = t(689938),
    B = t(597976);
let w = [],
    V = [, , , ,].fill(0).map((e, n) => n),
    W = [
        {
            cards: [, , , ,].fill(0).map((e, n) => n),
            look: R.U4.MEDIUM_BANNER
        },
        {
            cards: [, , , ,].fill(0).map((e, n) => n),
            look: R.U4.ROW
        },
        {
            cards: [, , , ,].fill(0).map((e, n) => n),
            look: R.U4.ROW
        }
    ];
function F(e) {
    let { channel: n, entrypoint: t, searchQuery: l, setSearchQuery: o, setScroller: r, isScrollCloseToBottom: s } = e,
        u = (0, c.e7)([m.Z], () => m.Z.getIsEnabled(), []),
        _ = t === P._b.TEXT && !n.isPrivate(),
        C = (0, S.Yn)(t),
        h = !(0, S.Yn)(t),
        f = t === P._b.TEXT,
        [A, E] = K(!0),
        [N, v] = K(_),
        [x, I] = K(C),
        [g, T] = K(h),
        L = A && N && x && g,
        b = (C || _) && !L,
        R = h && u;
    a.useEffect(() => {
        (0, p.w1)({
            guildId: n.guild_id,
            force: !0
        });
    }, [n.guild_id]),
        a.useEffect(() => {
            t === P._b.VOICE && p.ux();
        }, [t]);
    let M = l.length > 0;
    return (0, i.jsxs)('div', {
        className: B.container,
        children: [
            R ? (0, i.jsx)(Y, {}) : null,
            (0, i.jsx)(G, {
                searchQuery: l,
                setSearchQuery: o,
                placeholder: f ? k.Z.Messages.APP_LAUNCHER_SEARCH_PLACEHOLDER : k.Z.Messages.APP_LAUNCHER_SEARCH_ACTIVITIES_PLACEHOLDER
            }),
            (0, i.jsx)(d.Scroller, {
                ref: r,
                className: B.scrollableContent,
                fade: !0,
                children: M
                    ? (0, i.jsx)(H.Z, {
                          channel: n,
                          query: l,
                          entrypoint: t,
                          isScrollCloseToBottom: s
                      })
                    : (0, i.jsxs)('div', {
                          children: [
                              (0, i.jsx)(z, {
                                  channel: n,
                                  entrypoint: t,
                                  onEmptyState: E
                              }),
                              _ &&
                                  (0, i.jsx)(J, {
                                      channel: n,
                                      onEmptyState: v
                                  }),
                              C &&
                                  (0, i.jsx)(Q, {
                                      channel: n,
                                      entrypoint: t,
                                      onEmptyState: I
                                  }),
                              h &&
                                  (0, i.jsx)(q, {
                                      channel: n,
                                      onEmptyState: T
                                  }),
                              L &&
                                  (0, i.jsx)(j.A, {
                                      type: P.LG.HOME_EMPTY,
                                      textContent: t === P._b.TEXT ? k.Z.Messages.APP_LAUNCHER_HOME_EMPTY_STATE_SEARCH_SUGGEST_BODY : k.Z.Messages.APP_LAUNCHER_HOME_EMPTY_STATE_ACTIVITIES_BODY
                                  }),
                              b && (0, i.jsx)(Z.Z, {})
                          ]
                      })
            })
        ]
    });
}
function Y() {
    return (0, i.jsxs)('div', {
        className: B.developerShelfControlsContainer,
        children: [
            (0, i.jsx)(d.Text, {
                className: B.developerShelfControlsLabel,
                variant: 'text-sm/normal',
                children: k.Z.Messages.EMBEDDED_ACTIVITIES_DEVELOPER_SHELF_SUBTITLE
            }),
            (0, i.jsx)(h.W, { hideSearch: !0 })
        ]
    });
}
function G(e) {
    let { searchQuery: n, setSearchQuery: t, placeholder: l } = e,
        r = a.useRef(null),
        [s, c] = a.useState(!1),
        u = a.useMemo(
            () =>
                o().debounce(
                    (e) => {
                        (0, f.yw)(U.rMx.APP_LAUNCHER_SEARCH_QUERY_TYPED, {
                            query: e,
                            source: I.Z.entrypoint(),
                            location: v.Vh.APP_LAUNCHER_HOME
                        });
                    },
                    400,
                    {
                        leading: !1,
                        trailing: !0
                    }
                ),
            []
        ),
        m = a.useCallback(() => t(''), [t]),
        p = a.useCallback(() => {
            c(!0),
                (0, f.yw)(U.rMx.APP_LAUNCHER_SEARCH_FOCUSED, {
                    source: I.Z.entrypoint(),
                    location: v.Vh.APP_LAUNCHER_HOME
                });
        }, []),
        _ = a.useCallback(() => {
            c(!1);
        }, []),
        C = a.useCallback(
            (e) => {
                !s && p(), t(e), u(e);
            },
            [s, t, p, u]
        );
    return (
        a.useEffect(() => {
            let e = r.current;
            if (null == e) return;
            let n = () => {
                !s && p();
            };
            return (
                e.addEventListener('click', n),
                () => {
                    e.removeEventListener('click', n);
                }
            );
        }, [s, p]),
        (0, i.jsx)('div', {
            className: B.searchBarContainer,
            children: (0, i.jsx)(d.SearchBar, {
                ref: (e) => {
                    var n;
                    r.current = null !== (n = null == e ? void 0 : e.containerRef.current) && void 0 !== n ? n : null;
                },
                placeholder: l,
                query: n,
                onChange: C,
                onClear: m,
                size: d.SearchBar.Sizes.MEDIUM,
                onFocus: _,
                autoFocus: !0
            })
        })
    );
}
function z(e) {
    let { channel: n, entrypoint: t, onEmptyState: l } = e,
        o = t === P._b.VOICE,
        { frecentApps: r, loading: s } = (0, g.f)({
            channel: n,
            onlyActivityApps: o,
            allowCommandFetch: !0,
            includeAuthorizedAppsAndFetch: !0
        }),
        c = a.useMemo(() => {
            let e = [];
            for (let n of r) null != n.application && e.push({ application: n.application });
            return e;
        }, [r]),
        d = k.Z.Messages.APP_LAUNCHER_HOME_RECENT_APPS_HEADER,
        u = d;
    o && (u = k.Z.Messages.APP_LAUNCHER_HOME_RECENT_APPS_HEADER_ACTIVITIES);
    let { items: m, handleViewMore: p } = $({
        title: u,
        look: t === P._b.VOICE ? R.U4.LARGE_BANNER : R.U4.ROW,
        items: c,
        limit: 8,
        sectionName: P.L3.RECENT_APPS
    });
    a.useEffect(() => {
        if (!0 !== s.current)
            0 !== m.length &&
                (0, f.yw)(U.rMx.APP_LAUNCHER_FRECENTS_SEEN, {
                    num: m.length,
                    section_name: P.L3.RECENT_APPS,
                    location: P.G0.HOME,
                    source: t
                });
    }, [m.length, t, s]);
    let _ = s.current,
        C = !_ && 0 === m.length;
    return (a.useEffect(() => {
        l(C);
    }, [C, l]),
    !(function (e) {
        let { apps: n, onlyActivityApps: t } = e,
            i = a.useMemo(
                () =>
                    t
                        ? n.map((e) => {
                              let { application: n } = e;
                              return n.id;
                          })
                        : [],
                [n, t]
            );
        (0, x.Z)(i);
    })({
        apps: m,
        onlyActivityApps: o
    }),
    _ || C)
        ? null
        : (0, i.jsxs)('div', {
              children: [
                  (0, i.jsx)(O.Z, {
                      title: d,
                      buttonType: O.Z.buttonTypes.VIEW_MORE,
                      onClickViewButton: p
                  }),
                  (0, i.jsx)('div', {
                      className: B.sectionContentContainer,
                      children: (0, i.jsx)('div', {
                          className: B.frecentList,
                          children: m.map((e, t) => {
                              let { application: a } = e;
                              return o
                                  ? (0, i.jsx)(
                                        R.qR,
                                        {
                                            channel: n,
                                            application: a,
                                            look: R.U4.ICON,
                                            location: P.G0.HOME,
                                            sectionName: P.L3.RECENT_APPS,
                                            resultsPosition: t,
                                            isOneClickCTA: !0,
                                            fetchesApplication: !1
                                        },
                                        a.id
                                    )
                                  : (0, i.jsx)(
                                        R.kA,
                                        {
                                            application: a,
                                            look: R.U4.ICON,
                                            location: P.G0.HOME,
                                            sectionName: P.L3.RECENT_APPS,
                                            resultsPosition: t
                                        },
                                        a.id
                                    );
                          })
                      })
                  })
              ]
          });
}
function q(e) {
    let { channel: n, onEmptyState: t } = e;
    (0, C.g)();
    let l = (0, _.Z)({ guildId: n.getGuildId() }),
        o = R.U4.LARGE_BANNER,
        { trackSectionImpressionRef: r } = (0, y.Z)({
            sectionName: P.L3.ACTIVITIES,
            numItems: l.length,
            numVisibleItems: l.length
        }),
        s = (0, L.s)('ActivitiesShelfSection'),
        c = 0 === l.length;
    return (a.useEffect(() => {
        t(c);
    }, [t, c]),
    c)
        ? null
        : (0, i.jsxs)('div', {
              children: [
                  (0, i.jsx)('div', {
                      ref: (e) => (r.current = e),
                      children: (0, i.jsx)(O.Z, { title: k.Z.Messages.EMBEDDED_ACTIVITIES_SHELF_TITLE })
                  }),
                  (0, i.jsx)('div', {
                      className: B.sectionTwoColumnContentContainer,
                      children: l.map((e, t) => {
                          let { application: a } = e;
                          return (0, i.jsx)(
                              R.qR,
                              {
                                  channel: n,
                                  application: a,
                                  look: o,
                                  location: v.Vh.APP_LAUNCHER_HOME,
                                  sectionName: P.L3.ACTIVITIES,
                                  resultsPosition: t,
                                  sectionOverallPosition: 0,
                                  isOneClickCTA: !s,
                                  fetchesApplication: !1
                              },
                              a.id
                          );
                      })
                  })
              ]
          });
}
function J(e) {
    let { channel: n, onEmptyState: t } = e,
        l = P.L3.APPS_IN_THIS_SERVER,
        { appsInThisServer: o, isLoading: r } = (0, T.Z)({ channel: n }),
        { items: s, handleViewMore: c } = $({
            title: k.Z.Messages.APP_LAUNCHER_HOME_APPS_IN_SERVER_HEADER,
            look: R.U4.ROW,
            items: o,
            limit: 4,
            sectionName: l
        }),
        { trackSectionImpressionRef: d } = (0, y.Z)({
            sectionName: l,
            numItems: o.length,
            numVisibleItems: s.length
        }),
        u = !r && 0 === s.length;
    return (a.useEffect(() => {
        t(u);
    }, [u, t]),
    u)
        ? null
        : (0, i.jsxs)('div', {
              children: [
                  (0, i.jsx)('div', {
                      ref: (e) => (d.current = e),
                      children: (0, i.jsx)(O.Z, {
                          title: k.Z.Messages.APP_LAUNCHER_HOME_APPS_IN_SERVER_HEADER,
                          buttonType: O.Z.buttonTypes.VIEW_MORE,
                          onClickViewButton: c
                      })
                  }),
                  (0, i.jsx)('div', {
                      className: B.sectionRowsContentContainer,
                      children: r
                          ? V.map((e) => (0, i.jsx)(M.Z, { look: R.U4.ROW }, e))
                          : s.map((e, n) => {
                                let { application: t } = e;
                                return null != t
                                    ? (0, i.jsx)(
                                          R.kA,
                                          {
                                              application: t,
                                              look: R.U4.ROW,
                                              sectionName: l,
                                              resultsPosition: n,
                                              location: v.Vh.APP_LAUNCHER_HOME
                                          },
                                          t.id
                                      )
                                    : null;
                            })
                  })
              ]
          });
}
function Q(e) {
    let { channel: n, entrypoint: t, onEmptyState: l } = e,
        {
            fetchState: o,
            recommendationsSections: s,
            isInstallOnDemand: d
        } = (function (e) {
            let { channel: n, location: t } = e,
                i = n.id;
            a.useEffect(() => {
                (0, A.a)({
                    channelId: i,
                    location: t,
                    withCommands: !1
                });
            }, [i, t]);
            let { sectionDescriptors: l } = N.wi({
                    channel: n,
                    filters: { commandTypes: [u.yU.CHAT] },
                    options: {
                        placeholderCount: 0,
                        limit: D.tn,
                        includeFrecency: !0
                    },
                    allowFetch: !0
                }),
                o = a.useCallback((e) => !(null != l.find((n) => n.id === e.id)), [l]),
                { fetchState: s, recommendationsSections: d } = (0, c.cj)([E.ZP], () => ({
                    fetchState: E.ZP.getFetchState({
                        channelId: i,
                        location: t,
                        withCommands: !1
                    }),
                    recommendationsSections: E.ZP.getRecommendations({
                        channelId: i,
                        location: t,
                        withCommands: !1
                    })
                })),
                m = t === r.I.APP_LAUNCHER_VOICE;
            return {
                fetchState: s,
                recommendationsSections: a.useMemo(() => (m ? (0, S.pF)(d) : d), [d, m]),
                isInstallOnDemand: o
            };
        })({
            channel: n,
            location: t === P._b.TEXT ? r.I.APP_LAUNCHER_TEXT : r.I.APP_LAUNCHER_VOICE
        }),
        m = (function (e) {
            let { channel: n, recommendationsSections: t } = e;
            (0, C.g)();
            let i = (0, _.Z)({ guildId: n.getGuildId() });
            return a.useMemo(() => {
                if (!t.some((e) => e.appends_remaining_activities)) return w;
                let e = new Set();
                return (
                    t.forEach((n) => {
                        n.items.forEach((n) => {
                            null != n.application && e.add(n.application.id);
                        });
                    }),
                    i.filter((n) => !e.has(n.application.id))
                );
            }, [t, i]);
        })({
            channel: n,
            recommendationsSections: s
        }),
        p = o === E.M.FETCHING,
        h = !p && 0 === s.length;
    return (a.useEffect(() => {
        l(h);
    }, [h, l]),
    h)
        ? null
        : (0, i.jsx)(i.Fragment, {
              children: p
                  ? W.map((e, n) => {
                        let { cards: t, look: a } = e;
                        return (0, i.jsxs)(
                            'div',
                            {
                                children: [
                                    (0, i.jsx)(O.Z.Loading, {}),
                                    (0, i.jsx)('div', {
                                        className: a === R.U4.ROW ? B.sectionRowsContentContainer : B.sectionTwoColumnContentContainer,
                                        children: t.map((e) => (0, i.jsx)(M.Z, { look: a }, e))
                                    })
                                ]
                            },
                            n
                        );
                    })
                  : s.map((e, n) =>
                        (0, i.jsx)(
                            X,
                            {
                                recommendationsSection: e,
                                remainingActivities: m,
                                isInstallOnDemand: d,
                                position: n
                            },
                            e.key
                        )
                    )
          });
}
function X(e) {
    let n,
        t,
        { recommendationsSection: l, remainingActivities: o, isInstallOnDemand: r, position: c } = e,
        d = l.section_title;
    switch (l.type) {
        case s.i.BANNER_CARDS:
            n = R.U4.LARGE_BANNER;
            break;
        case s.i.SMALL_BANNER_CARDS:
            n = R.U4.MEDIUM_BANNER;
            break;
        default:
            n = R.U4.ROW;
    }
    let u = a.useMemo(() => {
            let e = l.items.map((e) => {
                let { application: n } = e;
                return {
                    application: n,
                    installOnDemand: r(n)
                };
            });
            return (
                l.appends_remaining_activities &&
                    e.push(
                        ...o.map((e) => {
                            let { application: n } = e;
                            return {
                                application: n,
                                installOnDemand: !0
                            };
                        })
                    ),
                e
            );
        }, [l.items, l.appends_remaining_activities, r, o]),
        m = l.key;
    switch (l.type) {
        case s.i.BANNER_CARDS:
        case s.i.SMALL_BANNER_CARDS:
            t = 6;
            break;
        case s.i.EXPANDABLE_LIST:
        default:
            t = 4;
    }
    let { items: p, handleViewMore: _ } = $({
            title: d,
            look: n,
            items: u,
            limit: t,
            sectionName: m,
            sectionOverallPosition: c
        }),
        { trackSectionImpressionRef: C } = (0, y.Z)({
            sectionName: m,
            numItems: u.length,
            numVisibleItems: p.length
        }),
        h = l.type !== s.i.SMALL_BANNER_CARDS;
    return (0, i.jsxs)('div', {
        children: [
            (0, i.jsx)('div', {
                ref: (e) => (C.current = e),
                children: (0, i.jsx)(O.Z, {
                    title: l.section_title,
                    buttonType: O.Z.buttonTypes.VIEW_MORE,
                    onClickViewButton: _
                })
            }),
            (0, i.jsx)('div', {
                className: n === R.U4.ROW ? B.sectionRowsContentContainer : B.sectionTwoColumnContentContainer,
                children: p.map((e, t) => {
                    let { application: a, installOnDemand: l } = e;
                    return (0, i.jsx)(
                        R.kA,
                        {
                            application: a,
                            look: n,
                            sectionName: m,
                            resultsPosition: t,
                            location: v.Vh.APP_LAUNCHER_HOME,
                            installOnDemand: l,
                            enableVideoBanner: h,
                            sectionOverallPosition: c
                        },
                        a.id
                    );
                })
            })
        ]
    });
}
function K(e) {
    let [n, t] = a.useState(!e);
    return [
        n,
        a.useCallback(function () {
            let e = !(arguments.length > 0) || void 0 === arguments[0] || arguments[0];
            t(e);
        }, [])
    ];
}
function $(e) {
    let { title: n, look: t, items: i, limit: l, sectionName: o, sectionOverallPosition: r } = e,
        { pushHistory: s } = (0, b.hH)();
    return a.useMemo(
        () =>
            i.length <= l
                ? {
                      items: i,
                      handleViewMore: void 0
                  }
                : {
                      items: i.slice(0, l),
                      handleViewMore: () => {
                          (0, f.yw)(U.rMx.APP_LAUNCHER_SECTION_VIEW_MORE, {
                              section_name: o,
                              source: I.Z.entrypoint(),
                              num: i.length
                          }),
                              s({
                                  type: b.gc.LIST,
                                  title: n,
                                  look: t,
                                  items: i,
                                  sectionName: o,
                                  sectionOverallPosition: r
                              });
                      }
                  },
        [i, l, o, s, n, t, r]
    );
}
