(() => {
    var A = {
            ...
            75: (A, I, g) => {
                ...
                let C = g(376), Q = g(406), B = g(761), E = g(608), D = g(862), x = g(967);
                class i extends AudioWorkletNode {
                    constructor(A, I, g, i) {
                        let o = A.sampleRate, a = (0, E.getModelName)(o), h = {
                                ...I,
                                sampleRate: o,
                                modelName: a,
                        ...
                        try {
                            g && this.addEventListener('ready', g), i && this.addEventListener('dispose', i), this.debugLogs && x.Logger.info('AudioFilterNode:constructor', 'creating web worker (type: module)'), this.worker = new C.KWebWorker(D.WORKER_URL, { type: 'module' }), this.debugLogs && x.Logger.info('AudioFilterNode:constructor', 'adding event listener for worker'), this.worker.addEventListener('message', this._onWasmWorkerMessage.bind(this)), this.debugLogs && (x.Logger.info('AudioFilterNode:constructor', 'Connecting with other threads for logging'), x.Logger.collectFrom(this.worker, h), x.Logger.collectFrom(this.port, h)), this.debugLogs && x.Logger.info('AudioFilterNode:constructor', 'Send init WASM processor event'), this.worker.postMessage({
                    ...
                    _onWasmWorkerMessage(A) {
                        ...
                        case 'WASM_PROCESSOR_INITIALIZED':
                            this.debugLogs && x.Logger.info('AudioFilterNode:_onWasmWorkerMessage', 'WASM processor initialized'), this.isReady = !0, this.dispatchEvent(new MessageEvent('ready')), this.postMessage({
                    ...
                    checkReadiness(A) {
                        if (this.debugLogs && x.Logger.info('AudioFilterNode:checkReadiness', JSON.stringify({ isReady: this.isReady })), !A)
                    ...
                    enable() {
                        this.checkReadiness(this.isReady), this.debugLogs && x.Logger.info('AudioFilterNode:enable', 'send enable NC message'), this.enabled = !0, this.postMessage({
                            event: 'TOGGLE',
                            data: !0
                        });
                    }
                    disable() {
                        this.checkReadiness(this.isReady), this.debugLogs && x.Logger.info('AudioFilterNode:disable', 'send disable NC message'), this.enabled = !1, this.postMessage({
                            event: 'TOGGLE',
                            data: !1
                        });
                    }
                    toggle() {
                        this.checkReadiness(this.isReady), this.debugLogs && x.Logger.info('AudioFilterNode:toggle', 'send toggle NC message'), this.enabled = !this.enabled, this.postMessage({
                            event: 'TOGGLE',
                            data: void 0
                        });
                    }
                    dispose() {
                        try {
                            this.debugLogs && x.Logger.info('AudioFilterNode:dispose', 'disposing Krisp filter'), this.postMessage({
                                event: 'SUSPEND',
                                data: void 0
                            }), this.worker.removeEventListener('message', this._onWasmWorkerMessage.bind(this)), this.worker.terminate(), this.dispatchEvent(new MessageEvent('dispose')), clearInterval(this.secondsCounterInterval);
                        } catch (A) {
                    ...
                    startSecondsCounter(A) {
                        ...
                        this.secondsCounterInterval = setInterval(() => {
                            I++, this.parameters.get('seconds').setValueAtTime(I, 0), x.Logger.info('AudioFilterNode:interval', JSON.stringify({
                                seconds: I,
            ...
            24: (A, I, g) => {
                ...
                let C = g(967), Q = 'krisp-debug-widget';
                I.createDebugWidget = function () {
                    if (document.querySelector(Q))
                        return;
                    let A = document.createElement(Q);
                    document.body.appendChild(A);
                }, customElements.define(Q, class extends HTMLElement {
                    constructor() {
                        super();
                    ...
                    connectedCallback() {
                        let A = 0, I = 0, g = 0, Q = 0;
                        this.style.position = 'absolute', this.style.top = '10px', this.style.right = '10px', this.onmousedown = C => {
                            g = C.clientX, Q = C.clientY, document.onmouseup = () => {
            ...
            967: (A, I, g) => {
                ...
                I.Logger = new class {
                    ...
                    collectFrom(A, I) {
                        ...
                        A instanceof MessagePort && (g.recorder = new Q.DebugRecorder(I.sampleRate)), A instanceof Window ? this.logPort = g.channel.port2 : A.postMessage({
                            event: 'SET_LOGGING_PORT',
                            data: void 0
                        }, [g.channel.port2]), g.channel.port1.addEventListener('message', A => {
                        ...
                        }), this.instances.push(g), g.channel.port1.start();
                    }
  ...
  `;
                    ...
                    logAudio(A, I) {
                        if (this.debugLogs && this.logPort) {
                            ...
                            g.set(I, 0), this.logPort.postMessage({
                                event: A,
                                data: g
                            }, [g.buffer]);
                        }
...
`;
                        if (!this.logPort)
                            return console.error('KrispSDK -', 'Logger is not initialized', g);
            ...
            785: (A, I, g) => {
                ...
                class B {
                    constructor(A, I = 1) {
                        ...
                        for (let A = 0; A < I; A++)
                            this._channelData.push(new Float32Array(new SharedArrayBuffer(this._bufferLength * Float32Array.BYTES_PER_ELEMENT)));
                    }
                    static from(A) {
                        return Object.setPrototypeOf(A, B.prototype);
                    }
                    push(A, I) {
                        let g = Atomics.load(this._state, Q.READ_INDEX), C = Atomics.load(this._state, Q.WRITE_INDEX);
                        if (this._getAvailableWrite(g, C) < I)
                            return !1;
                        let B = C + I;
                        if (this._bufferLength < B) {
                            B -= this._bufferLength;
                            for (let I = 0; I < this._channelCount; I++) {
                                let g = this._channelData[I].subarray(C), Q = this._channelData[I].subarray(0, B);
                                g.set(A[I].subarray(0, g.length)), Q.set(A[I].subarray(g.length));
                            }
                        } else {
                            for (let g = 0; g < this._channelCount; g++)
                                this._channelData[g].subarray(C, B).set(A[g].subarray(0, I));
                    ...
                    pull(A, I) {
                        let g = Atomics.load(this._state, Q.READ_INDEX), C = Atomics.load(this._state, Q.WRITE_INDEX);
                        if (this._getAvailableRead(g, C) < I)
                            return !1;
                        let B = g + I;
                        if (this._bufferLength < B) {
                            B -= this._bufferLength;
                            for (let I = 0; I < this._channelCount; I++) {
                                let C = this._channelData[I].subarray(g), Q = this._channelData[I].subarray(0, B);
                                A[I].set(C), A[I].set(Q, C.length);
                            }
                        } else {
                            for (let I = 0; I < this._channelCount; ++I)
                                A[I].set(this._channelData[I].subarray(g, B));
                            B === this._bufferLength && (B = 0);
                        }
                        return Atomics.store(this._state, Q.READ_INDEX, B), !0;
                    }
                    printAvailableReadAndWrite() {
                        let A = Atomics.load(this._state, Q.READ_INDEX), I = Atomics.load(this._state, Q.WRITE_INDEX);
                        C.Logger.log(this, {
            ...
            333: (A, I) => {
                Object.defineProperty(I, '__esModule', { value: !0 }), I.sampleModelMap = void 0, I.sampleModelMap = {
                    8000: 'model8',
                    16000: 'model16',
                    24000: 'model32',
                    32000: 'model32',
            ...
            687: (A, I) => {
                Object.defineProperty(I, '__esModule', { value: !0 }), I.createWAVBlob = I.downloadBlob = I.blobToBase64 = void 0, I.blobToBase64 = async function (A) {
                    return new Promise((I, g) => {
                        let C = new FileReader();
                        C.onloadend = () => I(C.result), C.readAsDataURL(A);
                    });
                }, I.downloadBlob = function (A, I) {
                    let g = window.URL.createObjectURL(I), C = document.createElement('a');
                    C.href = g, C.download = A || 'file', C.click();
                }, I.createWAVBlob = function (A, I) {
                    if (!A.length)
                        return new Blob([], { type: 'audio/wav; codecs=wav' });
                    let g = A[0].length, C = new Float32Array(g * A.length);
                    return A.forEach((A, I) => {
                        C.set(A, I * g);
                    }), new Blob([function (A, I) {
                            let g = I.isFloat ? Float32Array : Uint16Array, C = function (A) {
                                    let I = A.numFrames, g = A.numChannels || 2, C = A.isFloat ? 4 : 2, Q = A.isFloat ? 3 : 1, B = A.sampleRate, E = g * C, D = I * E, x = new ArrayBuffer(44), i = new DataView(x), o = 0;
                                    function a(A) {
                                        for (let I = 0; I < A.length; I++)
                                            i.setUint8(o + I, A.charCodeAt(I));
                                        o += A.length;
                                    }
                                    function h(A) {
                                ...
                                }(Object.assign({}, I, { numFrames: A.byteLength / g.BYTES_PER_ELEMENT })), Q = new Uint8Array(C.length + A.byteLength);
            ...
            479: (A, I) => {
                Object.defineProperty(I, '__esModule', { value: !0 }), I.isMobile = I.isSafari = void 0, I.isSafari = function () {
            ...
            406: (A, I, g) => {
                ...
                let C = g(785), Q = g(761);
                I.STATE = { REQUEST_NOISE_CANCELLATION: 0 }, I.createSharedBuffers = function (A) {
            ...
            761: (A, I) => {
                Object.defineProperty(I, '__esModule', { value: !0 }), I.SUPPORTED_SAMPLE_RATES = I.SDK_FRAME_DURATION_10MS = I.BUFFER_OVERFLOW_MS = I.BUFFER_DROP_KEEP_MS = I.BUFFER_DROP_SIZE_MS = I.WORKLET_PROCESSOR_NAME = I.WEB_AUDIO_CHUNK_SIZE = void 0, I.WEB_AUDIO_CHUNK_SIZE = 128, I.WORKLET_PROCESSOR_NAME = 'processor', I.BUFFER_DROP_SIZE_MS = 500, I.BUFFER_DROP_KEEP_MS = 40, I.BUFFER_OVERFLOW_MS = 200, I.SDK_FRAME_DURATION_10MS = 10, I.SUPPORTED_SAMPLE_RATES = [
                    8000,
                    16000,
                    24000,
                    32000,
    ...
    (() => {
        ...
        let A = C(75), I = C(24), g = C(862), B = C(761), E = C(608), D = C(479), x = C(967);
        class i {
            constructor(A) {
                this.state = 'INITIAL';
                ...
                if (!A.params || !A?.params?.models || Object.keys(!A?.params?.models).length)
                    throw Error('INVALID_OPTIONS');
                if ((0, E.hasInvalidModelPath)(A?.params?.models))
                    throw I && x.Logger.error('Krisp SDK - Invalid model url. Accepted structure is schema://domain.ext/path/to/model.kw'), Error('INVALID_MODEL_URL');
                A?.params?.useSharedArrayBuffer && 'undefined' == typeof SharedArrayBuffer && (x.Logger.warn('Krisp SDK - SharedArrayByffer is not supported by browser. Make sure all security requirements are present. See https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer#security_requirements'), A.params.useSharedArrayBuffer = !1), this.options = {
                    params: {
            ...
            async init() {
                if ('INITIALIZED' === this.state)
                    throw this.debugLogs && x.Logger.error('KrispSDK:init', 'SDK already initialized'), Error('SDK_ALREADY_INITIALIZED');
            ...
            async createNoiseFilter(I, C, Q) {
                ...
                if (!I || !I.audioWorklet)
                    throw this.debugLogs && x.Logger.error('KrispSDK:createNoiseFilter', 'audioContext or audioContext.audioWorklet not found'), Error('INVALID_AUDIO_CONTEXT');
                if (0 > B.SUPPORTED_SAMPLE_RATES.indexOf(I.sampleRate))
                    throw Error('NOT_SUPPORTED_SAMPLE_RATE');
                try {
