var r,
    i = n(47120);
var a = n(789020);
var o = n(653041);
var s = n(442837),
    l = n(570140),
    u = n(630388),
    c = n(709054),
    d = n(271383),
    _ = n(430824),
    E = n(594174),
    f = n(981631),
    h = n(372897);
function p(e, t, n) {
    return (
        t in e
            ? Object.defineProperty(e, t, {
                  value: n,
                  enumerable: !0,
                  configurable: !0,
                  writable: !0
              })
            : (e[t] = n),
        e
    );
}
let m = {
        notClaimed: !1,
        notEmailVerified: !1,
        notPhoneVerified: !1,
        newAccount: !1,
        newMember: !1,
        canChat: !0
    },
    I = new Set(),
    T = {};
function g(e) {
    let t;
    S(e), I.add(e);
    let n = _.Z.getGuild(e),
        r = E.default.getCurrentUser();
    if (null == n || n.verificationLevel === f.sFg.NONE || null == r || n.isOwner(r) || r.isPhoneVerified()) return;
    let i = d.ZP.getMember(n.id, r.id);
    if (null != i) {
        var a;
        if ((0, u.yE)(null !== (a = i.flags) && void 0 !== a ? a : 0, h.q.BYPASSES_VERIFICATION)) return;
        let e = new Set();
        for (let t of i.roles) {
            let r = _.Z.getRole(n.id, t);
            null != r && !r.managed && e.add(t);
        }
        let t = new Date('2022-12-02 00:00:00'),
            r = null == i.joinedAt || new Date(i.joinedAt) < t;
        if (!(n.hasFeature(f.oNc.GUILD_ONBOARDING_EVER_ENABLED) && !r) && e.size > 0) return;
    }
    let o = +r.createdAt + 60000 * f.YeM.ACCOUNT_AGE - Date.now(),
        s = +n.joinedAt + 60000 * f.YeM.MEMBER_AGE - Date.now(),
        c = n.verificationLevel >= f.sFg.LOW && !r.isClaimed(),
        p = !1,
        m = !1,
        g = !1,
        A = !1;
    !r.isPhoneVerified() && !r.isStaff() && ((p = n.verificationLevel >= f.sFg.LOW && !r.verified), (m = n.verificationLevel >= f.sFg.VERY_HIGH), (g = n.verificationLevel >= f.sFg.MEDIUM && o > 0), (A = n.verificationLevel >= f.sFg.HIGH && s > 0));
    let v = [];
    A && v.push(s),
        g && v.push(o),
        v.length > 0 &&
            (t = setTimeout(
                () =>
                    l.Z.dispatch({
                        type: 'GUILD_VERIFICATION_CHECK',
                        guildId: e
                    }),
                Math.max(...v)
            )),
        (T[e] = {
            notClaimed: c,
            notEmailVerified: p,
            notPhoneVerified: m,
            newAccount: g,
            newMember: A,
            canChat: !(c || p || m || g || A),
            accountDeadline: new Date(Date.now() + o),
            memberDeadline: new Date(Date.now() + s),
            timeoutRef: t
        });
}
function S(e) {
    let t = T[e];
    null != t && clearTimeout(t.timeoutRef), delete T[e];
}
function A() {
    for (let e in (I.clear(), T)) S(e);
}
function v() {
    c.default.keys(T).forEach(S);
}
function N(e) {
    I.delete(e.guild.id), g(e.guild.id);
}
function O(e) {
    let { guild: t } = e;
    S(t.id);
}
function R(e) {
    var t;
    let { guildId: n, user: r } = e;
    if (r.id !== (null === (t = E.default.getCurrentUser()) || void 0 === t ? void 0 : t.id)) return !1;
    I.delete(n);
}
function C() {
    I.clear();
}
function y(e) {
    let { guildId: t } = e;
    g(t);
}
class b extends (r = s.ZP.Store) {
    initialize() {
        this.waitFor(_.Z, E.default);
    }
    getCheck(e) {
        var t;
        return !I.has(e) && g(e), null !== (t = T[e]) && void 0 !== t ? t : m;
    }
    canChatInGuild(e) {
        return this.getCheck(e).canChat;
    }
}
p(b, 'displayName', 'GuildVerificationStore'),
    (t.Z = new b(l.Z, {
        CONNECTION_OPEN: A,
        CONNECTION_CLOSED: v,
        CURRENT_USER_UPDATE: C,
        GUILD_CREATE: N,
        GUILD_UPDATE: N,
        GUILD_DELETE: O,
        GUILD_MEMBER_UPDATE: R,
        GUILD_VERIFICATION_CHECK: y
    }));
