n.d(t, {
    NZ: function () {
        return eZ;
    },
    r: function () {
        return eN;
    }
}),
    n(47120);
var i,
    l,
    r = n(200651),
    a = n(192379),
    s = n(120356),
    o = n.n(s),
    c = n(512722),
    d = n.n(c),
    u = n(752877),
    h = n(848246),
    p = n(442837),
    m = n(704215),
    f = n(481060),
    g = n(846027),
    C = n(287734),
    x = n(872810),
    _ = n(410575),
    v = n(40851),
    I = n(317381),
    E = n(596040),
    b = n(100527),
    N = n(906732),
    Z = n(358221),
    S = n(887012),
    T = n(659580),
    j = n(793865),
    A = n(575175),
    y = n(464792),
    P = n(324085),
    M = n(243778),
    R = n(258609),
    L = n(446226),
    k = n(569545),
    O = n(74299),
    D = n(803647),
    w = n(554747),
    B = n(95764),
    U = n(459502),
    H = n(386542),
    G = n(140465),
    F = n(746599),
    V = n(455961),
    z = n(611845),
    W = n(800966),
    K = n(618158),
    Y = n(390322),
    q = n(791592),
    X = n(136995),
    J = n(402113),
    Q = n(197016),
    $ = n(386000),
    ee = n(698877),
    et = n(25827),
    en = n(199902),
    ei = n(523746),
    el = n(131951),
    er = n(944486),
    ea = n(594174),
    es = n(626135),
    eo = n(700785),
    ec = n(923973),
    ed = n(829750),
    eu = n(189771),
    eh = n(294629),
    ep = n(304745),
    em = n(985370),
    ef = n(544384),
    eg = n(560688),
    eC = n(127608),
    ex = n(76021),
    e_ = n(173507),
    ev = n(981631),
    eI = n(354459),
    eE = n(921944),
    eb = n(812801);
function eN(e, t) {
    if (el.Z.isVideoEnabled() === e) return;
    let n = () => g.Z.setVideoEnabled(e);
    e ? (0, e_.Z)(n, t) : n();
}
function eZ(e) {
    let { channel: t, currentUser: n, exitFullScreen: i, canGoLive: l, hasPermission: s, disabled: o } = e,
        c = (0, v.bp)(),
        d = (0, p.Wu)([en.Z], () => en.Z.getAllActiveStreams()),
        u = d.find((e) => e.ownerId === n.id),
        { activated: g, hqStreamingState: C } = (0, H.k)(h.q.STREAM_HIGH_QUALITY),
        x = null != u && g,
        _ = a.useRef(null),
        I = t.getGuildId(),
        E = a.useCallback(() => {
            if ((null == i || i(), !l)) return (0, eC.Z)();
            (0, ex.Z)(I, t.id, ev.ZY5.GUILD_CHANNEL);
        }, [I, t.id, l, i]),
        b = (0, G.x8)('GoLiveButton'),
        N = (0, P.Zm)(t, 'GoLiveButton'),
        Z = (0, P.bK)(),
        [S, T] = a.useState(!1);
    a.useEffect(() => {
        if ((T(t.isHDStreamSplashed && null == u && !Z), t.isHDStreamSplashed)) {
            let e = setTimeout(() => {
                T(!1);
            }, 300000);
            return () => clearTimeout(e);
        }
    }, [t.isHDStreamSplashed, u, N, Z]);
    let j = () => {
            if (l) {
                E();
                return;
            }
            (0, eC.Z)();
        },
        A = () => {
            (0, D.Z)(u);
        },
        R = F.pM,
        L = a.useCallback(() => {
            es.default.track(ev.rMx.PERK_DEMO_OFFER_DISMISSED, {
                guild_id: t.guild_id,
                channel_id: t.id,
                perk_type: h.q.STREAM_HIGH_QUALITY
            }),
                (0, F.qA)();
        }, [t.guild_id, t.id]),
        k = (e, t) => {
            let { onClick: n, ...i } = null != e ? e : { onClick: void 0 },
                l = null != u;
            return (0, r.jsx)(ee.O, {
                ...i,
                centerButton: !0,
                disabled: o || !s,
                className: eb.controlButton,
                hasPermission: s,
                streamActive: null != u,
                isSelfStream: !0,
                onPopoutClick: l
                    ? function (e) {
                          C.hqStreamingIsEnabled && !C.hqStreamingPopoutDismissed && R(), null == n || n(e);
                      }
                    : null,
                popoutOpen: t,
                shouldShowTooltip: !t,
                premiumGlow: S,
                buttonRef: _,
                onClick: null != u ? A : j
            });
        };
    return (0, r.jsxs)(r.Fragment, {
        children: [
            N
                ? (0, r.jsx)(M.ZP, {
                      contentTypes: [m.z.CONSUMABLE_HD_POTION_UPSELL],
                      bypassAutoDismiss: !0,
                      children: (e) => {
                          let { visibleContent: n, markAsDismissed: i } = e;
                          if (n === m.z.CONSUMABLE_HD_POTION_UPSELL)
                              return (0, r.jsx)(y.Z, {
                                  channel: t,
                                  buttonRef: _,
                                  dismissed: !1,
                                  onClick: () => {
                                      i(eE.L.TAKE_ACTION);
                                  },
                                  onDismiss: () => {
                                      i(eE.L.USER_DISMISS);
                                  }
                              });
                      }
                  })
                : b && 0 === d.length
                  ? (0, r.jsx)(M.ZP, {
                        contentTypes: [m.z.TRIAL_FOR_ALL_STREAM_POPOUT],
                        bypassAutoDismiss: !0,
                        children: (e) => {
                            let { visibleContent: t, markAsDismissed: n } = e;
                            if (t === m.z.TRIAL_FOR_ALL_STREAM_POPOUT)
                                return (0, r.jsx)(W.h, {
                                    buttonRef: _,
                                    dismissed: !1,
                                    onDismiss: () => n(eE.L.USER_DISMISS)
                                });
                        }
                    })
                  : x && C.hqStreamingIsEnabled
                    ? (0, r.jsx)(z.$, {
                          buttonRef: _,
                          dismissed: C.hqStreamingPopoutDismissed,
                          onDismiss: R
                      })
                    : (0, r.jsx)(V.b, {
                          channel: t,
                          buttonRef: _,
                          dismissed: C.hqStreamingOptInPopoutDismissed,
                          onDismiss: L
                      }),
            (0, r.jsx)(K.Z, {
                children: (0, r.jsx)(f.Popout, {
                    renderPopout: (e) => {
                        let { closePopout: i } = e;
                        return (0, r.jsx)(Y.Z, {
                            children: (0, r.jsx)(ef.Z, {
                                channel: t,
                                currentUser: n,
                                activeStreams: null != u ? [u] : [],
                                handleGoLive: E,
                                onClose: i,
                                appContext: c
                            })
                        });
                    },
                    position: 'top',
                    align: 'center',
                    animation: f.Popout.Animation.FADE,
                    children: (e, t) => {
                        let { ...n } = e,
                            { isShown: i } = t;
                        return (0, r.jsx)('div', { children: k(n, i) });
                    }
                })
            })
        ]
    });
}
function eS(e) {
    let { channel: t, idle: n } = e,
        i = t.getGuildId();
    return (0, E.Z)(i, t.id)
        ? (0, r.jsx)('div', {
              className: eb.buttonContainer,
              children: (0, r.jsx)(ep.M, {
                  channel: t,
                  idle: n
              })
          })
        : null;
}
((l = i || (i = {})).ACTIVITY = 'ACTIVITY'), (l.STREAM = 'STREAM'), (l.CALL = 'CALL'), (l.EVENT = 'EVENT');
let eT = a.memo(function (e) {
    let { connectedActivityApplicationId: t, currentUser: n, onDisconnectCall: i, channel: l } = e,
        s = (0, p.e7)([Z.Z], () => Z.Z.getSelectedParticipant(l.id)),
        { reducedMotion: o } = a.useContext(f.AccessibilityPreferencesContext),
        [c, d] = a.useState(!1),
        h = null == s ? void 0 : s.id,
        m = (0, p.e7)([en.Z], () => (null != h ? en.Z.getActiveStreamForStreamKey(h) : null), [h]),
        g = (null == s ? void 0 : s.type) === eI.fO.STREAM && null != m && m.ownerId !== (null == n ? void 0 : n.id),
        _ = (0, p.Wu)([en.Z], () =>
            en.Z.getAllActiveStreamsForChannel(l.id).filter((e) => {
                let { ownerId: t } = e;
                return t !== (null == n ? void 0 : n.id);
            })
        ),
        v = (0, w.qY)(l.id),
        I = a.useCallback(() => {
            if ((null == s ? void 0 : s.type) === eI.fO.ACTIVITY && s.id === t) return 'ACTIVITY';
            if (g) return 'STREAM';
            if (null != v) return 'EVENT';
            return 'CALL';
        }, [s, t, g, v]),
        [E, b] = a.useState(I()),
        N = (0, f.useSpring)(
            {
                opacity: c ? 0.2 : 1,
                transform: c && !o.enabled ? 'scale(0.7)' : 'scale(1)',
                config: {
                    mass: 2,
                    tension: c ? 600 : 1000,
                    friction: 45,
                    precision: 0.01,
                    clamp: c
                },
                onRest: () => {
                    b(I()), d(!1);
                }
            },
            'animate-always'
        );
    a.useEffect(() => {
        I() !== E && d(!0);
    }, [E, I]);
    let S = a.useCallback(() => {
            if (null != m) (0, x.g)((0, k.V9)(m));
            else for (let e of _) (0, x.g)((0, k.V9)(e));
        }, [_, m]),
        T = (e, t) => {
            let a = {
                centerButton: !0,
                className: eb.controlButton,
                popoutOpen: e
            };
            switch (E) {
                case 'ACTIVITY':
                    if (null == s || null == n) return;
                    return (0, r.jsx)(J.Z, {
                        ...a,
                        applicationId: s.id,
                        color: 'red',
                        channelId: l.id,
                        onPopoutClick: _.length > 0 ? t : null
                    });
                case 'STREAM':
                    return (0, r.jsx)(ee.O, {
                        ...a,
                        hasPermission: !0,
                        streamActive: !0,
                        color: 'red',
                        onClick: S,
                        onPopoutClick: _.length > 1 ? t : null,
                        isSelfStream: !1
                    });
                case 'CALL':
                    return (0, r.jsx)(Q.Z, {
                        ...a,
                        color: 'red',
                        onClick: () => (null == i ? void 0 : i()),
                        onPopoutClick: _.length > 0 ? t : null
                    });
                case 'EVENT':
                    return (0, r.jsx)(B.Z, {
                        channelId: l.id,
                        onClick: () => {
                            C.default.disconnect(), null == i || i();
                        }
                    });
            }
        };
    return (0, r.jsx)(f.Popout, {
        renderPopout: (e) => {
            let { closePopout: t } = e;
            return (0, r.jsx)(ef.Z, {
                channel: l,
                currentUser: n,
                activeStreams: _,
                handleGoLive: ev.VqG,
                hideSelfOptions: !0,
                onClose: t
            });
        },
        position: 'top',
        align: 'center',
        animation: f.Popout.Animation.FADE,
        children: (e, t) => {
            let { onClick: n } = e,
                { isShown: i } = t;
            return (0, r.jsx)(u.animated.div, {
                style: N,
                children: T(i, n)
            });
        }
    });
});
function ej(e) {
    let { channel: t, cameraUnavailable: n, hasCameraPermission: i, currentUser: l } = e,
        s = (0, v.bp)(),
        { reachedLimit: c, limit: d } = (0, ed.Z)(t),
        u = a.useCallback(() => {
            (0, e_.Z)(() => C.default.selectVoiceChannel(t.id, !0), s);
        }, [t.id, s]),
        h = a.useCallback(() => {
            n ? (0, eg.Z)() : u();
        }, [n, u]),
        m = (0, p.e7)([ei.Z], () => {
            let e = ei.Z.getCall(t.id);
            return (null == e ? void 0 : e.ringing.includes(l.id)) === !0;
        });
    return (0, r.jsxs)(r.Fragment, {
        children: [
            (0, r.jsx)(et.C, {
                enabled: !0,
                centerButton: !0,
                join: !0,
                color: 'green',
                channelLimitReached: c,
                channelLimit: d,
                channel: t,
                cameraUnavailable: n,
                hasPermission: i,
                onCameraUnavailable: h,
                className: eb.controlButton,
                iconClassName: eb.__invalid_joinIcon,
                onChange: u
            }),
            (0, r.jsx)(X.Z, {
                centerButton: !0,
                color: 'green',
                channel: t,
                className: o()(eb.controlButton, eb.__invalid_lastButton)
            }),
            m
                ? (0, r.jsx)(q.Z, {
                      color: 'red',
                      channel: t,
                      className: eb.controlButton
                  })
                : null
        ]
    });
}
t.ZP = function (e) {
    var t;
    let { channel: n, className: i, onDisconnectCall: l, exitFullScreen: a, idleProps: s } = e,
        c = (0, p.e7)([ea.default], () => {
            let e = ea.default.getCurrentUser();
            return d()(null != e, 'CenterControlTray: currentUser cannot be undefined'), e;
        }),
        { cameraUnavailable: u, enabled: h } = (0, ec.Z)(),
        m = (0, eu.Z)(n),
        { suppress: g, selfMute: C, mute: x } = (0, eh.Z)(n),
        { canGoLive: v } = (0, p.cj)([el.Z], () => ({ canGoLive: (0, O.Z)(el.Z) })),
        E = (0, L.Z)(),
        Z = (0, p.e7)([R.Z], () => null != R.Z.getAwaitingRemoteSessionInfo()),
        y = null != E,
        P = (0, p.e7)([er.Z], () => {
            var e;
            return (null !== (e = null == E ? void 0 : E.channelId) && void 0 !== e ? e : er.Z.getVoiceChannelId()) === n.id;
        }),
        M = (0, S.Z)(n, !0),
        k = (0, p.e7)([I.ZP], () => {
            let e = I.ZP.getSelfEmbeddedActivityForChannel(n.id);
            return null != e ? e.applicationId : null;
        }),
        { reachedLimit: D, limit: w } = (0, ed.Z)(n),
        { analyticsLocations: B } = (0, N.ZP)(b.Z.VOICE_CONTROL_TRAY);
    if (!P)
        return (0, r.jsx)(N.Gt, {
            value: B,
            children: (0, r.jsx)(ej, {
                channel: n,
                cameraUnavailable: u,
                hasCameraPermission: m,
                currentUser: c
            })
        });
    let H =
        eo.BT({
            permission: ev.Plq.ADD_REACTIONS,
            user: c,
            context: n
        }) &&
        !n.isPrivate() &&
        !M;
    return (0, r.jsx)(N.Gt, {
        value: B,
        children: (0, r.jsxs)(_.Z, {
            section: ev.jXE.VOICE_CONTROL_TRAY,
            children: [
                (0, r.jsx)('div', {
                    className: eb.eventPromptsContainer,
                    children: (0, r.jsx)(U.Z, { channelId: n.id })
                }),
                (0, r.jsxs)('div', {
                    className: o()(eb.wrapper, i),
                    children: [
                        !y &&
                            (0, r.jsx)(f.Popout, {
                                renderPopout: (e) => {
                                    let { closePopout: t } = e;
                                    return (0, r.jsx)(Y.Z, { children: (0, r.jsx)(j.Z, { onClose: t }) });
                                },
                                position: 'top',
                                align: 'center',
                                animation: f.Popout.Animation.FADE,
                                children: (e, t) => {
                                    let { onClick: n } = e,
                                        { isShown: i } = t;
                                    return (0, r.jsx)(et.C, {
                                        centerButton: !0,
                                        hasPermission: m,
                                        className: eb.controlButton,
                                        enabled: h,
                                        cameraUnavailable: u,
                                        onChange: eN,
                                        onCameraUnavailable: eg.Z,
                                        channelLimitReached: D,
                                        channelLimit: w,
                                        popoutOpen: i,
                                        onPopoutClick: n
                                    });
                                }
                            }),
                        !y &&
                            (0, r.jsx)(eS, {
                                channel: n,
                                idle: null === (t = null == s ? void 0 : s.idle) || void 0 === t || t
                            }),
                        !y &&
                            (0, r.jsx)(eZ, {
                                channel: n,
                                currentUser: c,
                                exitFullScreen: a,
                                canGoLive: v,
                                hasPermission: m
                            }),
                        (0, r.jsx)(f.Popout, {
                            renderPopout: (e) => {
                                let { closePopout: t } = e;
                                return (0, r.jsx)(Y.Z, {
                                    children: (0, r.jsx)(T.default, {
                                        onClose: t,
                                        renderInputDevices: !0,
                                        renderOutputDevices: !0,
                                        renderInputModes: !0
                                    })
                                });
                            },
                            align: 'center',
                            position: 'top',
                            animation: f.Popout.Animation.FADE,
                            children: (e, t) => {
                                let { onClick: n } = e,
                                    { isShown: i } = t;
                                return (0, r.jsx)($.Z, {
                                    centerButton: !0,
                                    onPopoutClick: y ? null : n,
                                    className: eb.controlButton,
                                    selfMute: C,
                                    serverMute: x,
                                    suppress: g,
                                    popoutOpen: i,
                                    awaitingRemote: Z,
                                    onClick: () => (0, A.Z)(x, g, ev.jXE.VOICE_CONTROL_TRAY)
                                });
                            }
                        }),
                        H && !y
                            ? (0, r.jsx)('div', {
                                  className: eb.buttonContainer,
                                  children: (0, r.jsx)(em.Z, { channel: n })
                              })
                            : null,
                        (0, r.jsx)(eT, {
                            connectedActivityApplicationId: k,
                            currentUser: c,
                            channel: n,
                            onDisconnectCall: l
                        })
                    ]
                })
            ]
        })
    });
};
