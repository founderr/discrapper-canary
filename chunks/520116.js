n.d(t, {
  Z: function() {
return M;
  }
}), n(653041);
var i = n(735250),
  s = n(470079),
  a = n(913527),
  r = n.n(a),
  l = n(91192),
  o = n(442837),
  c = n(481060),
  u = n(135938),
  d = n(900164),
  _ = n(554838),
  E = n(493892),
  I = n(661824),
  m = n(37772),
  T = n(695346),
  h = n(314897),
  N = n(496675),
  C = n(594174),
  f = n(55935),
  p = n(655354),
  g = n(809780),
  S = n(981631),
  A = n(689938),
  R = n(104219);
let O = {
  left: 4,
  right: -12
};

function x(e) {
  var t;
  let {
channel: n,
message: a,
compact: r,
isGroupStart: o,
treatSpam: u,
gotoChannel: d
  } = e, _ = (0, l.JA)(null !== (t = a.id) && void 0 !== t ? t : ''), E = s.useCallback(e => {
if ('ArrowLeft' === e.key) {
  var t;
  null === (t = document.querySelector('[data-recents-channel="'.concat(n.id, '"]'))) || void 0 === t || t.focus();
}
  }, [n.id]);
  return (0, i.jsx)(c.FocusRing, {
offset: O,
children: (0, i.jsxs)('div', {
  className: R.messageContainer,
  onKeyDown: E,
  ..._,
  children: [
    (0, i.jsx)(p.Z, {
      className: R.jumpButton,
      onJump: e => d(e, a.id)
    }),
    (0, i.jsx)(m.Z, {
      id: a.id,
      message: a,
      channel: n,
      className: R.message,
      compact: r,
      animateAvatar: !1,
      isGroupStart: o,
      onKeyDown: E,
      treatSpam: u
    })
  ]
}, a.id)
  });
}

function M(e) {
  var t, n;
  let {
channel: s,
channelRecord: a,
gotoChannel: l
  } = e, {
enabled: m
  } = u.Z.useExperiment({
location: '20e3b0_1'
  }, {
autoTrackExposure: !1
  }), p = T.jU.useSetting(), O = (0, o.e7)([N.Z], () => N.Z.can(S.Plq.CREATE_INSTANT_INVITE, a)), M = (0, E.P1)(a), v = !1, L = 0 === s.messages.length || r()(s.messages[0].timestamp).isSame(r()(), 'day'), Z = null !== (n = null === (t = C.default.getUser(h.default.getId())) || void 0 === t ? void 0 : t.hasFlag(S.xW$.SPAMMER)) && void 0 !== n && n, P = [];
  if (!s.collapsed) {
let e = null,
  t = null,
  n = s.messages.slice(0, g.hC);
n.forEach(n => {
  if (!(0, d.Z)(n, O)) {
    if (!L && (null == e || !e.isSame(n.timestamp, 'day'))) {
      let t = (0, f.vc)(n.timestamp, 'LL');
      P.push((0, i.jsx)(I.Z, {
        className: R.divider,
        children: t
      }, t)), e = r()(n.timestamp);
    }
    let s = null == t || (0, _.Z)(a, t, n);
    t = n, v = v || (0, E.DQ)(n), P.push((0, i.jsx)(x, {
      channel: a,
      message: n,
      compact: p,
      isGroupStart: s,
      treatSpam: !Z && m && (0, E.DQ)(n) && M,
      gotoChannel: l
    }, n.id));
  }
}), s.messages.length >= g.hC && P.push((0, i.jsxs)(c.Button, {
  color: c.Button.Colors.LINK,
  look: c.Button.Looks.LINK,
  onClick: e => l(e, n[n.length - 1].id),
  children: [
    A.Z.Messages.UNREADS_VIEW_CHANNEL,
    ' \u203A'
  ]
}, 'view-all')), 0 === P.length && (P = [(0, i.jsx)(c.Spinner, {}, 'spinner')]);
  }
  return v && M && u.Z.trackExposure({
location: '20e3b0_2'
  }), (0, i.jsx)('div', {
className: R.messages,
children: P
  });
}