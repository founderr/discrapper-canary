...
class S extends _.Z {
    ...
    constructor(e, t) {
        for (let n in (super(e, t), f(this, 'paymentId', null), 100027 === this.code ? (this.message = E.Z.Messages.BILLING_ERROR_NEGATIVE_INVOICE_AMOUNT) : 50048 === this.code ? (this.message = E.Z.Messages.BILLING_PAYMENT_SOURCE_INVALID) : 100002 === this.code ? (this.message = E.Z.Messages.BILLING_ERROR_UNKNOWN_PAYMENT_SOURCE) : 100042 === this.code ? (this.message = E.Z.Messages.BILLING_ERROR_PENDING_PAYMENT) : 100078 === this.code ? (this.message = E.Z.Messages.BILLING_TRIAL_REDEMPTION_DISABLED) : 100096 === this.code ? (this.message = E.Z.Messages.BILLING_BUNDLE_ALREADY_PURCHASED) : 100097 === this.code ? (this.message = E.Z.Messages.BILLING_BUNDLE_PARTIALLY_OWNED) : 100054 === this.code ? (this.message = E.Z.Messages.BILLING_ERROR_DECLINED_BY_ISSUER) : 429 === this.status ? (this.message = E.Z.Messages.BILLING_ERROR_RATE_LIMIT) : 0 === this.code ? (this.message = E.Z.Messages.BILLING_ERROR_GENERIC) : 400 === this.status && null != this.fields.captcha_key && (this.message = E.Z.Messages.BILLING_ERROR_INVALID_CAPTCHA_RESPONSE), this.fields)) {
            let e = h[n] || p[n];
            if (null != e) {
                let t = this.fields[n];
...
f(S, 'ErrorCodes', r), f(S, 'Fields', i), f(S, 'Sections', s), f(S, 'CARD_ERRORS', I), f(S, 'ADDRESS_ERRORS', m), (t.ZP = S);
