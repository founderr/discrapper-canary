...
e.exports = function(e, t, n, u, c, l, d, f) {
  ...
  for (T = 15, D = S; T >= 1 && 0 === L[T]; T--);
  if (D > T && (D = T), 0 === T) return c[l++] = 20971520, c[l++] = 20971520, f.bits = 1, 0;
  for (C = 1; C < T && 0 === L[C]; C++);
  for (D < C && (D = C), A = 1, x = 1; x <= 15; x++)
    if (A <<= 1, (A -= L[x]) < 0) return -1;
  if (A > 0 && (0 === e || 1 !== T)) return -1;
  for (x = 1, P[1] = 0; x < 15; x++) P[x + 1] = P[x] + L[x];
  for (w = 0; w < u; w++) 0 !== t[n + w] && (d[P[t[n + w]]++] = w);
  if (0 === e ? (N = B = d, b = 19) : 1 === e ? (N = i, I -= 257, B = a, F -= 257, b = 256) : (N = o, B = s, b = -1), R = 0, w = 0, x = C, _ = l, M = D, O = 0, m = -1, g = (k = 1 << D) - 1, 1 === e && k > 852 || 2 === e && k > 592) return 1;
  for (;;) {
    v = x - O, d[w] < b ? (y = 0, E = d[w]) : d[w] > b ? (y = B[F + d[w]], E = N[I + d[w]]) : (y = 96, E = 0), p = 1 << x - O, C = h = 1 << M;
    do c[_ + (R >> O) + (h -= p)] = v << 24 | y << 16 | E | 0; while (0 !== h);
    for (p = 1 << x - 1; R & p;) p >>= 1;
    ...
    if (x > D && (R & g) !== m) {
      for (0 === O && (O = D), _ += C, A = 1 << (M = x - O); M + O < T && !((A -= L[M + O]) <= 0);) {
        ;
        M++, A <<= 1
      }
      if (k += 1 << M, 1 === e && k > 852 || 2 === e && k > 592) return 1;
      c[m = R & g] = D << 24 | M << 16 | _ - l | 0
    }
  }
  return 0 !== R && (c[_ + R] = x - O << 24 | 4194304), f.bits = D, 0
}