...
t.Z = new class e extends _ {
        ...
        if (!this.renderLatestMessages.hasData()) {
            if (this.renderMessages.record(), t.length > 0 && this.renderMessagesWithCache.record(), r) {
                ...
                if (e === this.cachedChannelId) {
                    var o;
                    let n = this.cachedMessageIds.sort(a.compare).reverse()[0], r = t.sort(a.compare).reverse()[0];
    ...
    processNativeLogs(e, t) {
        ...
        for (let t of e)
            ...
            case 'ChatModule.updateRows() Start':
                if (null != this.extraProperties.time_first_native_message_render_start)
                    continue;
                this.extraProperties.time_first_native_message_render_start = l(n, t.timestamp);
                break;
            case 'ChatModule.updateRows() Finish':
                if (null != this.extraProperties.time_first_native_message_render_end)
                    continue;
    ...
    serializeTTITracker(e) {
        var t, i, a, o, s, l;
        let u = this.getStartTime(e), c = n(392711)(r.Z.logGroups['0'].logs).filter(e => e.log.startsWith('Require ')).map(e => {
                var t;
