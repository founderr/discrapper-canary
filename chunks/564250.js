...
for (E = 0; E < c.length; ++E) {
  if (E !== d) {
    var E, f, h, p = c[E],
      I = p.indexOf(']='),
      m = -1 === I ? p.indexOf('=') : I + 1;
    if (-1 === m ? (f = t.decoder(p, s.decoder, _, 'key'), h = t.strictNullHandling ? null : '') : (f = t.decoder(p.slice(0, m), s.decoder, _, 'key'), h = r.maybeMap(o(p.slice(m + 1), t), function(e) {
...
for (var i = r ? t : o(t, n), a = e.length - 1; a >= 0; --a) {
  ...
  if ('[]' === l && n.parseArrays)
    ...
    var u = '[' === l.charAt(0) && ']' === l.charAt(l.length - 1) ? l.slice(1, -1) : l,
      c = parseInt(u, 10);
    n.parseArrays || '' !== u ? !isNaN(c) && l !== u && String(c) === u && c >= 0 && n.parseArrays && c <= n.arrayLimit ? (s = [])[c] = i : '__proto__' !== u && (s[u] = i) : s = {
      0: i
    };
  }
  i = s;
