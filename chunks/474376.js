"use strict";
n.r(t), n.d(t, {
  useMultiAccountMenuItems: function() {
    return I
  }
}), n("653041");
var a = n("735250");
n("470079");
var s = n("442837"),
  i = n("481060"),
  l = n("44315"),
  r = n("598077"),
  o = n("246946"),
  u = n("594174"),
  d = n("441674"),
  c = n("759231"),
  f = n("626135"),
  E = n("51144"),
  h = n("480387"),
  _ = n("726745"),
  C = n("251423"),
  m = n("383832"),
  p = n("981631"),
  S = n("689938"),
  g = n("368110");

function I() {
  let e = (0, s.useStateFromStores)([u.default], () => u.default.getCurrentUser()),
    t = (0, s.useStateFromStores)([o.default], () => o.default.hidePersonalInformation),
    {
      multiAccountUsers: n
    } = (0, C.useMultiAccountUsers)(),
    I = t => {
      t !== (null == e ? void 0 : e.id) && (f.default.track(p.AnalyticEvents.MULTI_ACCOUNT_SWITCH_ATTEMPT, {
        location: {
          section: p.AnalyticsSections.PROFILE_POPOUT
        }
      }), h.switchAccount(t))
    },
    T = n.map(n => {
      let s = new r.default(n),
        o = s.id === (null == e ? void 0 : e.id),
        u = n.tokenStatus === _.MultiAccountTokenStatus.INVALID,
        f = t ? null : "#".concat(s.discriminator);
      return (0, a.jsx)(i.MenuItem, {
        id: s.id,
        focusedClassName: g.focused,
        label: e => {
          let {
            isFocused: r
          } = e;
          return (0, a.jsxs)("div", {
            className: g.userMenuItem,
            children: [(0, a.jsx)(i.Avatar, {
              src: s.getAvatarURL(void 0, 40),
              size: i.AvatarSizes.SIZE_24,
              "aria-label": n.username
            }), (0, a.jsxs)("div", {
              className: g.userMenuUsername,
              children: [(0, a.jsx)(i.Text, {
                className: g.userMenuText,
                variant: "text-sm/normal",
                children: E.default.getUserTag(s, {
                  mode: "username",
                  identifiable: t ? "never" : "always"
                })
              }), !s.isPomelo() && (0, a.jsx)(i.Text, {
                className: g.userMenuDiscriminator,
                variant: "text-sm/normal",
                children: f
              })]
            }), o && (0, a.jsx)(d.default, {
              color: (0, l.getColor)(r ? p.Color.WHITE_500 : p.Color.BRAND_500),
              backgroundColor: (0, l.getColor)(r ? p.Color.BRAND_500 : p.Color.WHITE_500),
              width: 18,
              height: 18,
              className: g.activeIcon
            }), u && (0, a.jsx)(c.default, {
              color: (0, l.getColor)(p.Color.RED_400),
              backgroundColor: (0, l.getColor)(p.Color.WHITE_500),
              width: 15,
              height: 15,
              className: g.activeIcon
            })]
          })
        },
        action: () => {
          u ? (0, m.default)() : I(s.id)
        }
      }, s.id)
    });
  return T.push((0, a.jsxs)(a.Fragment, {
    children: [(0, a.jsx)(i.MenuSeparator, {}), (0, a.jsx)(i.MenuItem, {
      id: "manage-accounts",
      label: S.default.Messages.SWITCH_ACCOUNTS_MANAGE_ACCOUNTS,
      action: m.default
    })]
  })), T
}