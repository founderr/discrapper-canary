...
function C(e) {
  let {
    ...
    linksCount: S,
    mediaCount: _
  } = (0, f.useServerActivityCountsForUser)(t, n), T = new Intl.NumberFormat(p.default.getLocale()).format(g), I = new Intl.NumberFormat(p.default.getLocale()).format(S), A = new Intl.NumberFormat(p.default.getLocale()).format(_), x = l.useCallback(async e => {
    if (null == C) return;
