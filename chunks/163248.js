...
e.default = class t {
    ...
    trigger(t) {
        ...
        if (t) {
            if ('number' == typeof t && isFinite(t))
                ...
                throw Error('`trigger` is not correct, must be a finite number or a supported date!');
            else
                this.data.trigger = (0, n.checkDate)(t, 'trigger');
        } else
    ...
    interval(t) {
        ...
        if (!t)
            return this.data.interval = null, this;
        if ('number' != typeof t || !isFinite(t))
    ...
    attach(t) {
        ...
        if ('string' == typeof t)
            ...
            throw Error('`attachment` needs to be a valid formed string or an object. See https://sebbo2002.github.io/ical-generator/develop/reference/classes/icalalarm.html#attach');
        if (!e.uri)
            throw Error('`attach.uri` is empty!');
        return this.data.attach = {
            uri: e.uri,
            mime: e.mime
        }, this;
    ...
    x(t, e) {
        if (void 0 === t)
            return (0, n.addOrGetCustomAttributes)(this.data);
    ...
    toString() {
        ...
        if (t += 'ACTION:' + this.data.type.toUpperCase() + '\r\n', 'number' == typeof this.data.trigger && this.data.trigger > 0 ? t += 'TRIGGER;RELATED=END:' + (0, n.toDurationString)(this.data.trigger) + '\r\n' : 'number' == typeof this.data.trigger ? t += 'TRIGGER:' + (0, n.toDurationString)(this.data.trigger) + '\r\n' : t += 'TRIGGER;VALUE=DATE-TIME:' + (0, n.formatDate)(this.event.timezone(), this.data.trigger) + '\r\n', this.data.repeat && !this.data.interval)
            throw Error('No value for `interval` in ICalAlarm given, but required for `repeat`!');
        if (this.data.repeat && (t += 'REPEAT:' + this.data.repeat + '\r\n'), this.data.interval && !this.data.repeat)
            throw Error('No value for `repeat` in ICalAlarm given, but required for `interval`!');
