...
function r(t, e, a, i) {
    if ((null == t ? void 0 : t.startsWith('/')) && (t = t.substr(1)), 'string' == typeof e || e instanceof Date) {
        let r = new Date(e), n = r.getUTCFullYear() + String(r.getUTCMonth() + 1).padStart(2, '0') + r.getUTCDate().toString().padStart(2, '0');
    ...
    if (l(e)) {
        ...
        let r = e;
        if (t)
            r = 'function' == typeof e.tz ? e.tz(t) : e;
        else if (i);
        else if ('function' == typeof e.utc)
            r = e.utc();
        else
            throw Error('Unable to convert dayjs object to UTC value: UTC plugin is not available!');
        return r.format('YYYYMMDD') + (a ? '' : 'T' + r.format('HHmmss') + (i || t ? '' : 'Z'));
    }
}
...
e.formatDateTZ = function (t, e, a, i) {
    let n = '', s = (null == i ? void 0 : i.floating) || !1;
...
e.checkNameAndMail = function (t, e) {
    ...
    if (a || 'string' != typeof e) {
        if (!a)
            throw Error('`' + t + '` needs to be a valid formed string or an object. See https://sebbo2002.github.io/ical-generator/develop/reference/interfaces/icalorganizer.html');
    } else
        throw Error('`' + t + '` isn\'t formated correctly. See https://sebbo2002.github.io/ical-generator/develop/reference/interfaces/icalorganizer.html');
    if (!a.name)
        throw Error('`' + t + '.name` is empty!');
    return a;
};
e.checkEnum = function (t, e) {
    let a = Object.values(t), i = String(e).toUpperCase();
    if (!i || !a.includes(i))
