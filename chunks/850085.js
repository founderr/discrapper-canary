...
t.default = e => {
  ...
  let z = () => {
      switch (f) {
        ...
        case g.pC.AVATAR_DECORATION:
          return {
            height: A.dGM, width: A.dGM
          };
        case g.pC.GUILD_BANNER:
          return {
            height: g._T, width: g.X_
          };
        case g.pC.SCHEDULED_EVENT_IMAGE:
          return {
            height: g.xT, width: g.d6
          };
        case g.pC.HOME_HEADER:
          return {
            height: g.Uo, width: g.N8
          }
      }
    },
    ...
    J = a.useCallback(e => {
      ...
      if (!x || e.clientX === t && e.clientY === n) return;
      let r = e.clientX - B.x;
      Z(r, e.clientY - B.y, L)
    }, [L, x, B, Z]),
    K = () => {
  ...
  return a.useEffect(() => (window.addEventListener("mouseup", K), window.addEventListener("resize", Y), () => {
    ...
    if (x) return window.addEventListener("mousemove", J), () => window.removeEventListener("mousemove", J)
  }, [J, x]), (0, r.jsx)(o.Gt, {
    value: G,
