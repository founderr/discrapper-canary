n.d(t, {
  W: function() {
return c;
  }
}), n(411104);
var r = n(512722),
  i = n.n(r),
  a = n(710845),
  s = n(459005),
  o = n(398463);

function l(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
value: n,
enumerable: !0,
configurable: !0,
writable: !0
  }) : e[t] = n, e;
}
let u = new a.Z('DispatcherWorkScheduler');
class c {
  _queueIdleCallback() {
throw Error('Not implemented');
  }
  _clearIdleCallback() {
throw Error('Not implemented');
  }
  get _hasExceededMaxConsecutiveFlushes() {
return this._consecutiveFlushesBeforeQueueEmpty >= o.OT;
  }
  _processWorkCallback(e) {
if (null == this._workCallbackFn)
  return;
if (this._hasExceededMaxConsecutiveFlushes) {
  u.log('Unable to fully flush work queue after max retries, skipping future deadline.'), this.telemetry.time(s.JV.TASK_TIME_AFTER_DEADLINE_EXPIRED), this._workCallbackFn(), this.telemetry.timeEnd(s.JV.TASK_TIME_AFTER_DEADLINE_EXPIRED), this.clearWorkTimeout(), this.telemetry.measure(s.aA.COUNT_FLUSH_BEFORE_QUEUE_EMPTY, parseInt(''.concat(this._consecutiveFlushesBeforeQueueEmpty))), this.telemetry.track(s.ug.EXCEEDED_MAX_CONSECUTIVE_FLUSHES), this._consecutiveFlushesBeforeQueueEmpty = 0, this._nextDispatchTimeout = o.II;
  return;
}
let t = performance.now(),
  n = this._workCallbackFn(e),
  r = performance.now();
this.clearWorkTimeout();
r - t > o.TC ? this._nextDispatchTimeout = o.II : this._nextDispatchTimeout = o.Lu, n ? (this._consecutiveFlushesBeforeQueueEmpty > 0 && this.telemetry.measure(s.aA.COUNT_FLUSH_BEFORE_QUEUE_EMPTY, parseInt(''.concat(this._consecutiveFlushesBeforeQueueEmpty))), this._consecutiveFlushesBeforeQueueEmpty = 0) : this._consecutiveFlushesBeforeQueueEmpty += 1;
  }
  get hasWorkScheduled() {
return null != this._flushTimeoutHandler || null != this._flushIdleHandler;
  }
  clearWorkTimeout() {
null != this._flushTimeoutHandler && (clearTimeout(this._flushTimeoutHandler), this._flushTimeoutHandler = null), this._clearIdleCallback(), this._nextDispatchTimeout = o.Lu, this._workCallbackFn = null;
  }
  requestWorkTimeout(e) {
if (this._workCallbackFn = e, !this.hasWorkScheduled)
  this.telemetry.time(s.JV.TIME_TO_QUEUE_EMPTY), this._nextDispatchTimeout === o.II && this.telemetry.track(s.ug.LONGER_DISPATCH), this.telemetry.track(s.ug.DISPATCH_REQUEST), this._flushTimeoutHandler = setTimeout(() => {
    i()(null != this._workCallbackFn, 'Work callback should be set'), this._queueIdleCallback();
  }, this._nextDispatchTimeout);
  }
  constructor() {
l(this, '_flushTimeoutHandler', null), l(this, '_flushIdleHandler', null), l(this, '_nextDispatchTimeout', o.Lu), l(this, '_workCallbackFn', null), l(this, '_consecutiveFlushesBeforeQueueEmpty', 0), l(this, 'telemetry', new s.mA()), l(this, '_logger', u);
  }
}