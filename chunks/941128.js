"use strict";
n(47120), n(411104), n(653041), n(733860);
var i, r, s, o, a, l, u, _, c = n(392711),
  d = n.n(c),
  E = n(442837),
  I = n(433517),
  T = n(570140),
  h = n(51025),
  f = n(594190),
  S = n(314897),
  A = n(173747),
  N = n(780570),
  m = n(830168),
  O = n(358085),
  R = n(417363),
  p = n(981631),
  g = n(186901);
(o = i || (i = {})).INSTALL = "Install", o.REPAIR = "Repair", (a = r || (r = {})).PATCH = "Patch", a.REPAIR = "Repair";
let C = [g.ff.AUTHENTICATION_FAILED, g.ff.NOT_ENTITLED],
  v = "DispatchManagerStore",
  L = [],
  D = [],
  M = !1,
  P = null,
  y = null,
  U = !1,
  b = new Map,
  G = !1,
  w = null;

function B() {
  let e = {
    queue: L,
    paused: M,
    userActions: Array.from(b)
  };
  I.K.set(v, e)
}

function x() {
  let e = L[0];
  if (null != e) {
    var t, n;
    let {
      comboId: i,
      action: r
    } = e, {
      applicationId: s,
      branchId: o
    } = (0, N.CP)(i);
    if (t = s, n = o, (null == P || P.applicationId !== t || P.branchId !== n) && (null == y || y.applicationId !== t || y.branchId !== n)) {
      let e = S.default.getToken(),
        t = S.default.getId();
      if (null == e) throw Error("missing user token");
      G = !m.Z.setCurrentTask(s, o, r, t, e)
    }
  }
}

function k(e, t) {
  let n = (0, N.Tu)(e, t);
  return L.findIndex(e => e.comboId === n)
}

function V(e, t, n, i) {
  let r = (0, N.Tu)(e, t),
    s = {
      comboId: r,
      action: i
    },
    o = D.indexOf(r); - 1 !== o && D.splice(o, 1);
  let a = k(e, t);
  0 !== a && (n ? -1 === a && (L.push(s), x()) : (a > 0 && L.splice(a, 1), L.unshift(s), x())), !n && M && m.Z.resume(), B()
}

function Z(e, t) {
  let n = (0, N.Tu)(e, t),
    i = D.indexOf(n); - 1 !== i && D.splice(i, 1);
  let r = k(e, t); - 1 !== r && (L.splice(r, 1), B()), x()
}

function H(e) {
  let {
    applicationId: t,
    branchId: n
  } = e;
  Z(t, n)
}

function F(e) {
  let {
    applicationId: t,
    branchId: n
  } = e, i = (0, N.Tu)(t, n), r = D.indexOf(i); - 1 !== r && D.splice(r, 1)
}

function Y() {
  let e = S.default.getToken(),
    t = S.default.getId();
  if (null != e) m.Z.setCredentials(t, e)
}

function j() {
  for (let e of f.ZP.getRunningDiscordApplicationIds()) h.al(e, e);
  let e = f.ZP.getVisibleGame();
  return !M && null != e && e.pid !== w && h.wO(), w = null == e ? null : e.pid, !1
}
class W extends(s = E.ZP.Store) {
  initialize() {
    var e;
    let t = null !== (e = I.K.get(v)) && void 0 !== e ? e : {
      queue: null,
      paused: null,
      userActions: null
    };
    if (null != t.queue) L = t.queue.map(e => "string" == typeof e ? {
      comboId: e,
      action: "Patch"
    } : e);
    null != t.paused && (M = t.paused), null != t.userActions && (b = new Map(Array.from(t.userActions))), this.waitFor(R.Z, f.ZP), this.syncWith([f.ZP], j), this.waitFor(R.Z)
  }
  get activeItems() {
    return L.map(e => {
      let {
        comboId: t
      } = e;
      return (0, N.CP)(t)
    })
  }
  get finishedItems() {
    return D.map(N.CP)
  }
  get paused() {
    return M
  }
  getQueuePosition(e, t) {
    return k(e, t)
  }
  isCorruptInstallation() {
    return G
  }
}
_ = "DispatchManagerStore", (u = "displayName") in(l = W) ? Object.defineProperty(l, u, {
  value: _,
  enumerable: !0,
  configurable: !0,
  writable: !0
}) : l[u] = _, t.Z = new W(T.Z, {
  DISPATCH_APPLICATION_INSTALL: function(e) {
    let {
      applicationId: t,
      branchId: n
    } = e;
    b.set((0, N.Tu)(t, n), "Install"), V(t, n, !1, "Patch")
  },
  DISPATCH_APPLICATION_UPDATE: function(e) {
    let {
      applicationId: t,
      branchId: n,
      automatic: i
    } = e;
    V(t, n, i, "Patch")
  },
  DISPATCH_APPLICATION_UNINSTALL: function(e) {
    H(e), F(e)
  },
  DISPATCH_APPLICATION_CANCEL: H,
  DISPATCH_APPLICATION_REPAIR: function(e) {
    let {
      applicationId: t,
      branchId: n
    } = e;
    b.set((0, N.Tu)(t, n), "Repair"), V(t, n, !1, "Repair")
  },
  DISPATCH_APPLICATION_MOVE_UP: function(e) {
    let {
      applicationId: t,
      branchId: n
    } = e, i = k(t, n);
    if (i < 1) return !1;
    L.splice(0, 0, L.splice(i, 1)[0]), x(), M && m.Z.resume(), B()
  },
  DISPATCH_APPLICATION_REMOVE_FINISHED: F,
  DISPATCH_APPLICATION_STATE_UPDATE: function(e) {
    let {
      state: t
    } = e;
    !U && (U = !0, x(), !M && m.Z.resume());
    let n = M;
    M = t.paused, P = t.currentTask, y = t.nextTask;
    let i = !1;
    L = L.filter(e => {
      let {
        comboId: t
      } = e, {
        applicationId: n,
        branchId: r
      } = (0, N.CP)(t), s = R.Z.getState(n, r), o = A.Z.getTargetBuildId(n, r), a = A.Z.getTargetManifests(n, r);
      if (null != s && s.type === p.vxO.UP_TO_DATE && s.buildId === s.targetBuildId && s.buildId === o && d().isEqual(s.manifestIds, s.targetManifestIds) && d().isEqual(s.manifestIds, a)) {
        if (D.push(t), b.has(t)) {
          switch (b.get(t)) {
            case "Install":
              h.XT(n, s);
              break;
            case "Repair":
              h.Wx(n, s)
          }
          b.delete(t)
        }
        return i = !0, !1
      }
      return !0
    }), x(), (i || n !== M) && B()
  },
  DISPATCH_APPLICATION_ERROR: function(e) {
    let {
      error: t
    } = e, {
      code: n
    } = t;
    if (null != n) {
      if (C.includes(n)) Y();
      else if (n === g.ff.APPLICATION_NOT_FOUND) {
        let {
          context: e
        } = t;
        if (null != e) {
          let {
            application_id: t,
            branch_id: n
          } = e;
          Z(t, n)
        }
      }
    }
  },
  CONNECTION_OPEN: function() {
    (0, O.isDesktop)() && Y()
  },
  LOGOUT: function() {
    I.K.remove(v), (0, O.isDesktop)() && m.Z.pause()
  }
})