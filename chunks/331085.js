...
function x(e) {
  let {
    ...
    h.current = o.useCallback(() => {
      l({
        step: 0
      }), c ? n(e => e + 1) : (C.info("document is not visible, will defer reconnection when document becomes visible."), a(!0))
    }, [c]);
    let _ = o.useCallback(() => {
        (function(e) {
          let {
      ...
      g = o.useCallback(() => {
        C.error("Could not complete QR code login, trying to restart with a new QR code."), l({
          step: 0
        }), !E.pending && E.fail(_)
      }, [_, E]);
    return o.useEffect(() => {
      c && s && 0 === r.step && (C.info("reconnecting, now that document is visible"), a(!1), n(e => e + 1))
    }, [r, c, s, a]), o.useEffect(() => {
      let t = Date.now(),
      ...
      return a.onmessage = async t => {
        ...
        switch (s.op) {
          ...
          case "pending_remote_init": {
            E.succeed(), m.ComponentDispatch.dispatch(R.ComponentActions.WAVE_EMPHASIZE);
          ...
          case "pending_ticket": {
            m.ComponentDispatch.dispatch(R.ComponentActions.WAVE_EMPHASIZE), r("remote auth handshake started, awaiting ticket/cancel.");
          ...
          case "pending_finish": {
            m.ComponentDispatch.dispatch(R.ComponentActions.WAVE_EMPHASIZE), r("remote auth handshake started, awaiting finish/cancel.");
            let e = s.encrypted_user_payload,
              t = await (0, p.decodeEncodedUserRecord)(h(), e);
            l({
              step: 2,
          ...
          case "finish": {
            m.ComponentDispatch.dispatch(R.ComponentActions.WAVE_EMPHASIZE), r("remote auth handshake finished.");
