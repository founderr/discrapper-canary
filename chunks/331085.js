...
function D(e) {
      ...
      return a.onmessage = async t => {
        ...
        switch (s.op) {
          ...
          case "pending_remote_init": {
            E.succeed(), m.ComponentDispatch.dispatch(A.ComponentActions.WAVE_EMPHASIZE);
            let e = await (0, p.publicKeyFingerprint)(_());
          ...
          case "pending_ticket": {
            m.ComponentDispatch.dispatch(A.ComponentActions.WAVE_EMPHASIZE), r("remote auth handshake started, awaiting ticket/cancel.");
            let e = s.encrypted_user_payload,
              t = await (0, p.decodeEncodedUserRecord)(_(), e);
            l({
              step: 3,
              user: t
            });
          ...
          case "pending_finish": {
            m.ComponentDispatch.dispatch(A.ComponentActions.WAVE_EMPHASIZE), r("remote auth handshake started, awaiting finish/cancel.");
            let e = s.encrypted_user_payload,
              t = await (0, p.decodeEncodedUserRecord)(_(), e);
            l({
              step: 2,
          ...
          case "finish": {
            m.ComponentDispatch.dispatch(A.ComponentActions.WAVE_EMPHASIZE), r("remote auth handshake finished.");
            let t = s.encrypted_token;
            l({
              step: 5
            });
            let n = await (0, p.decryptEncodedCiphertext)(_(), t);
            e(n);
            return
          }
          case "cancel":
  ...
  return o.useEffect(() => {
    4 === n.step && null != n.ticket && d.default.post({
      url: A.Endpoints.REMOTE_AUTH_LOGIN,
      body: {
        ticket: n.ticket
      },
      oldFormErrors: !0
