n.d(t, {
  Z: function() {
return U;
  }
}), n(47120);
var r = n(735250),
  i = n(470079),
  a = n(442837),
  s = n(481060),
  o = n(727637),
  l = n(58540),
  u = n(100527),
  c = n(906732),
  d = n(564990),
  _ = n(680295),
  E = n(271383),
  f = n(430824),
  h = n(785717),
  p = n(481046),
  m = n(27144),
  I = n(318661),
  T = n(78675),
  g = n(113557),
  S = n(169979),
  A = n(215105),
  N = n(502762),
  v = n(544989),
  O = n(481932),
  R = n(195387),
  C = n(272510),
  y = n(171368),
  D = n(958120),
  L = n(215633),
  b = n(228168),
  M = n(689938),
  P = n(335182);

function U(e) {
  let {
user: t,
currentUser: n,
guildId: U,
channelId: w,
messageId: x,
roleId: G,
closePopout: k,
setPopoutRef: B,
disableUserProfileLink: F = __OVERLAY__,
newAnalyticsLocations: V = []
  } = e, {
analyticsLocations: H
  } = (0, c.ZP)([
...V,
u.Z.BITE_SIZE_PROFILE_POPOUT
  ]), Z = (0, h.Q1)({
layout: 'BITE_SIZE_POPOUT',
userId: t.id,
guildId: U,
channelId: w,
messageId: x,
roleId: G
  }), {
pastActivityEnabled: Y
  } = (0, m.z)({
location: 'BiteSizeProfilePopout'
  }), j = (0, a.e7)([f.Z], () => null != U ? f.Z.getGuild(U) : null), W = (0, a.e7)([E.ZP], () => null != U ? E.ZP.getMember(U, t.id) : null), K = i.useMemo(() => null != U ? {
[U]: [t.id]
  } : {}, [
U,
t.id
  ]);
  (0, l.$)(K);
  let {
profileStatusEditEnabled: z
  } = (0, p.K)({
location: 'BiteSizeProfilePopout'
  }), q = i.useRef(null), Q = (0, I.ZP)(t.id, U), X = (0, o.Z)(q);
  i.useEffect(() => {
null == B || B(null == q ? void 0 : q.current);
  }, [
q,
B
  ]);
  let [$, J] = i.useState(), [ee, et] = i.useState(), en = (e, t) => {
J(e), et(t);
  };
  i.useEffect(() => {
Y && (0, d.JX)(t.id);
  }, [
t,
Y
  ]);
  let er = e => {
null == k || k(), (0, y.openUserProfileModal)({
  sourceAnalyticsLocations: H,
  userId: t.id,
  guildId: U,
  channelId: w,
  messageId: x,
  roleId: G,
  ...e
});
  };
  return (0, r.jsx)(c.Gt, {
value: H,
children: (0, r.jsx)(h.Mt, {
  layout: 'BITE_SIZE_POPOUT',
  userId: t.id,
  guildId: U,
  channelId: w,
  messageId: x,
  roleId: G,
  shouldTrackViewOnMount: null == W || null != W.fullProfileLoadedTimestamp,
  children: (0, r.jsxs)(s.Dialog, {
    ref: q,
    'aria-label': t.username,
    children: [
      (0, r.jsxs)(N.Z, {
        user: t,
        displayProfile: Q,
        profileType: b.y0.BITE_SIZE,
        children: [
          (0, r.jsxs)(v.Z, {
            profileType: b.y0.BITE_SIZE,
            children: [
              (0, r.jsx)(R.Z, {
                user: t,
                guildId: U,
                channelId: w,
                onClose: k
              }),
              (0, r.jsx)(O.Z, {
                profileType: b.y0.BITE_SIZE,
                user: t
              }),
              t.id !== n.id && (0, r.jsx)(C.Z, {
                user: t,
                guildId: U,
                viewProfileItem: F ? null : (0, r.jsx)(s.MenuItem, {
                  id: 'view-profile',
                  label: M.Z.Messages.VIEW_FULL_PROFILE,
                  action: () => {
                    Z({
                      action: 'PRESS_VIEW_PROFILE',
                      analyticsLocations: H
                    }), er();
                  }
                })
              })
            ]
          }),
          (0, r.jsx)(s.Popout, {
            renderPopout: e => {
              let {
                setPopoutRef: n
              } = e;
              return null == $ ? (0, r.jsx)(r.Fragment, {}) : (0, r.jsx)(A.Z, {
                user: t,
                guildId: U,
                channelId: w,
                profileType: b.y0.BITE_SIZE,
                sourceDetails: ee,
                sourceType: $,
                setPopoutRef: n,
                onReply: en
              });
            },
            animationPosition: 'top',
            position: 'bottom',
            align: 'center',
            shouldShow: null != $,
            children: () => (0, r.jsxs)('header', {
              className: P.header,
              children: [
                (0, r.jsx)(T.Z, {
                  user: t,
                  displayProfile: Q,
                  guildId: U,
                  profileType: b.y0.BITE_SIZE,
                  hasProfileEffect: (null == Q ? void 0 : Q.profileEffectId) != null
                }),
                (0, r.jsx)(g.Z, {
                  location: 'BiteSizeProfilePopout',
                  user: t,
                  displayProfile: Q,
                  guildId: U,
                  channelId: w,
                  profileType: b.y0.BITE_SIZE,
                  onOpenProfile: F ? void 0 : er,
                  onReply: en
                }),
                (0, r.jsx)(S.Z, {
                  location: 'BiteSizeProfilePopout',
                  user: t,
                  guildId: U,
                  channelId: w,
                  profileType: b.y0.BITE_SIZE,
                  editEnabled: z,
                  onReply: en,
                  onClose: k
                })
              ]
            })
          }),
          (0, r.jsx)(D.Z, {
            user: t,
            currentUser: n,
            displayProfile: Q,
            guild: j,
            isHovering: X,
            onOpenProfile: F ? void 0 : er,
            channelId: w,
            onClose: k
          }),
          (0, r.jsx)(L.Z, {
            user: t,
            guildId: U,
            channelId: w,
            onClose: k
          })
        ]
      }),
      (null == Q ? void 0 : Q.profileEffectId) != null && (0, r.jsx)(_.Z, {
        profileEffectId: null == Q ? void 0 : Q.profileEffectId,
        isHovering: X
      })
    ]
  })
})
  });
}