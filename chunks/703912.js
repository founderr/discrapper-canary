...
async function A(e, t, n) {
    ...
    if (null == G ? void 0 : G.aborted) throw new C.Z({ errorCode: S.lTL.UNKNOWN_ERROR }, 'Request aborted');
    if (null == N) throw new C.Z({ errorCode: S.lTL.OAUTH2_ERROR }, 'No Client ID provided');
    if (null != v) throw new C.Z({ errorCode: S.lTL.OAUTH2_ERROR }, 'Redirect URI cannot be used in the RPC OAuth2 Authorization flow');
    let w = [];
    if (('string' == typeof x ? (w = x.split(' ').filter((e) => e.length > 0)) : Array.isArray(x) && (w = x), null == p.default.getCurrentUser())) throw new C.Z({ errorCode: S.lTL.OAUTH2_ERROR }, 'Client is not logged in');
        ...
        l.tn
            ...
            .then(
                (n) => {
                    ...
                    if (null == l || !a || l.id !== a.id) throw new C.Z({ errorCode: S.lTL.INVALID_TOKEN }, 'Token does not match current user');
                    return (
    ...
    return {
        [S.Etm.AUTHENTICATE]: (0, o.S)(S.Etm.AUTHENTICATE, {
            handler(n) {
                    ...
                    let o = s.x.IDENTIFY,
                        d = () =>
                            A(
                                ...
                                if (null != s.error) {
                                    var r;
                                    throw new C.Z({ errorCode: S.lTL.OAUTH2_ERROR }, 'OAuth2 Error: '.concat(s.error, ': ').concat(null !== (r = s.error_description) && void 0 !== r ? r : 'unknown error'));
                                }
                                return (
                                    !(function (e, t, n, i) {
                                        var a;
                ...
                if (null == l) throw new C.Z({ errorCode: S.lTL.INVALID_TOKEN }, 'No access token provided');
                return v(a, l);
            }
        }),
        [S.Etm.AUTHORIZE]: {
            handler(n) {
                ...
                if (null != a.authorization.accessToken) throw new C.Z({ errorCode: S.lTL.INVALID_COMMAND }, 'Already authenticated');
                if (a.authorization.authing) throw new C.Z({ errorCode: S.lTL.INVALID_COMMAND }, 'Already authing');
                return (
                    ...
                    l.tn
                        ...
                        .then(
                            (n) => {
                                let i = n.body;
                                if (a.application.id !== i.id) throw new C.Z({ errorCode: S.lTL.INVALID_CLIENTID }, "Application does not match the connection's");
                        ...
                        .then((e) => {
                            if (((a.authorization.authing = !1), null == e)) throw new C.Z({ errorCode: S.lTL.UNKNOWN_ERROR }, 'Unknown error occurred');
