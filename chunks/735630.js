        ...
        if (((e.samples = []), (e.samples_duration = 0), (e.samples_size = 0), (n = e.mdia.minf.stbl.stco || e.mdia.minf.stbl.co64), (r = e.mdia.minf.stbl.stsc), (i = e.mdia.minf.stbl.stsz || e.mdia.minf.stbl.stz2), (a = e.mdia.minf.stbl.stts), (s = e.mdia.minf.stbl.ctts), (o = e.mdia.minf.stbl.stss), (l = e.mdia.minf.stbl.stsd), (u = e.mdia.minf.stbl.subs), (f = e.mdia.minf.stbl.stdp), (c = e.mdia.minf.stbl.sbgps), (d = e.mdia.minf.stbl.sgpds), (v = -1), (b = -1), (I = -1), (T = -1), (S = 0), (y = 0), (A = 0), _.initSampleGroups(e, null, c, d), void 0 !== i)) {
            for (t = 0; t < i.sample_sizes.length; t++) {
                var t,
                    ...
                    A,
                    N = {};
                (N.number = t), (N.track_id = e.tkhd.track_id), (N.timescale = e.mdia.mdhd.timescale), (N.alreadyRead = 0), (e.samples[t] = N), (N.size = i.sample_sizes[t]), (e.samples_size += N.size), 0 === t ? ((h = 1), (p = 0), (N.chunk_index = h), (N.chunk_run_index = p), (E = r.samples_per_chunk[p]), (g = 0), (m = p + 1 < r.first_chunk.length ? r.first_chunk[p + 1] - 1 : 1 / 0)) : t < E ? ((N.chunk_index = h), (N.chunk_run_index = p)) : (h++, (N.chunk_index = h), (g = 0), h <= m || (m = ++p + 1 < r.first_chunk.length ? r.first_chunk[p + 1] - 1 : 1 / 0), (N.chunk_run_index = p), (E += r.samples_per_chunk[p])), (N.description_index = r.sample_description_index[N.chunk_run_index] - 1), (N.description = l.entries[N.description_index]), (N.offset = n.chunk_offsets[N.chunk_index - 1] + g), (g += N.size), t > v && (b++, v < 0 && (v = 0), (v += a.sample_counts[b])), t > 0 ? ((e.samples[t - 1].duration = a.sample_deltas[b]), (e.samples_duration += e.samples[t - 1].duration), (N.dts = e.samples[t - 1].dts + e.samples[t - 1].duration)) : (N.dts = 0), s ? (t >= I && (T++, I < 0 && (I = 0), (I += s.sample_counts[T])), (N.cts = e.samples[t].dts + s.sample_offsets[T])) : (N.cts = N.dts), o ? (t == o.sample_numbers[S] - 1 ? ((N.is_sync = !0), S++) : ((N.is_sync = !1), (N.degradation_priority = 0)), u && u.entries[y].sample_delta + A == t + 1 && ((N.subsamples = u.entries[y].subsamples), (A += u.entries[y].sample_delta), y++)) : (N.is_sync = !0), _.process_sdtp(e.mdia.minf.stbl.sdtp, N, N.number), f ? (N.degradation_priority = f.priority[t]) : (N.degradation_priority = 0), u && u.entries[y].sample_delta + A == t && ((N.subsamples = u.entries[y].subsamples), (A += u.entries[y].sample_delta)), (c.length > 0 || d.length > 0) && _.setSampleGroupProperties(e, N, t, e.sample_groups_info);
            }
                        ...
                        for (c = l.trafs[e], d = this.getTrackById(c.tfhd.track_id), f = this.getTrexById(c.tfhd.track_id), r = c.tfhd.flags & u.TFHD_FLAG_SAMPLE_DESC ? c.tfhd.default_sample_description_index : f ? f.default_sample_description_index : 1, i = c.tfhd.flags & u.TFHD_FLAG_SAMPLE_DUR ? c.tfhd.default_sample_duration : f ? f.default_sample_duration : 0, a = c.tfhd.flags & u.TFHD_FLAG_SAMPLE_SIZE ? c.tfhd.default_sample_size : f ? f.default_sample_size : 0, s = c.tfhd.flags & u.TFHD_FLAG_SAMPLE_FLAGS ? c.tfhd.default_sample_flags : f ? f.default_sample_flags : 0, c.sample_number = 0, c.sbgps.length > 0 && _.initSampleGroups(d, c, c.sbgps, d.mdia.minf.stbl.sgpds, c.sgpds), t = 0; t < c.truns.length; t++) {
                            ...
                            for (n = 0; n < m.sample_count; n++) {
                                ...
                                var g = !!(c.tfhd.flags & u.TFHD_FLAG_BASE_DATA_OFFSET),
                                    ...
                                    b = 0;
                                (b = g ? c.tfhd.base_data_offset : E ? l.start : 0 === t ? l.start : o), 0 === t && 0 === n ? (v ? (p.offset = b + m.data_offset) : (p.offset = b)) : (p.offset = o), (o = p.offset + p.size), (c.sbgps.length > 0 || c.sgpds.length > 0 || d.mdia.minf.stbl.sbgps.length > 0 || d.mdia.minf.stbl.sgpds.length > 0) && _.setSampleGroupProperties(d, p, p.number_in_traf, c.sample_groups_info);
                            }
                        }
                        if (c.subs) {
                            d.has_fragment_subsamples = !0;
                            var I = c.first_sample_index;
                            for (t = 0; t < c.subs.entries.length; t++) (I += c.subs.entries[t].sample_delta), ((p = d.samples[I - 1]).subsamples = c.subs.entries[t].subsamples);
                        }
                    }
        }
    }),
