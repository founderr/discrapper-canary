n.d(t, {
    Z: function () {
        return C;
    }
});
var i = n(735250);
n(470079);
var s = n(120356),
    a = n.n(s),
    r = n(399606),
    l = n(524437),
    o = n(481060),
    c = n(494404),
    u = n(377171),
    d = n(317632),
    _ = n(602478),
    E = n(768943),
    I = n(121254),
    m = n(742989),
    T = n(818186),
    N = n(689938),
    h = n(182987);
function C(e) {
    let { tab: t, setTab: n, children: s, badgeState: C, closePopout: f } = e,
        { showReminders: p } = m.Z.useExperiment({ location: 'RecentsHeader' }, { autoTrackExposure: !1 }),
        { enabled: g } = I.Z.useExperiment({ location: 'RecentsPopout' }, { autoTrackExposure: !1 }),
        { enabled: S } = _.Z.useExperiment({ location: 'RecentsHeader' }),
        A = (0, r.e7)([d.Z], () => d.Z.getUnseenInviteCount()),
        R = (0, r.e7)([E.Z], () => E.Z.getMessageReminders()).length;
    return (0, i.jsxs)(c.h4, {
        className: a()(h.header, h.expanded),
        children: [
            (0, i.jsxs)('div', {
                className: a()(h.expandedInboxHeader),
                children: [
                    (0, i.jsx)(o.InboxIcon, {
                        size: 'md',
                        color: 'currentColor',
                        className: h.inboxIcon
                    }),
                    (0, i.jsx)(o.Text, {
                        className: h.inboxTitle,
                        variant: 'text-lg/semibold',
                        children: N.Z.Messages.INBOX
                    }),
                    (0, i.jsx)(T.Z, { closePopout: f })
                ]
            }),
            (0, i.jsxs)('div', {
                className: a()(h.expandedTabSection),
                children: [
                    (0, i.jsxs)(o.TabBar, {
                        selectedItem: t,
                        type: 'top-pill',
                        onItemSelect: n,
                        children: [
                            (0, i.jsxs)(o.TabBar.Item, {
                                id: l.X.FOR_YOU,
                                'aria-label': N.Z.Messages.FOR_YOU,
                                className: a()(h.tab, h.expanded, { [h.active]: t === l.X.FOR_YOU }),
                                children: [
                                    N.Z.Messages.FOR_YOU,
                                    (null == C ? void 0 : C.badgeForYou)
                                        ? (0, i.jsx)(o.CircleBadge, {
                                              color: u.Z.STATUS_DANGER,
                                              className: a()(h.iconBadge)
                                          })
                                        : null
                                ]
                            }),
                            (0, i.jsx)(o.TabBar.Item, {
                                id: l.X.UNREADS,
                                className: a()(h.tab, h.expanded, { [h.active]: t === l.X.UNREADS }),
                                children: N.Z.Messages.UNREADS_TAB_LABEL
                            }),
                            (0, i.jsx)(o.TabBar.Item, {
                                'aria-label': N.Z.Messages.MENTIONS,
                                id: l.X.MENTIONS,
                                className: a()(h.tab, h.expanded, { [h.active]: t === l.X.MENTIONS }),
                                children: N.Z.Messages.MENTIONS
                            }),
                            S
                                ? (0, i.jsxs)(o.TabBar.Item, {
                                      'aria-label': 'game_invites',
                                      id: l.X.GAME_INVITES,
                                      className: a()(h.tab, h.expanded, { [h.active]: t === l.X.GAME_INVITES }),
                                      children: [
                                          N.Z.Messages.GAME_INVITES,
                                          A > 0
                                              ? (0, i.jsx)(o.CircleBadge, {
                                                    color: u.Z.STATUS_DANGER,
                                                    className: a()(h.iconBadge)
                                                })
                                              : null
                                      ]
                                  })
                                : null,
                            g
                                ? (0, i.jsx)(o.TabBar.Item, {
                                      'aria-label': 'bookmarks',
                                      id: l.X.BOOKMARKS,
                                      className: a()(h.tab, h.expanded, { [h.active]: t === l.X.BOOKMARKS }),
                                      children: N.Z.Messages.MESSAGE_BOOKMARKS_HEADER
                                  })
                                : null,
                            p
                                ? (0, i.jsxs)(o.TabBar.Item, {
                                      'aria-label': 'todos',
                                      id: l.X.TODOS,
                                      className: a()(h.tab, h.expanded, { [h.active]: t === l.X.TODOS }),
                                      children: [
                                          N.Z.Messages.MESSAGE_REMINDERS_HEADER,
                                          R > 0
                                              ? (0, i.jsx)(o.CircleBadge, {
                                                    color: u.Z.STATUS_DANGER,
                                                    className: a()(h.iconBadge)
                                                })
                                              : null
                                      ]
                                  })
                                : null
                        ]
                    }),
                    (0, i.jsx)('div', {
                        className: h.controls,
                        children: s
                    })
                ]
            })
        ]
    });
}
