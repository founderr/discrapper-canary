n.d(t, {
  Z: function() {
    return ek
  }
}), n(47120), n(653041);
var i = n(735250),
  s = n(470079),
  l = n(120356),
  a = n.n(l),
  r = n(442837),
  o = n(704215),
  c = n(846519),
  u = n(400354),
  d = n(652844),
  E = n(789639),
  h = n(481060),
  _ = n(355467),
  I = n(239091),
  m = n(230711),
  g = n(255952),
  p = n(410575),
  T = n(941129),
  N = n(83200),
  S = n(634894),
  C = n(586902),
  A = n(515550),
  f = n(1585),
  Z = n(304761),
  v = n(865427),
  L = n(571250),
  O = n(628581),
  R = n(55311),
  x = n(575175),
  M = n(64220),
  P = n(377171),
  D = n(676035),
  b = n(427217),
  y = n(605236),
  j = n(243778),
  U = n(730417),
  G = n(524347),
  w = n(853197),
  k = n(810280),
  B = n(60762),
  H = n(258609),
  V = n(278464),
  F = n(917112),
  Y = n(443669),
  z = n(332473),
  W = n(240504),
  K = n(804545),
  q = n(637906),
  Q = n(39370),
  X = n(19199),
  J = n(111653),
  $ = n(866483),
  ee = n(14357),
  et = n(393333),
  en = n(763296),
  ei = n(640806),
  es = n(484459),
  el = n(584427),
  ea = n(52538),
  er = n(331187),
  eo = n(879815),
  ec = n(294629),
  eu = n(314897),
  ed = n(592125),
  eE = n(819640),
  eh = n(131951),
  e_ = n(19780),
  eI = n(885110),
  em = n(246946),
  eg = n(594174),
  ep = n(78839),
  eT = n(285952),
  eN = n(546591),
  eS = n(976401),
  eC = n(333454),
  eA = n(864441),
  ef = n(626135),
  eZ = n(768581),
  ev = n(572004),
  eL = n(70956),
  eO = n(374023),
  eR = n(51144),
  ex = n(998502),
  eM = n(981631),
  eP = n(215023),
  eD = n(702512),
  eb = n(689938),
  ey = n(812118);

function ej(e, t, n) {
  return t in e ? Object.defineProperty(e, t, {
    value: n,
    enumerable: !0,
    configurable: !0,
    writable: !0
  }) : e[t] = n, e
}
let eU = ex.ZP.getEnableHardwareAcceleration() ? h.AnimatedAvatar : h.Avatar,
  eG = 30 * eL.Z.Millis.DAY;
class ew extends s.PureComponent {
  get isCopiedStreakGodlike() {
    let {
      copiedStreak: e
    } = this.state;
    return e >= 10
  }
  componentDidUpdate(e) {
    let {
      speakingWhileMuted: t,
      occluded: n
    } = e, {
      speakingWhileMuted: i,
      occluded: s
    } = this.props;
    s !== n && this.handleOccludedChanged(), i !== t && this.handleSpeakingWhileMutedChanged()
  }
  componentWillUnmount() {
    this.copiedTimeout.stop(), this.copiedDecayTimeout.stop(), this.speakingWhileMutedTooltipTimeout.stop()
  }
  renderCopiedSuccess() {
    let {
      copiedStreak: e
    } = this.state, t = [eb.Z.Messages.COPY_SUCCESS_1, eb.Z.Messages.COPY_SUCCESS_2, eb.Z.Messages.COPY_SUCCESS_3, eb.Z.Messages.COPY_SUCCESS_4, eb.Z.Messages.COPY_SUCCESS_5, eb.Z.Messages.COPY_SUCCESS_6, eb.Z.Messages.COPY_SUCCESS_7, eb.Z.Messages.COPY_SUCCESS_8, eb.Z.Messages.COPY_SUCCESS_9, eb.Z.Messages.COPY_SUCCESS_10, eb.Z.Messages.COPY_SUCCESS_11], n = Math.min(Math.max(0, e - 1), t.length - 1);
    return (0, i.jsx)(h.Shaker, {
      isShaking: this.isCopiedStreakGodlike,
      children: t[n]
    })
  }
  renderNameTag() {
    let e;
    let {
      hovered: t
    } = this.state, {
      customStatusActivity: n,
      userTag: s,
      currentUser: l,
      status: a
    } = this.props;
    if (null == l) return null;
    let r = eR.ZP.getName(l),
      o = null != a && a !== eM.Skl.UNKNOWN,
      c = null != n;
    return (c || o && l.isPomelo()) && (e = (0, i.jsx)(eN.Z, {
      hoverText: s,
      forceHover: t,
      children: c ? (0, i.jsx)(b.Z, {
        activity: n,
        emojiClassName: ey.emoji,
        className: ey.customStatus
      }) : eR.ZP.humanizeStatus(a)
    })), (0, i.jsxs)(i.Fragment, {
      children: [(0, i.jsx)("div", {
        className: ey.panelTitleContainer,
        children: (0, i.jsx)(eA.Z, {
          children: r
        })
      }), (0, i.jsx)("div", {
        className: ey.panelSubtextContainer,
        children: (0, i.jsx)(eC.Z, {
          children: null != e ? e : s
        })
      })]
    })
  }
  renderCopiableNameTag() {
    let {
      currentUser: e
    } = this.props, {
      shouldShowNametagTooltip: t,
      shouldShowCopiedFeedback: n
    } = this.state;
    if (null == e) return null;
    let s = null,
      l = h.TooltipColors.PRIMARY;
    return ev.wS && (s = eb.Z.Messages.ACCOUNT_CLICK_TO_COPY, n && (s = this.renderCopiedSuccess(), l = this.isCopiedStreakGodlike ? h.TooltipColors.RED : h.TooltipColors.GREEN)), (0, i.jsx)(h.Tooltip, {
      text: s,
      forceOpen: t,
      "aria-label": !!ev.wS && eb.Z.Messages.ACCOUNT_CLICK_TO_COPY,
      color: l,
      delay: 500,
      onAnimationRest: (e, i) => {
        n && !t && i.phase === eM.UkZ.LEAVE && this.setState({
          shouldShowCopiedFeedback: !1
        })
      },
      children: e => {
        let {
          onClick: t,
          onMouseEnter: s,
          ...l
        } = e;
        return (0, i.jsx)(h.Clickable, {
          ...l,
          onMouseEnter: () => {
            n ? (this.copiedTimeout.stop(), this.setState({
              shouldShowNametagTooltip: !1
            }, s)) : null == s || s()
          },
          onClick: () => {
            null == t || t(), this.handleCopyTag()
          },
          className: a()(ey.nameTag, {
            [ey.canCopy]: ev.wS
          }),
          children: this.renderNameTag()
        })
      }
    })
  }
  renderAvatarWithPopout() {
    let {
      speaking: e,
      streaming: t,
      currentUser: n,
      status: s,
      showTagAsButton: l,
      showTaglessAccountPanel: r
    } = this.props;
    if (null == n) return null;
    let o = (0, eZ.NZ)({
      avatarDecoration: n.avatarDecoration,
      size: (0, f.y9)(h.AvatarSizes.SIZE_32)
    });
    return (0, i.jsx)(p.Z, {
      object: eM.qAy.AVATAR,
      children: (0, i.jsx)(h.Popout, {
        renderPopout: this.renderStatusPickerPopout,
        position: "top",
        align: "left",
        animation: h.Popout.Animation.FADE,
        spacing: 14,
        fixed: !0,
        preload: () => (0, es.Z)(n.id, null == n ? void 0 : n.getAvatarURL(null, 80)),
        children: (c, u) => (0, i.jsxs)(h.Clickable, {
          ...c,
          "aria-label": eb.Z.Messages.SET_STATUS,
          className: a()(ey.avatarWrapper, {
            [ey.withTagAsButton]: l,
            [ey.withTagless]: r
          }),
          children: [(0, i.jsx)(eU, {
            size: h.AvatarSizes.SIZE_32,
            src: n.getAvatarURL(void 0, 32, !1),
            avatarDecoration: o,
            "aria-label": n.username,
            status: t ? eM.Skl.STREAMING : s,
            isSpeaking: e,
            className: ey.avatar
          }), l || r ? (0, i.jsx)("div", {
            className: a()(ey.nameTag, {
              [ey.canCopy]: ev.wS
            }),
            children: r ? (0, i.jsx)(eS.Z, {
              tooltipText: null,
              icon: u.isShown ? h.ChevronSmallDownIcon : h.ChevronSmallUpIcon,
              role: "switch",
              "aria-label": eb.Z.Messages.SET_STATUS
            }) : this.renderNameTag()
          }) : null]
        })
      })
    })
  }
  renderNameZone() {
    let {
      currentUser: e,
      dismissibleContents: t
    } = this.props, {
      showTagAsButton: n,
      showTaglessAccountPanel: s
    } = this.props;
    return null == e ? null : (0, i.jsxs)(i.Fragment, {
      children: [(0, i.jsx)(j.Z, {
        contentTypes: t.avatar,
        children: e => {
          let {
            visibleContent: t,
            markAsDismissed: n
          } = e;
          if (t === o.z.PREMIUM_SUBSCRIPTION_APRIL_MARKETING_DECO_TOOLTIP) return (0, i.jsx)(M.Z, {
            markAsDismissed: n,
            children: () => this.renderAvatarWithPopout()
          });
          return this.renderAvatarWithPopout()
        }
      }), n || s ? null : this.renderCopiableNameTag()]
    })
  }
  render() {
    let {
      currentUser: e,
      selfDeaf: t,
      selfMute: n,
      awaitingRemote: s,
      serverMute: l,
      serverDeaf: a,
      suppress: r,
      dismissibleContents: c
    } = this.props, {
      shouldShowSpeakingWhileMutedTooltip: u
    } = this.state;
    return null == e ? null : (0, i.jsxs)("div", {
      className: ey.container,
      onMouseEnter: this.handleMouseEnter,
      onMouseLeave: this.handleMouseLeave,
      children: [this.renderNameZone(), (0, i.jsxs)(eT.Z, {
        grow: 0,
        children: [(0, i.jsx)(eH, {
          selfMute: n,
          serverMute: l,
          suppress: r,
          awaitingRemote: s,
          tooltipText: u ? eb.Z.Messages.ACCOUNT_SPEAKING_WHILE_MUTED : (0, O.Z)(n, l, r, s),
          tooltipColor: u ? h.TooltipColors.GREEN : void 0,
          tooltipForceOpen: u || void 0,
          onMouseEnter: this.handleMouseEnterMute,
          onMouseLeave: this.handleMouseLeaveMute,
          onClick: this.handleToggleSelfMute,
          onContextMenu: this.handleInputAudioContextMenu
        }), (0, i.jsx)(eV, {
          selfDeaf: t,
          serverDeaf: a,
          onClick: this.handleToggleSelfDeaf,
          onContextMenu: this.handleOutputAudioContextMenu,
          awaitingRemote: s
        }), (0, i.jsx)(j.Z, {
          contentTypes: c.settings,
          children: e => {
            let {
              visibleContent: t,
              markAsDismissed: n
            } = e;
            switch (t) {
              case o.z.PROFILE_THEMES_FEATURE_EDUCATION_TOOLTIP_TAKE_2:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(Q.Z, {}),
                  children: this.renderSettingsGear
                });
              case o.z.QUEST_2_ENROLLMENT_TOOLTIP:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(k.Z, {
                    markAsDismissed: n,
                    partnerGame: eD.X2.FORTNITE
                  }),
                  children: this.renderSettingsGear
                });
              case o.z.QUEST_2_COMPLETION_TOOLTIP:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(B.Z, {
                    markAsDismissed: n,
                    partnerGame: eD.X2.FORTNITE
                  }),
                  children: this.renderSettingsGear
                });
              case o.z.USER_SAFETY_SAFE_DIRECT_MESSAGING:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(er.Z, {
                    markAsDismissed: n
                  }),
                  children: this.renderSettingsGear
                });
              case o.z.USER_SAFETY_CONSUMER_EDUCATION_BLOCK:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)($.Z, {
                    markAsDismissed: n
                  }),
                  children: this.renderSettingsGear
                });
              case o.z.USER_SAFETY_CONSUMER_EDUCATION_MUTE:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(ee.Z, {
                    markAsDismissed: n
                  }),
                  children: this.renderSettingsGear
                });
              case o.z.SOUNDBOARD_VOLUME_EDUCATION:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(ei.Z, {
                    markAsDismissed: n
                  }),
                  children: this.renderSettingsGear
                });
              case o.z.USER_SAFETY_CONSUMER_EDUCATION_REPORTING:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(et.P, {
                    markAsDismissed: n
                  }),
                  children: this.renderSettingsGear
                });
              case o.z.PER_GUILD_COLLECTIBLES_CUSTOMIZATION_COACHMARK:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(ea.Z, {}),
                  children: this.renderSettingsGear
                });
              case o.z.PRIDE_ICONS_COACHMARK:
                return (0, i.jsx)(h.Popout, {
                  position: "top",
                  align: "center",
                  shouldShow: !0,
                  renderPopout: () => (0, i.jsx)(A.Z, {
                    markAsDismissed: n
                  }),
                  children: this.renderSettingsGear
                });
              default:
                return this.renderSettingsGear()
            }
          }
        })]
      })]
    })
  }
  constructor(...e) {
    super(...e), ej(this, "copiedTimeout", new c.V7), ej(this, "copiedDecayTimeout", new c.V7), ej(this, "speakingWhileMutedTooltipTimeout", new c.V7), ej(this, "state", {
      hovered: !1,
      copiedStreak: 0,
      shouldShowNametagTooltip: !1,
      shouldShowCopiedFeedback: !1,
      shouldShowSpeakingWhileMutedTooltip: !1,
      hoveringOnMute: !1
    }), ej(this, "handleToggleSelfMute", () => {
      let {
        serverMute: e,
        suppress: t
      } = this.props;
      (0, x.Z)(e, t)
    }), ej(this, "handleToggleSelfDeaf", () => {
      let {
        serverDeaf: e
      } = this.props;
      (0, R.Z)(e)
    }), ej(this, "handleOpenAccountSettings", () => {
      this.handleOpenSettings()
    }), ej(this, "handleOpenSettings", function() {
      let e = arguments.length > 0 && void 0 !== arguments[0] ? arguments[0] : eM.oAB.ACCOUNT,
        t = arguments.length > 1 ? arguments[1] : void 0,
        n = arguments.length > 2 ? arguments[2] : void 0;
      m.Z.open(e, t, n)
    }), ej(this, "handleOpenSettingsContextMenu", e => {
      let {
        currentUser: t
      } = this.props;
      null != t && (0, I.jW)(e, async () => {
        let {
          default: e
        } = await Promise.all([n.e("49237"), n.e("99387"), n.e("96427"), n.e("77298"), n.e("23357"), n.e("19503"), n.e("15972"), n.e("12013"), n.e("6416"), n.e("43906"), n.e("32776"), n.e("95900"), n.e("33053"), n.e("8016"), n.e("45397"), n.e("68136"), n.e("76540"), n.e("4934"), n.e("22646"), n.e("87624"), n.e("5528"), n.e("44517"), n.e("43331"), n.e("30419"), n.e("18824"), n.e("31649"), n.e("2535")]).then(n.bind(n, 923422));
        return n => (0, i.jsx)(e, {
          ...n,
          user: t,
          premiumSubscription: this.props.premiumSubscription,
          webBuildOverride: this.props.webBuildOverride
        })
      })
    }), ej(this, "handleInputAudioContextMenu", e => {
      (0, I.jW)(e, async () => {
        let {
          default: e
        } = await Promise.resolve().then(n.bind(n, 659580));
        return () => (0, i.jsx)(e, {
          onClose: I.Zy,
          renderInputDevices: !0,
          renderInputModes: !0,
          renderInputVolume: !0
        })
      })
    }), ej(this, "handleOutputAudioContextMenu", e => {
      (0, I.jW)(e, async () => {
        let {
          default: e
        } = await Promise.resolve().then(n.bind(n, 659580));
        return () => (0, i.jsx)(e, {
          onClose: I.Zy,
          renderOutputDevices: !0,
          renderOutputVolume: !0
        })
      })
    }), ej(this, "handleMouseEnter", () => {
      this.setState({
        hovered: !0
      })
    }), ej(this, "handleMouseLeave", () => {
      this.setState({
        hovered: !1
      })
    }), ej(this, "handleMouseEnterMute", () => {
      this.setState({
        hoveringOnMute: !0,
        shouldShowSpeakingWhileMutedTooltip: !1
      }), this.speakingWhileMutedTooltipTimeout.stop()
    }), ej(this, "handleMouseLeaveMute", () => {
      this.setState({
        hoveringOnMute: !1
      })
    }), ej(this, "handleCopyTag", () => {
      let {
        currentUser: e
      } = this.props, {
        shouldShowNametagTooltip: t,
        copiedStreak: n
      } = this.state;
      if (null == e) return;
      (0, ev.JG)(eR.ZP.getUserTag(e, {
        decoration: "never",
        identifiable: "always"
      })), ef.default.track(eM.rMx.TEXT_COPIED, {
        type: "User Tag"
      });
      let i = {
        shouldShowCopiedFeedback: !0,
        shouldShowNametagTooltip: !0
      };
      !t && (i.copiedStreak = n + 1), this.setState(i, () => {
        this.copiedTimeout.start(1e3, () => this.setState({
          shouldShowNametagTooltip: !1
        })), this.copiedDecayTimeout.start(2e3, () => this.setState({
          copiedStreak: 0
        }))
      })
    }), ej(this, "handleOccludedChanged", () => {
      let {
        occluded: e
      } = this.props;
      e && this.setState({
        shouldShowNametagTooltip: !1,
        shouldShowSpeakingWhileMutedTooltip: !1
      })
    }), ej(this, "handleSpeakingWhileMutedChanged", () => {
      let {
        selfMute: e,
        serverMute: t,
        suppress: n,
        speakingWhileMuted: i,
        occluded: s
      } = this.props, {
        hoveringOnMute: l
      } = this.state;
      i ? !l && e && !t && !n && !s && this.setState({
        shouldShowSpeakingWhileMutedTooltip: !0
      }, () => {
        this.speakingWhileMutedTooltipTimeout.start(2e3, () => this.setState({
          shouldShowSpeakingWhileMutedTooltip: !1
        }))
      }) : (this.setState({
        shouldShowSpeakingWhileMutedTooltip: !1
      }), this.speakingWhileMutedTooltipTimeout.stop())
    }), ej(this, "renderStatusPickerPopout", e => {
      let {
        closePopout: t
      } = e, {
        showRichProfilePopup: n,
        currentUser: s
      } = this.props;
      if (n) {
        let e = {
          userId: null == s ? void 0 : s.id,
          user: s,
          onClose: () => {
            this.handleMouseLeave(), null == t || t()
          }
        };
        return (0, i.jsx)("div", {
          className: ey.accountProfilePopoutWrapper,
          children: (0, i.jsx)(Y.Z, {
            location: "StatusPickerPopout",
            ...e
          })
        })
      }
      return (0, i.jsx)(g.Z, {
        onClose: () => {
          this.handleMouseLeave(), null == t || t()
        }
      })
    }), ej(this, "renderSettingsGear", () => (0, i.jsx)(eB, {
      isEligibleForPomelo: this.props.isEligibleForPomelo,
      webBuildOverride: this.props.webBuildOverride,
      onClick: this.handleOpenAccountSettings,
      onContextMenu: this.handleOpenSettingsContextMenu
    }))
  }
}

function ek() {
  var e, t;
  let n = (0, r.e7)([eg.default], () => eg.default.getCurrentUser()),
    l = (0, U._g)(eD.X2.FORTNITE),
    a = (0, r.e7)([eu.default], () => eu.default.getId()),
    c = (0, D.a)(),
    {
      streaming: u,
      status: d
    } = (0, r.cj)([eI.Z], () => ({
      streaming: null != eI.Z.findActivity(e => e.type === eM.IIU.STREAMING),
      status: eI.Z.getStatus()
    })),
    E = (0, C.Z)({
      userId: a
    }),
    I = eR.ZP.useUserTag(n, {
      decoration: "never"
    }),
    m = (0, r.e7)([em.Z], () => em.Z.hidePersonalInformation),
    g = (0, r.e7)([e_.Z, ed.Z], () => {
      let e = e_.Z.getChannelId();
      return null != e ? ed.Z.getChannel(e) : null
    }),
    {
      mute: p,
      selfMute: A,
      suppress: f
    } = (0, ec.Z)(g),
    {
      selfDeaf: L,
      deaf: O
    } = (0, eo.Z)(g),
    R = (0, r.e7)([Z.C], () => {
      var e;
      return (0, v.fD)() ? null === (e = Z.C.getCurrentBuildOverride().overrides) || void 0 === e ? void 0 : e.discord_web : null
    }),
    x = (0, r.e7)([ep.ZP], () => ep.ZP.getPremiumTypeSubscription()),
    M = (0, r.e7)([eh.Z], () => eh.Z.getEverSpeakingWhileMuted()),
    P = (0, r.e7)([eE.Z], () => eE.Z.hasLayers()),
    b = (0, h.useModalsStore)(h.hasAnyModalOpenSelector) || P || eO.s.isDisallowPopupsSet(),
    j = (0, z.b)();
  s.useEffect(() => {
    F.Z.trackExposure({
      location: "08bd40_3"
    })
  });
  let k = (0, r.e7)([H.Z], () => null != H.Z.getAwaitingRemoteSessionInfo()),
    {
      isPopupEnabled: B,
      isJoinedTagButtonEnabled: Y,
      isTaglessAccountPanelEnabled: Q
    } = F.Z.useExperiment({
      location: "08bd40_4"
    }, {
      autoTrackExposure: !1
    }),
    $ = (0, r.e7)([G.Z], () => G.Z.isCurrentQuestCompleted),
    ee = (0, r.e7)([G.Z], () => G.Z.currentDropQuestGameTitle),
    et = (0, V.n)(),
    ei = (0, N.O)(),
    es = (0, X.D)(),
    ea = (0, T.u)(),
    er = (0, J.t)(),
    eT = (0, el.Z)(n),
    eN = (null == n ? void 0 : null === (e = n.avatarDecoration) || void 0 === e ? void 0 : e.skuId) === eP.rL,
    eS = "account";
  (0, S.j)({
    location: eS + " auto on",
    autoTrackExposure: !0
  }), (0, S.j)({
    location: eS + " auto off",
    autoTrackExposure: !1
  }), (0, X.o)();
  let eC = (0, r.e7)([en.Z], () => en.Z.hasHadOtherUserPlaySoundInSession()),
    eA = function() {
      let e = (0, y.wE)(o.z.PROFILE_THEMES_FEATURE_EDUCATION_TOOLTIP_TAKE_2),
        [t, n] = (0, r.Wu)([W.Z], () => [W.Z.hasFetchedRelevance, W.Z.profileThemesRelevanceExceeded]),
        [i, l] = (0, r.Wu)([ep.ZP], () => [ep.ZP.hasFetchedMostRecentPremiumTypeSubscription(), ep.ZP.getMostRecentPremiumTypeSubscription()]),
        {
          enabled: a
        } = q.Z.useExperiment({
          location: "08bd40_1"
        }, {
          autoTrackExposure: !0
        }),
        c = s.useMemo(() => {
          let e = new Date().getTime();
          return null != l && (l.status === eM.O0b.ACTIVE || e - l.currentPeriodEnd.getTime() < eG)
        }, [l]);
      return s.useEffect(() => {
        (async () => {
          if (e || null != n || !a) return;
          if (!i) {
            await (0, _.ou)();
            return
          }
          if (!c) !t && await (0, _.l0)()
        })()
      }, [a, n, e, i, t, c]), !e && !!a && !c && null != n && n
    }(),
    {
      enabled: ef
    } = (0, K._)({
      location: "AccountConnected"
    }),
    eZ = {
      avatar: [],
      settings: []
    };
  return !b && (eA && eZ.settings.push(o.z.PROFILE_THEMES_FEATURE_EDUCATION_TOOLTIP_TAKE_2), l && (eZ.settings.push(o.z.QUEST_2_ENROLLMENT_TOOLTIP), $ && ee === (null === (t = (0, w.BS)(eD.X2.FORTNITE)) || void 0 === t ? void 0 : t.title) && eZ.settings.push(o.z.QUEST_2_COMPLETION_TOOLTIP)), et && ei && eZ.settings.push(o.z.USER_SAFETY_SAFE_DIRECT_MESSAGING), es && (1 === ea ? eZ.settings.push(o.z.USER_SAFETY_CONSUMER_EDUCATION_MUTE) : eZ.settings.push(o.z.USER_SAFETY_CONSUMER_EDUCATION_BLOCK)), eC && eZ.settings.push(o.z.SOUNDBOARD_VOLUME_EDUCATION), er && eZ.settings.push(o.z.USER_SAFETY_CONSUMER_EDUCATION_REPORTING), eT && eZ.settings.push(o.z.PER_GUILD_COLLECTIBLES_CUSTOMIZATION_COACHMARK), ef && eZ.settings.push(o.z.PRIDE_ICONS_COACHMARK), eN && eZ.avatar.push(o.z.PREMIUM_SUBSCRIPTION_APRIL_MARKETING_DECO_TOOLTIP)), (0, i.jsx)(ew, {
    currentUser: n,
    customStatusActivity: c,
    dismissibleContents: eZ,
    userTag: I,
    hidePrivateData: m,
    occluded: b,
    premiumSubscription: x,
    selfDeaf: L,
    selfMute: A,
    serverDeaf: O,
    serverMute: p,
    showRichProfilePopup: B,
    showTagAsButton: Y,
    showTaglessAccountPanel: Q,
    speaking: E,
    speakingWhileMuted: M,
    status: d,
    streaming: u,
    suppress: f,
    webBuildOverride: R,
    awaitingRemote: k,
    isEligibleForPomelo: j
  })
}

function eB(e) {
  let {
    webBuildOverride: t,
    isEligibleForPomelo: n,
    onClick: s,
    onContextMenu: l
  } = e, a = (0, h.useRedesignIconContext)().enabled, r = null, o = h.SettingsIcon, c = (0, E.i)();
  return a ? o = null != t ? h.SettingsInfoIcon : n ? h.SettingsCircleIcon : c.Component : null != t ? r = (0, i.jsx)(h.CircleInformationIcon, {
    size: "custom",
    color: "currentColor",
    width: 9,
    height: 9,
    className: ey.hasBuildOverride,
    "aria-hidden": !0
  }) : n && (r = (0, i.jsx)(h.CircleBadge, {
    color: P.Z.STATUS_DANGER,
    className: ey.hasBuildOverride,
    "aria-hidden": !0
  })), (0, i.jsx)(eS.Z, {
    tooltipText: null != t ? eb.Z.Messages.USER_SETTINGS_WITH_BUILD_OVERRIDE.format({
      webBuildOverride: t.id
    }) : eb.Z.Messages.USER_SETTINGS,
    onClick: s,
    onContextMenu: l,
    innerClassName: null != t || n ? ey.buildOverrideButton : null,
    icon: o,
    ...c.events,
    children: r
  })
}

function eH(e) {
  let {
    selfMute: t,
    serverMute: n,
    suppress: l,
    awaitingRemote: a,
    tooltipText: r,
    tooltipColor: o,
    tooltipForceOpen: c,
    onMouseEnter: u,
    onMouseLeave: E,
    onClick: _,
    onContextMenu: I
  } = e, m = t || l || n, {
    Component: g,
    play: p,
    events: T
  } = (0, d.O)(m ? "unmute" : "mute"), N = n || l ? h.MicrophoneDenyIcon : g;
  return s.useEffect(() => () => p(), [m, p]), (0, i.jsx)(eS.Z, {
    tooltipText: r,
    tooltipColor: o,
    tooltipForceOpen: c,
    onMouseEnter: () => {
      u(), T.onMouseEnter()
    },
    onMouseLeave: () => {
      E(), T.onMouseLeave()
    },
    icon: (0, i.jsx)(N, {
      size: "custom",
      width: 20,
      height: 20,
      color: m ? h.tokens.colors.STATUS_DANGER : "currentColor"
    }),
    iconForeground: m ? ey.strikethrough : null,
    onClick: _,
    onContextMenu: I,
    role: "switch",
    "aria-label": eb.Z.Messages.MUTE,
    "aria-checked": m,
    disabled: a
  })
}

function eV(e) {
  let {
    selfDeaf: t,
    serverDeaf: n,
    awaitingRemote: l,
    onClick: r,
    onContextMenu: o
  } = e, c = t || n, {
    Component: d,
    play: E,
    events: {
      onMouseEnter: _,
      onMouseLeave: I
    }
  } = (0, u.l)(c ? "undeafen" : "deafen"), m = n ? h.HeadphonesDenyIcon : d;
  return s.useEffect(() => () => E(), [c, E]), (0, i.jsx)(eS.Z, {
    tooltipText: (0, L.Z)(t, n, l),
    onMouseEnter: _,
    onMouseLeave: I,
    icon: (0, i.jsx)(m, {
      size: "custom",
      width: 20,
      height: 20,
      color: c ? h.tokens.colors.STATUS_DANGER : "currentColor"
    }),
    onClick: r,
    onContextMenu: o,
    innerClassName: a()({
      [ey.redIcon]: n
    }),
    iconForeground: c ? ey.strikethrough : null,
    role: "switch",
    "aria-label": eb.Z.Messages.DEAFEN,
    "aria-checked": c,
    disabled: l
  })
}