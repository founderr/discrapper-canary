"use strict";
l.r(t), l.d(t, {
  ChannelSelectorFooter: function() {
    return O
  },
  default: function() {
    return V
  }
}), l("47120");
var i = l("735250"),
  a = l("470079"),
  n = l("442837"),
  s = l("481060"),
  r = l("153867"),
  d = l("607070"),
  o = l("100527"),
  c = l("906732"),
  u = l("740492"),
  v = l("592125"),
  f = l("430824"),
  I = l("496675"),
  E = l("944486"),
  m = l("368874"),
  h = l("810090"),
  T = l("566620"),
  x = l("317381"),
  C = l("146936"),
  S = l("678173"),
  p = l("696068"),
  A = l("361213"),
  _ = l("952561"),
  g = l("778569"),
  N = l("513202"),
  j = l("412019"),
  D = l("981631"),
  y = l("689938"),
  L = l("777874");

function M(e) {
  return (0, n.useStateFromStores)([f.default, I.default], () => {
    let t = f.default.getGuild(e);
    return null != t && I.default.can(D.Permissions.CREATE_INSTANT_INVITE, t)
  }, [e])
}
let b = ["embedded_background"];

function V(e) {
  var t, l;
  let {
    applicationId: o,
    guildId: c,
    selectedChannelId: I,
    setSelectedChannelId: x,
    enableSelectedTextChannelInvite: C
  } = e, _ = (0, n.useStateFromStores)([d.default], () => d.default.useReducedMotion), N = (0, n.useStateFromStores)([v.default], () => v.default.getChannel(I)), D = (0, S.useActivityShelfItemData)(null != c ? c : null, null != o ? o : "", N), [V, O] = a.useState(C && !u.default.disableInviteWithTextChannelActivityLaunch), R = (0, p.useSelectVoiceChannelOptions)(c);
  a.useEffect(() => {
    var e;
    T.fetchShelf({
      guildId: c
    }), x(null !== (e = (0, p.getActivityVoiceChannelSuggestion)({
      guildId: c
    })) && void 0 !== e ? e : void 0)
  }, [c, x]);
  let B = null == D ? void 0 : D.activity.activity_preview_video_asset_id,
    F = null != B ? (0, A.default)(null !== (t = null == D ? void 0 : D.application.id) && void 0 !== t ? t : "", B) : null,
    {
      url: H
    } = (0, g.default)({
      applicationId: null !== (l = null == D ? void 0 : D.application.id) && void 0 !== l ? l : "",
      size: 1024,
      names: b
    }),
    Y = (0, n.useStateFromStores)([f.default], () => f.default.getGuild(c)),
    k = (0, n.useStateFromStores)([v.default, E.default], () => v.default.getChannel(E.default.getChannelId())),
    P = M(null != c ? c : void 0);
  if ((null == D ? void 0 : D.application) == null || null == Y) return null;
  let w = (null == k ? void 0 : k.name) == null || (null == k ? void 0 : k.name) === "" ? y.default.Messages.EMBEDDED_ACTIVITIES_FALLBACK_CHANNEL_NAME : "#".concat(k.name);
  return (0, i.jsxs)(s.Scroller, {
    className: L.scroll,
    children: [null != F || null != H ? (0, i.jsx)(m.default, {
      aspectRatio: 16 / 9,
      className: L.imageContainer,
      children: (0, i.jsx)(h.default, {
        className: L.video,
        src: F,
        loop: !0,
        autoPlay: !_,
        poster: H,
        muted: !0
      })
    }) : null, (0, i.jsxs)("div", {
      className: L.descriptionArea,
      children: [(0, i.jsx)(s.Heading, {
        variant: "heading-xl/semibold",
        children: null == D ? void 0 : D.application.name
      }), (0, i.jsx)(s.Text, {
        className: L.descriptionAreaSubtext,
        variant: "text-md/normal",
        children: null == D ? void 0 : D.application.description
      })]
    }), (0, i.jsx)(s.Select, {
      placeholder: y.default.Messages.EMBEDDED_ACTIVITIES_SELECT_VOICE_CHANNEL,
      optionClassName: L.__invalid_option,
      options: R,
      isSelected: e => {
        let {
          channel: t
        } = e;
        return t.id === I
      },
      select: e => {
        let {
          channel: t
        } = e;
        return x(t.id)
      },
      serialize: e => {
        let {
          channel: t
        } = e;
        return t.id
      },
      renderOptionValue: () => {
        let e = R.find(e => e.value.channel.id === I);
        return null == e ? null : (0, i.jsx)(j.SelectChannelOption, {
          channel: e.value.channel,
          users: e.value.users
        })
      },
      renderOptionLabel: e => {
        let {
          value: {
            channel: t,
            users: l
          }
        } = e;
        return (0, i.jsx)(j.SelectChannelOption, {
          channel: t,
          users: l
        })
      }
    }), P && C ? (0, i.jsxs)(s.Clickable, {
      className: L.checkboxContainer,
      onClick: () => {
        let e = !V;
        r.default.updatedUnsyncedSettings({
          disableInviteWithTextChannelActivityLaunch: !e
        }), O(e)
      },
      children: [(0, i.jsx)(s.Checkbox, {
        type: s.Checkbox.Types.INVERTED,
        className: L.checkbox,
        value: V,
        displayOnly: !0
      }), (0, i.jsxs)(s.Text, {
        variant: "text-sm/normal",
        children: [y.default.Messages.EMBEDDED_ACTIVITIES_SEND_ACTIVITY_INVITE, " ", (0, i.jsx)("strong", {
          children: w
        })]
      })]
    }) : null]
  })
}

function O(e) {
  let {
    applicationId: t,
    guildId: l,
    locationObject: a,
    onBack: r,
    onClose: d,
    selectedChannelId: f,
    enableSelectedTextChannelInvite: I
  } = e, {
    analyticsLocations: m
  } = (0, c.default)(o.default.ACTIVITY_CHANNEL_SELECTOR), h = (0, n.useStateFromStores)([v.default], () => v.default.getChannel(f)), p = (0, S.useActivityShelfItemData)(null != l ? l : null, null != t ? t : null, h), A = (0, _.default)(), g = (0, n.useStateFromStores)([v.default, E.default], () => v.default.getChannel(E.default.getChannelId())), j = (0, n.useStateFromStores)([u.default], () => I && !u.default.disableInviteWithTextChannelActivityLaunch), b = (0, n.useStateFromStores)([x.default], () => null != f && "" !== f && x.default.getEmbeddedActivitiesForChannel(f).some(e => e.applicationId === t)), V = M(l), O = async () => {
    var e;
    if (null != f && "" !== f && null != p && null != l && "" !== l) await (0, C.default)({
      targetApplicationId: null === (e = p.application) || void 0 === e ? void 0 : e.id,
      currentEmbeddedApplication: A,
      locationObject: a,
      channelId: f,
      guildId: l,
      embeddedActivitiesManager: N.default,
      analyticsLocations: m
    }) && (d(), null != g && V && j && await T.sendEmbeddedActivityInvite({
      activityChannelId: f,
      invitedChannelId: g.id,
      applicationId: p.application.id,
      location: D.AnalyticsLocations.ACTIVITY_SHELF
    }))
  };
  return (0, i.jsx)(c.AnalyticsLocationProvider, {
    value: m,
    children: (0, i.jsxs)("div", {
      className: L.footerContainer,
      children: [(0, i.jsx)(s.Clickable, {
        onClick: r,
        className: L.backToBrowse,
        children: (0, i.jsx)(s.Text, {
          color: "header-secondary",
          variant: "text-md/normal",
          children: y.default.Messages.BACK
        })
      }), null == f ? (0, i.jsx)("div", {}) : (0, i.jsx)(s.Button, {
        className: L.launchButton,
        onClick: O,
        color: b ? s.Button.Colors.GREEN : void 0,
        fullWidth: !0,
        children: b ? y.default.Messages.EMBEDDED_ACTIVITIES_JOIN : y.default.Messages.EMBEDDED_ACTIVITIES_LAUNCH
      })]
    })
  })
}