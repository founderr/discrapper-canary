...
function M(e) {
  let {
    ...
    setSlide: a,
    onClose: o
  } = e, [u, m] = l.useState(!1), [x, j] = l.useState(null), {
    challenge: M
  } = s.methods.find(e => "webauthn" === e.type), A = async () => {
    m(!0), j(null);
    let e = d.isPlatformEmbedded && h.ZP.supportsFeature(f.eRX.WEBAUTHN) ? h.ZP.webAuthnAuthenticate(M) : i.U2(JSON.parse(M)).then(e => JSON.stringify(e)),
      s = async e => {
        try {
          await t({
            mfaType: "webauthn",
...
function C(e) {
  ...
  let T = null == m ? _.Z.Messages.TWO_FA_ENTER_SMS_TOKEN_SENDING : _.Z.Messages.TWO_FA_ENTER_SMS_TOKEN_SENT.format({
    phoneNumber: m
  });
