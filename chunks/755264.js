...
function E(e) {
  ...
  let {
state: i
  } = s.ZP.PersistedStore.migrateAndReadStoreState('GIFFavoritesStore', [
e => null == e ? {
  favorites: [],
  timesFavorited: 0
} : Array.isArray(e) ? {
...
let a = r.JM.create();
return a.format = 'IMAGE' === t.format ? r.EO.IMAGE : 'VIDEO' === t.format ? r.EO.VIDEO : r.EO.NONE, a.src = t.src, a.width = t.width, a.height = t.height, a.order = i.favorites.length - n + e, {
  url: t.url,
  favorite: a
};
  });
...
run(e) {
  ...
  let {
    state: n
  } = s.ZP.PersistedStore.migrateAndReadStoreState('StickersPersistedStore', [
...
run(e) {
  let {
    state: t
  } = s.ZP.PersistedStore.migrateAndReadStoreState('ApplicationCommandFrecency', []);
  if (null == t)
    return !1;
  let n = !1;
...
run(e) {
  let {
    state: t
  } = s.ZP.PersistedStore.migrateAndReadStoreState('SoundboardFavoriteStore', []);
  if (null == t)
    return !1;
  let n = !1;
  return a().size(t.favoriteSounds) > 0 && (e.favoriteSoundboardSounds = r.h_.create(), o.default.keys(t.favoriteSounds).forEach(n => {
