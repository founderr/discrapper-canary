n.d(t, {
  Z: function() {
return C;
  }
}), n(47120);
var r = n(735250),
  i = n(470079),
  a = n(120356),
  s = n.n(a),
  o = n(106351),
  l = n(507274),
  u = n(541716),
  c = n(752305),
  d = n(893718),
  _ = n(131704),
  E = n(699516),
  f = n(823379),
  h = n(5192),
  p = n(51144),
  m = n(785717),
  I = n(285050),
  T = n(448520),
  g = n(485216),
  S = n(228168),
  A = n(689938),
  N = n(628372);
let v = (0, _.kt)({
id: '1',
type: o.d.DM
  }),
  O = e => {
let {
  input: t,
  username: n,
  sourceType: r,
  sourceDetails: i
} = e;
switch (r) {
  case S.N9.ACTIVITY:
    let a = A.Z.Messages.USER_PROFILE_REPLIED_TO_ACTIVITY.format({
      username: n
    });
    return '> -# *'.concat(a, '*\n').concat(t);
  case S.N9.AVATAR:
    let s = A.Z.Messages.USER_PROFILE_REPLIED_TO_AVATAR.format({
      username: n
    });
    return '> -# *'.concat(s, '*\n').concat(t);
  case S.N9.STATUS:
    let o = A.Z.Messages.USER_PROFILE_REPLIED_TO_STATUS.format({
      username: n
    });
    return null != i ? '> -# *'.concat(o, '*').concat('\n > '.concat(i), '\n').concat(t) : '> -# *'.concat(o, '*\n').concat(t);
  default:
    (0, f.vE)(r);
}
  },
  R = e => {
switch (e) {
  case S.N9.ACTIVITY:
    return A.Z.Messages.USER_PROFILE_REPLY_TO_ACTIVITY_PLACEHOLDER;
  case S.N9.AVATAR:
    return A.Z.Messages.USER_PROFILE_REPLY_TO_AVATAR_PLACEHOLDER;
  case S.N9.STATUS:
    return A.Z.Messages.USER_PROFILE_REPLY_TO_STATUS_PLACEHOLDER;
  default:
    (0, f.vE)(e);
}
  };

function C(e) {
  let {
user: t,
guildId: n,
channelId: a,
profileType: o,
sourceType: _,
sourceDetails: f,
onReply: A,
setPopoutRef: C,
modalKey: y
  } = e, {
trackUserProfileAction: D
  } = (0, m.KZ)(), {
notifyReplySent: L
  } = (0, I.e)(), {
sendReply: b
  } = (0, T.D)(_), [M, P] = i.useState(''), [U, w] = i.useState((0, c.JM)(M)), x = i.useRef(!1), G = i.useRef(null);
  i.useEffect(() => {
null == C || C(null == G ? void 0 : G.current);
  }, [
G,
C
  ]);
  let k = async e => {
var n;
if (null == e)
  return;
D({
  action: b
});
let r = O({
  input: e,
  username: null !== (n = E.Z.getNickname(t.id)) && void 0 !== n ? n : p.ZP.getName(t),
  sourceType: _,
  sourceDetails: f
});
await (0, g.Z)({
  userId: t.id,
  content: r,
  location: 'UserProfileReplyPopout',
  openChannel: !1,
  whenReady: !1
}), L();
  };
  return (0, r.jsx)(l.V, {
ref: G,
children: (0, r.jsx)('div', {
  className: s()(N.container, {
    [N.panel]: o === S.y0.PANEL
  }),
  children: (0, r.jsx)(d.Z, {
    parentModalKey: y,
    emojiPickerCloseOnModalOuterClick: !0,
    innerClassName: N.inner,
    editorClassName: N.editor,
    type: u.I.USER_PROFILE_REPLY,
    placeholder: R(_).format({
      username: h.ZP.getName(n, a, t)
    }),
    channel: v,
    textValue: M,
    richValue: U,
    onChange: (e, t, n) => {
      if (t !== M)
        P(t), w(n);
    },
    focused: x.current,
    onFocus: () => {
      x.current = !0;
    },
    onBlur: e => {
      var t;
      if (null === (t = G.current) || void 0 === t ? void 0 : t.contains(e.relatedTarget)) {
        x.current = !1;
        return;
      }
      null !== G.current && (x.current = !1, null == A || A(null));
    },
    onSubmit: async e => {
      let {
        value: t
      } = e;
      try {
        return await k(t.trim()), null == A || A(null), {
          shouldClear: !0,
          shouldRefocus: !1
        };
      } catch {
        return {
          shouldClear: !1,
          shouldRefocus: !1
        };
      }
    }
  })
})
  });
}