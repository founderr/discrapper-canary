(this.webpackChunkdiscord_app = this.webpackChunkdiscord_app || []).push([
    ["27755"], {
        69448: function(e, t, n) {
            "use strict";
            e.exports = n.p + "f6da6e7ed1d57e445d0e.svg"
        },
        601248: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                ChatMarkUnreadIcon: function() {
                    return i
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("669491"),
                u = n("75196");
            let i = e => {
                let {
                    width: t = 24,
                    height: n = 24,
                    color: i = a.default.colors.INTERACTIVE_NORMAL,
                    colorClass: s = "",
                    ...r
                } = e;
                return (0, l.jsxs)("svg", {
                    ...(0, u.default)(r),
                    width: t,
                    height: n,
                    viewBox: "0 0 24 24",
                    fill: "none",
                    xmlns: "http://www.w3.org/2000/svg",
                    children: [(0, l.jsx)("path", {
                        d: "M12.9344 21.9569C13.1779 21.9344 13.3563 21.7285 13.3978 21.4876C13.4177 21.3721 13.4445 21.2575 13.478 21.1444C13.556 20.8815 13.4569 20.5878 13.2352 20.4263C12.4866 19.8808 12 18.9972 12 18V15C12 13.3431 13.3431 12 15 12C15.485 12 15.943 12.1151 16.3484 12.3194C16.4853 12.3884 16.6441 12.4051 16.7895 12.3567C17.4849 12.1254 18.2284 12 19 12C19.8301 12 20.6264 12.1445 21.3652 12.4097C21.6653 12.5174 22.0001 12.3189 22.0001 12C22.0001 6.47715 17.5229 2 12.0001 2C6.47722 2 2.00006 6.47715 2.00006 12C2.00006 13.9707 2.57012 15.8083 3.55434 17.3568C3.67708 17.5499 3.66399 17.8017 3.51353 17.9742L1.44664 20.3425C0.882113 20.9893 1.34151 22 2.20006 22H12.0001C12.3151 22 12.6268 21.9854 12.9344 21.9569Z",
                        fill: "string" == typeof i ? i : i.css,
                        className: s
                    }), (0, l.jsx)("path", {
                        d: "M18 17H16.7637C17.3133 16.3861 18.1121 16 19 16C20.6569 16 22 17.3431 22 19C22 20.6569 20.6569 22 19 22C18.2385 22 17.5454 21.7176 17.0159 21.2503C16.6018 20.8848 15.9699 20.9242 15.6044 21.3383C15.239 21.7523 15.2784 22.3843 15.6924 22.7497C16.5733 23.5272 17.7326 24 19 24C21.7614 24 24 21.7614 24 19C24 16.2386 21.7614 14 19 14C17.874 14 16.8354 14.372 16 14.9997C15.9998 14.4475 15.5522 14 15 14C14.4477 14 14 14.4477 14 15V18C14 18.5523 14.4477 19 15 19H18C18.5523 19 19 18.5523 19 18C19 17.4477 18.5523 17 18 17Z",
                        fill: "string" == typeof i ? i : i.css,
                        className: s
                    })]
                })
            }
        },
        35647: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                IdIcon: function() {
                    return i
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("669491"),
                u = n("75196");
            let i = e => {
                let {
                    width: t = 24,
                    height: n = 24,
                    color: i = a.default.colors.INTERACTIVE_NORMAL,
                    colorClass: s = "",
                    ...r
                } = e;
                return (0, l.jsxs)("svg", {
                    ...(0, u.default)(r),
                    width: t,
                    height: n,
                    viewBox: "0 0 24 24",
                    fill: "none",
                    xmlns: "http://www.w3.org/2000/svg",
                    children: [(0, l.jsx)("path", {
                        d: "M15.2979 14.48C14.8405 14.928 14.2199 15.152 13.4359 15.152H12.0499V9.18803H13.4359C14.2199 9.18803 14.8405 9.41203 15.2979 9.86003C15.7552 10.308 15.9839 11.078 15.9839 12.17C15.9839 13.262 15.7552 14.032 15.2979 14.48Z",
                        fill: "string" == typeof i ? i : i.css,
                        className: s
                    }), (0, l.jsx)("path", {
                        fillRule: "evenodd",
                        clipRule: "evenodd",
                        d: "M5 2C3.34315 2 2 3.34315 2 5V19C2 20.6569 3.34315 22 5 22H19C20.6569 22 22 20.6569 22 19V5C22 3.34315 20.6569 2 19 2H5ZM6 17H8.044V7.34003H6V17ZM10.0059 7.34003V17H13.4359C14.9012 17 16.0305 16.5847 16.8239 15.754C17.6265 14.9234 18.0279 13.7287 18.0279 12.17C18.0279 10.6114 17.6265 9.41669 16.8239 8.58603C16.0305 7.75536 14.9012 7.34003 13.4359 7.34003H10.0059Z",
                        fill: "string" == typeof i ? i : i.css,
                        className: s
                    })]
                })
            }
        },
        991630: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return f
                }
            }), n("222007");
            var l = n("37983"),
                a = n("884691"),
                u = n("414456"),
                i = n.n(u),
                s = n("587974"),
                r = n("315102"),
                o = n("524768"),
                c = n("616674"),
                d = n("363577");

            function f(e) {
                let {
                    section: t,
                    isSelected: n,
                    width: u,
                    height: f,
                    className: C,
                    selectable: h = !1,
                    onFocus: m,
                    onBlur: p,
                    onMouseOver: v,
                    onMouseLeave: M,
                    ...E
                } = e, [g, I] = a.useState(!1), N = a.useCallback(() => {
                    I(!0), null == m || m()
                }, [m]), x = a.useCallback(() => {
                    I(!1), null == p || p()
                }, [p]), _ = a.useCallback(() => {
                    I(!0), null == v || v()
                }, [v]), A = a.useCallback(() => {
                    I(!1), null == M || M()
                }, [M]), S = a.useMemo(() => {
                    if (t.type === o.ApplicationCommandSectionType.APPLICATION) {
                        var e;
                        return r.default.getApplicationIconURL({
                            id: t.id,
                            icon: t.icon,
                            bot: null === (e = t.application) || void 0 === e ? void 0 : e.bot,
                            botIconFirst: !0,
                            size: u
                        })
                    }
                    return d
                }, [t, u]);
                return (0, l.jsx)("div", {
                    ...E,
                    className: i(c.wrapper, C, {
                        [c.selectable]: h,
                        [c.selected]: h && n
                    }),
                    onFocus: N,
                    onBlur: x,
                    onMouseOver: _,
                    onMouseLeave: A,
                    children: (0, l.jsx)(s.default, {
                        className: c.mask,
                        mask: h && (n || g) ? s.MaskIDs.SQUIRCLE : s.MaskIDs.AVATAR_DEFAULT,
                        width: u,
                        height: f,
                        children: (0, l.jsx)("img", {
                            alt: "",
                            className: c.icon,
                            style: {
                                width: u,
                                height: f
                            },
                            src: S
                        })
                    })
                })
            }
        },
        943161: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return d
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("414456"),
                u = n.n(a),
                i = n("83910"),
                s = n("93393"),
                r = n("317041"),
                o = n("749296");
            let c = e => {
                switch (e.id) {
                    case r.BuiltInSectionId.BUILT_IN:
                        return i.default;
                    case r.BuiltInSectionId.FRECENCY:
                        return s.default;
                    default:
                        return
                }
            };

            function d(e) {
                let {
                    section: t,
                    className: n,
                    width: a,
                    height: i,
                    padding: s,
                    isSelected: r,
                    selectable: d = !1,
                    ...f
                } = e, C = c(t);
                return (0, l.jsx)("div", {
                    className: u(o.wrapper, n, {
                        [o.selectable]: d,
                        [o.selected]: d && r
                    }),
                    style: {
                        width: a,
                        height: i,
                        padding: null != s ? s : 0
                    },
                    children: null != C ? (0, l.jsx)(C, {
                        className: o.icon,
                        width: a,
                        height: i,
                        ...f
                    }) : null
                })
            }
        },
        355263: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                getIconComponent: function() {
                    return s
                },
                pxToRem: function() {
                    return r
                },
                getCommandQuery: function() {
                    return o
                }
            });
            var l = n("524768"),
                a = n("991630"),
                u = n("943161"),
                i = n("317041");
            let s = e => e.type === l.ApplicationCommandSectionType.BUILT_IN ? u.default : a.default;

            function r(e) {
                return "".concat(e / 16, "rem")
            }

            function o(e, t) {
                let n = t,
                    l = !1,
                    a = t.indexOf(":");
                if (a >= 0) {
                    let e = t.lastIndexOf(" ", a);
                    e >= 0 ? (t = t.substring(0, e), l = !0) : t = t.substring(0, a)
                } else t = t.substring(0, t.length);
                let u = t.split(" ", i.MAX_SUBCOMMAND_LEVEL + 1);
                return u.length > i.MAX_SUBCOMMAND_LEVEL && (l = !0, u.pop()), t = u.join(" "), (n.length > t.length || t.endsWith(" ")) && (l = !0, t = t.trimEnd()), {
                    text: t,
                    parts: u,
                    hasSpaceTerminator: l
                }
            }
        },
        633043: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return i
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("77078"),
                u = n("850572"),
                i = () => (0, l.jsx)("div", {
                    className: u.loadingWrapper,
                    children: (0, l.jsx)(a.Dots, {
                        dotRadius: 4,
                        themed: !0
                    })
                })
        },
        861370: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return r
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("77078"),
                u = n("845579"),
                i = n("662255"),
                s = n("306160");

            function r(e) {
                let {
                    id: t,
                    label: n,
                    onSuccess: r,
                    shiftId: o,
                    showIconFirst: c
                } = e, d = u.DeveloperMode.useSetting();
                if (__OVERLAY__ || !d || !s.SUPPORTS_COPY) return null;
                let f = "devmode-copy-id-".concat(t);
                return (0, l.jsx)(a.MenuItem, {
                    id: f,
                    label: n,
                    action: function(e) {
                        let n = null != o && e.shiftKey ? o : t;
                        (0, s.copy)(n), null == r || r()
                    },
                    icon: i.default,
                    showIconFirst: c
                }, f)
            }
        },
        385065: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return v
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("118810"),
                u = n("77078"),
                i = n("272030"),
                s = n("798609"),
                r = n("861370"),
                o = n("152298"),
                c = n("26051"),
                d = n("667358"),
                f = n("563816"),
                C = n("518155"),
                h = n("718381"),
                m = n("875834"),
                p = n("782340");

            function v(e) {
                var t;
                let n, v, M, {
                        channel: E,
                        message: g,
                        target: I,
                        attachment: N,
                        onSelect: x,
                        onHeightUpdate: _
                    } = e,
                    A = I;
                if (null != N) v = n = N.url;
                else
                    for (;
                        (0, a.isElement)(A);)(0, a.isElement)(A, HTMLImageElement) && null != A.src && (v = A.src), (0, a.isElement)(A, HTMLAnchorElement) && null != A.href && (n = A.href, M = A.textContent), A = A.parentNode;
                let S = null === (t = document.getSelection()) || void 0 === t ? void 0 : t.toString(),
                    T = (0, o.default)(S),
                    O = (0, d.default)(S),
                    y = (0, f.default)({
                        commandType: s.ApplicationCommandType.MESSAGE,
                        commandTargetId: g.id,
                        channel: E,
                        guildId: void 0,
                        onShow: _
                    }),
                    j = (0, m.default)(g, E),
                    R = (0, C.default)(g, E),
                    b = (0, h.default)(g, E),
                    w = (0, c.default)(null != n ? n : v, M),
                    L = (0, r.default)({
                        id: g.id,
                        label: p.default.Messages.COPY_ID_MESSAGE,
                        shiftId: "".concat(g.channel_id, "-").concat(g.id)
                    });
                return (0, l.jsxs)(u.Menu, {
                    navId: "message",
                    onClose: i.closeContextMenu,
                    "aria-label": p.default.Messages.MESSAGE_ACTIONS_MENU_LABEL,
                    onSelect: x,
                    children: [(0, l.jsx)(u.MenuGroup, {
                        children: T
                    }), (0, l.jsx)(u.MenuGroup, {
                        children: O
                    }), (0, l.jsxs)(u.MenuGroup, {
                        children: [y, j, R, b]
                    }), (0, l.jsx)(u.MenuGroup, {
                        children: w
                    }), (0, l.jsx)(u.MenuGroup, {
                        children: L
                    })]
                })
            }
        },
        518155: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return d
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("281071"),
                u = n("77078"),
                i = n("83900"),
                s = n("599110"),
                r = n("306160"),
                o = n("49111"),
                c = n("782340");

            function d(e, t) {
                return r.SUPPORTS_COPY && e.type !== o.MessageTypes.GUILD_INVITE_REMINDER ? (0, l.jsx)(u.MenuItem, {
                    id: "copy-link",
                    label: c.default.Messages.COPY_MESSAGE_LINK,
                    icon: i.default,
                    action: () => {
                        (0, a.copy)("".concat(location.protocol, "//").concat(location.host).concat(o.Routes.CHANNEL(t.guild_id, t.id, e.id))), s.default.track(o.AnalyticEvents.MESSAGE_LINK_COPIED, {
                            message_id: e.id,
                            channel: e.channel_id
                        })
                    }
                }) : null
            }
        },
        718381: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return h
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("446674"),
                u = n("77078"),
                i = n("819689"),
                s = n("249561"),
                r = n("300322"),
                o = n("957255"),
                c = n("697218"),
                d = n("228220"),
                f = n("49111"),
                C = n("782340");

            function h(e, t) {
                let n = (0, a.useStateFromStores)([c.default], () => c.default.getCurrentUser()),
                    h = (0, r.useIsActiveChannelOrUnarchivableThread)(t),
                    m = (0, a.useStateFromStores)([o.default], () => o.default.can(f.Permissions.MANAGE_MESSAGES, t), [t]);
                return e.state !== f.MessageStates.SENDING && (e.author.id === (null == n ? void 0 : n.id) || m) && e.type in f.MessageTypesDeletable && h ? (0, l.jsx)(u.MenuItem, {
                    id: "delete",
                    label: C.default.Messages.DELETE_MESSAGE,
                    action: function(n) {
                        e.state === f.MessageStates.SEND_FAILED ? i.default.deleteMessage(t.id, e.id, !0) : n.shiftKey ? i.default.deleteMessage(t.id, e.id) : s.default.confirmDelete(t, e, !0)
                    },
                    color: "danger",
                    icon: d.default
                }) : null
            }
        },
        875834: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return o
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("77078"),
                u = n("300322"),
                i = n("993477"),
                s = n("456936"),
                r = n("782340");

            function o(e, t) {
                let n = (0, u.useIsActiveChannelOrUnarchivableThread)(t);
                return n ? (0, l.jsx)(a.MenuItem, {
                    id: "mark-unread",
                    label: r.default.Messages.MARK_UNREAD,
                    action: () => (0, s.default)(t.id, e.id),
                    icon: i.default
                }) : null
            }
        },
        146574: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                getNativeContextMenuChannelAnalytics: function() {
                    return u
                }
            });
            var l = n("18494"),
                a = n("724210");

            function u() {
                var e;
                let t = null !== (e = l.default.getCurrentlySelectedChannelId()) && void 0 !== e ? e : void 0;
                return null != t && (0, a.isStaticChannelRoute)(t) ? {
                    channel_static_route: t
                } : {
                    channel_id: t
                }
            }
        },
        152298: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return r
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("77078"),
                u = n("773336"),
                i = n("50885"),
                s = n("782340");

            function r(e, t) {
                return u.isPlatformEmbedded && (null == e ? void 0 : e.length) !== 0 ? (0, l.jsx)(a.MenuItem, {
                    id: "copy",
                    label: s.default.Messages.COPY,
                    hint: (0, u.isMac)() ? "⌘C" : "Ctrl+C",
                    action: () => {
                        i.default.copy(e), null == t || t.focus()
                    }
                }) : null
            }
        },
        26051: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return p
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("77078"),
                u = n("689226"),
                i = n("467475"),
                s = n("24373"),
                r = n("48703"),
                o = n("599110"),
                c = n("128259"),
                d = n("773336"),
                f = n("50885"),
                C = n("146574"),
                h = n("49111"),
                m = n("782340");

            function p(e, t, n, p) {
                let v = (0, i.default)(null == n ? void 0 : n.getChannelId());
                if (!d.isPlatformEmbedded || null == e || "" === e || v || (null == p ? void 0 : p.shouldHideMediaOptions) === !0 || null != e && null != (0, s.isStickerAssetUrl)(e) || null != e && (0, u.isRoleIconAssetUrl)(e)) return null;
                let M = (0, r.getHostname)(e),
                    E = n => {
                        o.default.track(h.AnalyticEvents.CONTEXT_MENU_LINK_OPENED, {
                            hostname: M,
                            ...(0, C.getNativeContextMenuChannelAnalytics)()
                        }), (0, c.handleClick)({
                            href: e,
                            trusted: (0, c.isLinkTrusted)(e, t),
                            shouldConfirm: !0
                        }, n)
                    };
                return [(0, l.jsx)(a.MenuItem, {
                    id: "copy-native-link",
                    label: m.default.Messages.COPY_LINK,
                    action: () => {
                        o.default.track(h.AnalyticEvents.CONTEXT_MENU_LINK_COPIED, {
                            hostname: M,
                            ...(0, C.getNativeContextMenuChannelAnalytics)()
                        }), f.default.copy(e)
                    }
                }, "copy-native-link"), (0, l.jsx)(a.MenuItem, {
                    id: "open-native-link",
                    label: m.default.Messages.OPEN_LINK,
                    action: e => E(e)
                }, "open-native-link")]
            }
        },
        667358: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return f
                }
            });
            var l = n("37983"),
                a = n("884691"),
                u = n("627445"),
                i = n.n(u),
                s = n("77078"),
                r = n("599110"),
                o = n("773336"),
                c = n("49111"),
                d = n("782340");

            function f(e) {
                let t = a.useCallback(() => {
                    i(null != e, "text cannot be null"), r.default.track(c.AnalyticEvents.MESSAGE_MENU_GOOGLE_SEARCHED), window.open("https://www.google.com/search?q=".concat(encodeURIComponent(e)), "_blank")
                }, [e]);
                return o.isPlatformEmbedded && null != e && (null == e ? void 0 : e.length) !== 0 ? [(0, l.jsx)(s.MenuItem, {
                    id: "search-google",
                    label: d.default.Messages.SEARCH_WITH_GOOGLE,
                    action: t
                }, "search-google")] : null
            }
        },
        563816: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return E
                }
            });
            var l = n("37983"),
                a = n("884691"),
                u = n("627445"),
                i = n.n(u),
                s = n("446674"),
                r = n("77078"),
                o = n("240249"),
                c = n("524768"),
                d = n("389153"),
                f = n("916565"),
                C = n("355263"),
                h = n("633043"),
                m = n("305961"),
                p = n("317041"),
                v = n("782340"),
                M = n("205735"),
                E = e => {
                    let {
                        commandType: t,
                        commandTargetId: n,
                        channel: u,
                        guildId: E,
                        onShow: g
                    } = e, I = a.useRef(!1), N = a.useRef(0), x = (0, s.useStateFromStores)([m.default], () => m.default.getGuild(null != E ? E : u.guild_id)), {
                        hasMoreAfter: _,
                        scrollDown: A,
                        sectionDescriptors: S,
                        commands: T,
                        placeholders: O
                    } = o.useDiscovery(u, {
                        commandType: t
                    }, {
                        placeholderCount: p.CONTEXT_MENU_PLACEHOLDER_COUNT,
                        limit: p.CONTEXT_MENU_COMMANDS_QUERY_LIMIT
                    }), {
                        visibleCommands: y,
                        sections: j
                    } = a.useMemo(() => {
                        let e = T.concat(O),
                            t = {};
                        return S.forEach(e => {
                            t[e.id] = e
                        }), {
                            visibleCommands: e,
                            sections: t
                        }
                    }, [T, S, O]), R = a.useMemo(() => (0, d.canUseContextMenuCommands)(u, y.length, o.getCommandTypeCount(u, t, null == x ? void 0 : x.applicationCommandCounts)), [u, t, y.length, null == x ? void 0 : x.applicationCommandCounts]);
                    a.useEffect(() => {
                        R !== I.current && (R && (null == g || g()), I.current = R)
                    }, [R, g]), a.useEffect(() => {
                        b(N.current)
                    }, [y]);
                    let b = a.useCallback(e => {
                            _ && e + 500 > 34 * y.length - 40 && A(), N.current = e
                        }, [_, A, y]),
                        w = a.useCallback(e => {
                            if (e.inputType === c.ApplicationCommandInputType.PLACEHOLDER) return (0, l.jsx)(r.MenuItem, {
                                id: "menu-command-".concat(e.id),
                                render: () => (0, l.jsx)(h.default, {})
                            }, "menu-command-".concat(e.id));
                            i(null != u, "menu item should not show if channel is null");
                            let t = j[e.applicationId],
                                a = null != t ? (0, C.getIconComponent)(t) : void 0;
                            return (0, l.jsx)(r.MenuItem, {
                                id: e.id,
                                label: e.displayName,
                                showIconFirst: !0,
                                icon: () => null != a ? (0, l.jsx)(a, {
                                    channel: u,
                                    section: t,
                                    width: 18,
                                    height: 18,
                                    selectable: !1
                                }) : null,
                                action: () => {
                                    (0, f.default)({
                                        command: e,
                                        optionValues: {},
                                        context: {
                                            channel: u,
                                            guild: x
                                        },
                                        commandTargetId: n
                                    })
                                }
                            }, e.id)
                        }, [u, x, n, j]);
                    return R && (0, l.jsx)(r.MenuItem, {
                        id: "apps",
                        label: v.default.Messages.APPS,
                        onChildrenScroll: b,
                        childRowHeight: 34,
                        listClassName: M.list,
                        disabled: 0 === y.length,
                        children: y.map(w)
                    })
                }
        },
        555158: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                HelpMessageTypes: function() {
                    return a
                },
                default: function() {
                    return h
                }
            });
            var l, a, u = n("37983");
            n("884691");
            var i = n("414456"),
                s = n.n(i),
                r = n("77078"),
                o = n("36694"),
                c = n("381546"),
                d = n("68238"),
                f = n("423487"),
                C = n("557296");
            (l = a || (a = {}))[l.WARNING = 0] = "WARNING", l[l.INFO = 1] = "INFO", l[l.ERROR = 2] = "ERROR", l[l.POSITIVE = 3] = "POSITIVE";
            var h = function(e) {
                let {
                    children: t,
                    messageType: n,
                    className: l,
                    textColor: a = "text-normal",
                    textVariant: i = "text-sm/medium"
                } = e, h = function(e) {
                    switch (e) {
                        case 0:
                            return f.default;
                        case 1:
                            return d.default;
                        case 2:
                            return c.default;
                        case 3:
                            return o.default
                    }
                }(n), m = function(e) {
                    switch (e) {
                        case 0:
                            return C.warning;
                        case 1:
                            return C.info;
                        case 2:
                            return C.error;
                        case 3:
                            return C.positive
                    }
                }(n);
                return (0, u.jsxs)("div", {
                    className: s(C.container, m, l),
                    children: [(0, u.jsx)("div", {
                        className: C.iconDiv,
                        children: (0, u.jsx)(h, {
                            className: C.icon
                        })
                    }), (0, u.jsx)(r.Text, {
                        className: C.text,
                        color: a,
                        variant: i,
                        children: t
                    })]
                })
            }
        },
        662255: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return s
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("469563"),
                u = n("35647"),
                i = n("75196"),
                s = (0, a.replaceIcon)(function(e) {
                    let {
                        width: t = 24,
                        height: n = 24,
                        color: a = "currentColor",
                        foreground: u,
                        ...s
                    } = e;
                    return (0, l.jsx)("svg", {
                        ...(0, i.default)(s),
                        width: t,
                        height: n,
                        viewBox: "0 0 24 24",
                        children: (0, l.jsx)("path", {
                            className: u,
                            fill: a,
                            fillRule: "evenodd",
                            clipRule: "evenodd",
                            d: "M3.37868 2.87868C3.94129 2.31607 4.70435 2 5.5 2H19.5C20.2956 2 21.0587 2.31607 21.6213 2.87868C22.1839 3.44129 22.5 4.20435 22.5 5V19C22.5 19.7956 22.1839 20.5587 21.6213 21.1213C21.0587 21.6839 20.2956 22 19.5 22H5.5C4.70435 22 3.94129 21.6839 3.37868 21.1213C2.81607 20.5587 2.5 19.7956 2.5 19V5C2.5 4.20435 2.81607 3.44129 3.37868 2.87868ZM7.65332 16.3125H9.47832V7.6875H7.65332V16.3125ZM11.23 7.6875V16.3125H14.2925C15.6008 16.3125 16.6091 15.9417 17.3175 15.2C18.0341 14.4583 18.3925 13.3917 18.3925 12C18.3925 10.6083 18.0341 9.54167 17.3175 8.8C16.6091 8.05833 15.6008 7.6875 14.2925 7.6875H11.23ZM15.955 14.0625C15.5466 14.4625 14.9925 14.6625 14.2925 14.6625H13.055V9.3375H14.2925C14.9925 9.3375 15.5466 9.5375 15.955 9.9375C16.3633 10.3375 16.5675 11.025 16.5675 12C16.5675 12.975 16.3633 13.6625 15.955 14.0625Z"
                        })
                    })
                }, u.IdIcon, void 0, {
                    size: 24
                })
        },
        993477: function(e, t, n) {
            "use strict";
            n.r(t), n.d(t, {
                default: function() {
                    return s
                }
            });
            var l = n("37983");
            n("884691");
            var a = n("469563"),
                u = n("601248"),
                i = n("75196"),
                s = (0, a.replaceIcon)(function(e) {
                    let {
                        width: t = 24,
                        height: n = 24,
                        color: a = "currentColor",
                        foreground: u,
                        ...s
                    } = e;
                    return (0, l.jsx)("svg", {
                        ...(0, i.default)(s),
                        width: t,
                        height: n,
                        viewBox: "0 0 24 24",
                        children: (0, l.jsx)("path", {
                            className: u,
                            fill: a,
                            d: "M14 3H20C21 3 22.0001 4 22.0001 5V19.0003C22.0001 20 21 21 20 21H14C13 21 6 13 6 13H2V11H6C6 11 13 3 14 3Z"
                        })
                    })
                }, u.ChatMarkUnreadIcon, void 0, {
                    size: 24
                })
        }
    }
]);