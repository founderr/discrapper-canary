(this.webpackChunkdiscord_app = this.webpackChunkdiscord_app || []).push([
    [49799], {
        478717: (e, t, r) => {
            var n = {
                "./2017-11-16.mp4": 684795,
                "./2019-12-17.mp4": 662325,
                "./channel-following.png": 15176,
                "./discovery.jpg": 788557,
                "./discriminator.mp4": 22625,
                "./g250k-cl.mp4": 603628,
                "./header.mp4": 879334,
                "./hypesquad-hack-week/header.png": 133650,
                "./hypesquad-hack-week/paladins-loot.png": 631896,
                "./ko-new-year.jpg": 931806,
                "./league-rp-cl.mp4": 425058,
                "./polish-week.png": 428915,
                "./special-template/desktop-cta.png": 561323,
                "./special-template/update-badge.svg": 361850,
                "./stickers-launch/2020_PMM_STICKERS_Mrkt_BlogHdr_2500x1000.png": 15808,
                "./stickers-launch/2020_PMM_STICKERS_Mrkt_DesktopHero_Left_180x220.png": 227227,
                "./stickers-launch/2020_PMM_STICKERS_Mrkt_DesktopHero_Right_180x220.png": 999455,
                "./stickers-launch/2020_PMM_STICKERS_Mrkt_Desktop_Changelog_658X220.png": 678811,
                "./stickers-launch/2020_PMM_STICKERS_Mrkt_Desktop_FooterCTA_238x220.png": 236340,
                "./stickers-launch/2020_PMM_STICKERS_Mrkt_MobileHero_375x190.png": 359546,
                "./stickers-launch/2020_PMM_STICKERS_Mrkt_Mobile_FooterCTA_343x78.png": 35448,
                "./store.jpg": 404252
            };

            function o(e) {
                var t = c(e);
                return r(t)
            }

            function c(e) {
                if (!r.o(n, e)) {
                    var t = new Error("Cannot find module '" + e + "'");
                    t.code = "MODULE_NOT_FOUND";
                    throw t
                }
                return n[e]
            }
            o.keys = function() {
                return Object.keys(n)
            };
            o.resolve = c;
            e.exports = o;
            o.id = 478717
        },
        684795: (e, t, r) => {
            e.exports = r.p + "dca6464b06c48b1fab5f2e8320446b32.mp4"
        },
        662325: (e, t, r) => {
            e.exports = r.p + "ae046b62e28bb1ec1fbdde8185d32b9e.mp4"
        },
        15176: (e, t, r) => {
            e.exports = r.p + "19ee7a767639c0af819b5976b493e9d8.png"
        },
        788557: (e, t, r) => {
            e.exports = r.p + "230cd026d4bfdeb914b266938cc506a1.jpg"
        },
        22625: (e, t, r) => {
            e.exports = r.p + "7f34e6e3da7d1403bad96e20b9b3a32c.mp4"
        },
        603628: (e, t, r) => {
            e.exports = r.p + "7c2b742b0f9040988b4bb4e351fdedb9.mp4"
        },
        879334: (e, t, r) => {
            e.exports = r.p + "7f73b3e68d81916d67cc58a3deeca567.mp4"
        },
        133650: (e, t, r) => {
            e.exports = r.p + "abaa70ae2be9f361759c60b2ee85ff12.png"
        },
        631896: (e, t, r) => {
            e.exports = r.p + "3522a06815d204da075ffda2431ef7f2.png"
        },
        931806: (e, t, r) => {
            e.exports = r.p + "9acacf43b5dddb33196fa60a4a6c6b2f.jpg"
        },
        425058: (e, t, r) => {
            e.exports = r.p + "d6a1cc0cb7f729ae421d609d72839816.mp4"
        },
        428915: (e, t, r) => {
            e.exports = r.p + "1b9af825f3ccd5ffdf9580e338d4e5e7.png"
        },
        561323: (e, t, r) => {
            e.exports = r.p + "bb867095876d3f6d81cc12e745ee9f58.png"
        },
        361850: (e, t, r) => {
            e.exports = r.p + "06d8e21539e634cbc0f2ecf38f1bb2bb.svg"
        },
        15808: (e, t, r) => {
            e.exports = r.p + "fe875f7f2f0c4869d155e7e487f570f2.png"
        },
        227227: (e, t, r) => {
            e.exports = r.p + "7f9b927507a1c9321da58ab0fafffcd0.png"
        },
        999455: (e, t, r) => {
            e.exports = r.p + "1c3ba8528a8f16c8cf0c55e286010420.png"
        },
        678811: (e, t, r) => {
            e.exports = r.p + "831ba24f72b89cc77c325317372b7239.png"
        },
        236340: (e, t, r) => {
            e.exports = r.p + "3785e11ebe8fdcd64db5bb49f2e5299d.png"
        },
        359546: (e, t, r) => {
            e.exports = r.p + "506bf478f5e1a271a27b673e6f25749a.png"
        },
        35448: (e, t, r) => {
            e.exports = r.p + "07d9032e5d30865e0cc076396f0b027a.png"
        },
        404252: (e, t, r) => {
            e.exports = r.p + "9d402086959fccbf418074a4b96f6d19.jpg"
        },
        656212: (e, t, r) => {
            "use strict";
            r.r(t);
            r.d(t, {
                default: () => Oe
            });
            var n = r(785893),
                o = r(667294),
                c = r(882723),
                i = r(402391),
                a = r(318715),
                l = r(384411),
                s = r(734620),
                u = r(907784);

            function f(e, t) {
                var r = (0, a.cj)([s.Z], (function() {
                        var r = null != e ? s.Z.getChangelog(e, t) : null,
                            n = null != e ? s.Z.getChangelog(e, "en-US") : null,
                            o = null != e && s.Z.getChangelogLoadStatus(e, "en-US");
                        return {
                            changelog: r,
                            loadState: null != e && s.Z.getChangelogLoadStatus(e, t),
                            defaultChangelog: n,
                            defaultLoadState: o
                        }
                    }), [e, t]),
                    n = r.changelog,
                    c = r.loadState,
                    l = r.defaultChangelog,
                    f = r.defaultLoadState;
                o.useEffect((function() {
                    null != e && null == n && c === u.LU.NOT_LOADED && i.Z.fetchChangelog(e, t)
                }), [e, n, c, t]);
                return null == e ? {
                    id: e,
                    changelog: null,
                    loaded: !1
                } : null == n && c === u.LU.LOADED_FAILURE ? {
                    id: e,
                    changelog: l,
                    loaded: f !== u.LU.NOT_LOADED
                } : {
                    id: e,
                    changelog: n,
                    loaded: c !== u.LU.NOT_LOADED
                }
            }
            var p = r(652591),
                d = r(294184),
                g = r.n(d),
                b = r(730381),
                h = r.n(b),
                y = r(112184),
                O = r(883064),
                m = r(386364),
                j = r(802139),
                v = r(107364),
                C = r(319463),
                w = r(750349),
                x = r(313197),
                P = r(786170),
                _ = r(120053),
                k = r.n(_),
                S = r(757278);

            function E(e, t, r) {
                t in e ? Object.defineProperty(e, t, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[t] = r;
                return e
            }

            function L(e) {
                for (var t = 1; t < arguments.length; t++) {
                    var r = null != arguments[t] ? arguments[t] : {},
                        n = Object.keys(r);
                    "function" == typeof Object.getOwnPropertySymbols && (n = n.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                        return Object.getOwnPropertyDescriptor(r, e).enumerable
                    }))));
                    n.forEach((function(t) {
                        E(e, t, r[t])
                    }))
                }
                return e
            }

            function R(e, t) {
                t = null != t ? t : {};
                Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : function(e, t) {
                    var r = Object.keys(e);
                    if (Object.getOwnPropertySymbols) {
                        var n = Object.getOwnPropertySymbols(e);
                        t && (n = n.filter((function(t) {
                            return Object.getOwnPropertyDescriptor(e, t).enumerable
                        })));
                        r.push.apply(r, n)
                    }
                    return r
                }(Object(t)).forEach((function(r) {
                    Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r))
                }));
                return e
            }
            var M, D = k().defaultRules.lheading,
                N = k().defaultRules.heading,
                Z = k().defaultRules.link,
                A = k().defaultRules.image,
                T = k().defaultRules.list,
                I = k().defaultRules.blockQuote,
                H = k().defaultRules.paragraph,
                G = /\{(.+?)}/,
                U = /^\$(\w+?)\$/;
            M = r(641596);
            var B = function(e) {
                    var t = e.transformUpperCase,
                        r = void 0 !== t && t;
                    return function(e, t, n) {
                        var o = G.exec(e[1]),
                            c = e[1].replace(G, "");
                        r && (c = c.toUpperCase());
                        return {
                            className: null != o ? o[1] : null,
                            level: "=" === e[2] ? 1 : 2,
                            content: k().parseInline(t, c, n)
                        }
                    }
                },
                W = function(e) {
                    return R(L({}, M.baseRules), {
                        image: L({}, A, "function" == typeof M.customRules.image ? M.customRules.image(e) : M.customRules.image),
                        link: L({}, Z, "function" == typeof M.customRules.link ? M.customRules.link(e) : M.customRules.link),
                        list: L({}, T, "function" == typeof M.customRules.list ? M.customRules.list(e) : M.customRules.list),
                        interpolation: {
                            order: S.ZP.order,
                            match: function(e) {
                                return U.exec(e)
                            },
                            parse: function(e, t, r) {
                                var n = r.interpolations[e[1]];
                                return null == n ? {
                                    type: "text",
                                    content: e[0]
                                } : {
                                    type: "interpolation",
                                    renderer: n
                                }
                            },
                            react: function(e) {
                                return e.renderer()
                            }
                        },
                        lheading: L(R(L({}, D), {
                            parse: B({
                                transformUpperCase: !0
                            })
                        }), "function" == typeof M.customRules.lheading ? M.customRules.lheading(e) : M.customRules.lheading),
                        heading: L({}, N, "function" == typeof M.customRules.heading ? M.customRules.heading(e) : M.customRules.heading),
                        blockQuote: L({}, I, "function" == typeof M.customRules.blockQuote ? M.customRules.blockQuote(e) : M.customRules.blockQuote),
                        paragraph: L({}, H, "function" == typeof M.customRules.paragraph ? M.customRules.paragraph(e) : M.customRules.paragraph)
                    })
                };
            const F = function(e) {
                return L({}, W(e))
            };
            var K = r(2590),
                Q = r(473708),
                V = r(397714),
                q = r.n(V),
                z = r(214468),
                X = r.n(z);

            function $(e, t) {
                if (!(e instanceof t)) throw new TypeError("Cannot call a class as a function")
            }

            function Y(e) {
                Y = Object.setPrototypeOf ? Object.getPrototypeOf : function(e) {
                    return e.__proto__ || Object.getPrototypeOf(e)
                };
                return Y(e)
            }

            function J(e, t) {
                return !t || "object" !== te(t) && "function" != typeof t ? function(e) {
                    if (void 0 === e) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
                    return e
                }(e) : t
            }

            function ee(e, t) {
                ee = Object.setPrototypeOf || function(e, t) {
                    e.__proto__ = t;
                    return e
                };
                return ee(e, t)
            }
            var te = function(e) {
                return e && "undefined" != typeof Symbol && e.constructor === Symbol ? "symbol" : typeof e
            };

            function re(e) {
                var t = function() {
                    if ("undefined" == typeof Reflect || !Reflect.construct) return !1;
                    if (Reflect.construct.sham) return !1;
                    if ("function" == typeof Proxy) return !0;
                    try {
                        Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], (function() {})));
                        return !0
                    } catch (e) {
                        return !1
                    }
                }();
                return function() {
                    var r, n = Y(e);
                    if (t) {
                        var o = Y(this).constructor;
                        r = Reflect.construct(n, arguments, o)
                    } else r = n.apply(this, arguments);
                    return J(this, r)
                }
            }
            var ne = O.Z.reactParserFor(F(q())),
                oe = {};
            const ce = function(e) {
                ! function(e, t) {
                    if ("function" != typeof t && null !== t) throw new TypeError("Super expression must either be null or a function");
                    e.prototype = Object.create(t && t.prototype, {
                        constructor: {
                            value: e,
                            writable: !0,
                            configurable: !0
                        }
                    });
                    t && ee(e, t)
                }(i, e);
                var t = re(i);

                function i() {
                    $(this, i);
                    var e;
                    (e = t.apply(this, arguments)).scrollerRef = o.createRef();
                    e.handleScroll = function() {
                        var t = e.scrollerRef.current;
                        if (null != t) {
                            var r, n;
                            null === (n = (r = e.props).onScroll) || void 0 === n || n.call(r, t)
                        }
                    };
                    e.trackLinkClick = function(t) {
                        e.props.track(K.rMx.CHANGE_LOG_CTA_CLICKED, {
                            cta_type: "inline_link",
                            target: t
                        }, !1)
                    };
                    e.renderHeader = function() {
                        var t = e.props.changeLog;
                        return (0, n.jsxs)(v.Z.Child, {
                            grow: 1,
                            shrink: 1,
                            children: [(0, n.jsx)(c.Heading, {
                                variant: "heading-lg/semibold",
                                children: Q.Z.Messages.WHATS_NEW
                            }), (0, n.jsx)(c.Text, {
                                variant: "text-xs/normal",
                                className: q().date,
                                children: Q.Z.Messages.WHATS_NEW_DATE.format({
                                    date: null != t.date && "" !== t.date ? h()(t.date).toDate() : new Date
                                })
                            })]
                        })
                    };
                    return e
                }
                var a = i.prototype;
                a.track = function(e, t, r) {
                    this.props.track(e, t, r)
                };
                a.renderFooter = function() {
                    return (0, n.jsxs)("div", {
                        className: q().footer,
                        children: [(0, n.jsx)(c.Anchor, {
                            "aria-label": Q.Z.Messages.TWITTER,
                            className: q().socialLink,
                            href: (0,
                                y.Z)(K.RK.TWITTER),
                            target: "blank",
                            children: (0, n.jsx)(x.Z, {})
                        }), (0, n.jsx)(c.Anchor, {
                            "aria-label": Q.Z.Messages.FACEBOOK,
                            className: q().socialLink,
                            href: K.fK7.FACEBOOK_URL,
                            target: "blank",
                            children: (0, n.jsx)(C.Z, {})
                        }), (0, n.jsx)(c.Anchor, {
                            "aria-label": Q.Z.Messages.INSTAGRAM,
                            className: q().socialLink,
                            href: K.fK7.INSTAGRAM_URL,
                            target: "blank",
                            children: (0, n.jsx)(w.Z, {})
                        }), (0, n.jsx)(c.Text, {
                            variant: "text-xs/normal",
                            children: Q.Z.Messages.FOLLOW_US_FOR_MORE_UPDATES
                        })]
                    })
                };
                a.renderVideo = function() {
                    var e = this,
                        t = this.props.changeLog,
                        o = 451,
                        c = 254,
                        i = t.youtube_video_id,
                        a = t.video,
                        l = t.image;
                    if (null != i && "" !== i) return (0, n.jsx)(j.BC, {
                        className: g()(q().video),
                        allowFullScreen: !1,
                        href: "https://youtu.be/".concat(i),
                        thumbnail: {
                            url: "https://i.ytimg.com/vi/".concat(i, "/maxresdefault.jpg"),
                            width: o,
                            height: c
                        },
                        video: {
                            url: "https://www.youtube.com/embed/".concat(i, "?vq=large&rel=0&controls=0&showinfo=0"),
                            width: o,
                            height: c
                        },
                        maxWidth: o,
                        maxHeight: c,
                        renderVideoComponent: m.lV,
                        renderImageComponent: m.Yi,
                        renderLinkComponent: m.iT,
                        onPlay: function() {
                            e.track(K.rMx.CHANGE_LOG_VIDEO_INTERACTED, {}, !0)
                        }
                    });
                    if (null != a && "" !== a) {
                        var s = a.startsWith("https://") ? a : r(478717)("./".concat(a));
                        return (0, n.jsx)(P.Z, {
                            src: s,
                            poster: l,
                            width: o,
                            height: c,
                            loop: !0,
                            muted: !0,
                            autoPlay: !0,
                            className: q().video,
                            onPlay: function() {
                                e.track(K.rMx.CHANGE_LOG_VIDEO_INTERACTED, {}, !0)
                            }
                        })
                    }
                    if (null != l && "" !== l) {
                        var u = l.startsWith("https://") ? l : r(478717)("./".concat(l));
                        return (0, n.jsx)("img", {
                            className: q().image,
                            alt: "",
                            src: u,
                            width: o,
                            height: c
                        })
                    }
                    return null
                };
                a.render = function() {
                    var e = this.props.changeLog;
                    return (0, n.jsxs)(c.ModalRoot, {
                        transitionState: c.ModalTransitionState.ENTERED,
                        className: X().modal,
                        children: [(0, n.jsxs)(c.ModalHeader, {
                            align: v.Z.Justify.BETWEEN,
                            separator: !1,
                            children: [this.renderHeader(), (0, n.jsx)(v.Z.Child, {
                                grow: 0,
                                children: (0, n.jsx)(c.ModalCloseButton, {
                                    onClick: this.props.onClose
                                })
                            })]
                        }), (0, n.jsx)(c.ModalContent, {
                            className: g()(X().content, q().container, {}),
                            scrollerRef: this.scrollerRef,
                            onScroll: this.handleScroll,
                            children: (0, n.jsxs)("div", {
                                tabIndex: 0,
                                role: "region",
                                "aria-label": Q.Z.Messages.CHANGELOG_CONTENT_LABEL,
                                children: [this.renderVideo(), ne(e.body, !1, {
                                    changeLog: this,
                                    interpolations: oe,
                                    onLinkClick: this.trackLinkClick
                                })]
                            })
                        }), (0, n.jsx)(c.ModalFooter, {
                            direction: v.Z.Direction.HORIZONTAL,
                            children: this.renderFooter()
                        })]
                    })
                };
                return i
            }(o.PureComponent);
            var ie = r(463652),
                ae = r.n(ie);

            function le(e, t) {
                if (!(e instanceof t)) throw new TypeError("Cannot call a class as a function")
            }

            function se(e, t, r) {
                t in e ? Object.defineProperty(e, t, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[t] = r;
                return e
            }

            function ue(e) {
                ue = Object.setPrototypeOf ? Object.getPrototypeOf : function(e) {
                    return e.__proto__ || Object.getPrototypeOf(e)
                };
                return ue(e)
            }

            function fe(e) {
                for (var t = 1; t < arguments.length; t++) {
                    var r = null != arguments[t] ? arguments[t] : {},
                        n = Object.keys(r);
                    "function" == typeof Object.getOwnPropertySymbols && (n = n.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                        return Object.getOwnPropertyDescriptor(r, e).enumerable
                    }))));
                    n.forEach((function(t) {
                        se(e, t, r[t])
                    }))
                }
                return e
            }

            function pe(e, t) {
                t = null != t ? t : {};
                Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : function(e, t) {
                    var r = Object.keys(e);
                    if (Object.getOwnPropertySymbols) {
                        var n = Object.getOwnPropertySymbols(e);
                        t && (n = n.filter((function(t) {
                            return Object.getOwnPropertyDescriptor(e, t).enumerable
                        })));
                        r.push.apply(r, n)
                    }
                    return r
                }(Object(t)).forEach((function(r) {
                    Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r))
                }));
                return e
            }

            function de(e, t) {
                return !t || "object" !== be(t) && "function" != typeof t ? function(e) {
                    if (void 0 === e) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
                    return e
                }(e) : t
            }

            function ge(e, t) {
                ge = Object.setPrototypeOf || function(e, t) {
                    e.__proto__ = t;
                    return e
                };
                return ge(e, t)
            }
            var be = function(e) {
                return e && "undefined" != typeof Symbol && e.constructor === Symbol ? "symbol" : typeof e
            };

            function he(e) {
                var t = function() {
                    if ("undefined" == typeof Reflect || !Reflect.construct) return !1;
                    if (Reflect.construct.sham) return !1;
                    if ("function" == typeof Proxy) return !0;
                    try {
                        Boolean.prototype.valueOf.call(Reflect.construct(Boolean, [], (function() {})));
                        return !0
                    } catch (e) {
                        return !1
                    }
                }();
                return function() {
                    var r, n = ue(e);
                    if (t) {
                        var o = ue(this).constructor;
                        r = Reflect.construct(n, arguments, o)
                    } else r = n.apply(this, arguments);
                    return de(this, r)
                }
            }
            var ye = function(e) {
                ! function(e, t) {
                    if ("function" != typeof t && null !== t) throw new TypeError("Super expression must either be null or a function");
                    e.prototype = Object.create(t && t.prototype, {
                        constructor: {
                            value: e,
                            writable: !0,
                            configurable: !0
                        }
                    });
                    t && ge(e, t)
                }(r, e);
                var t = he(r);

                function r() {
                    le(this, r);
                    var e;
                    (e = t.apply(this, arguments)).maxScrolledPercentage = 0;
                    e.mountedAt = 0;
                    e.close = function() {
                        (0, c.closeModal)(u.Xd)
                    };
                    e.handleScroll = function(t) {
                        var r = t.getScrollerState(),
                            n = r.offsetHeight,
                            o = r.scrollHeight,
                            c = r.scrollTop / (o - n);
                        e.maxScrolledPercentage = Math.min(Math.max(e.maxScrolledPercentage, c), 1)
                    };
                    e.track = function(t) {
                        var r = arguments.length > 1 && void 0 !== arguments[1] ? arguments[1] : {},
                            n = arguments.length > 2 && void 0 !== arguments[2] && arguments[2],
                            o = e.props.changeLog,
                            c = o.date,
                            i = o.revision,
                            a = fe({
                                change_log_id: "".concat(c, ":").concat(i)
                            }, r);
                        n || (a = fe({
                            seconds_open: Math.round((Date.now() - e.mountedAt) / 1e3),
                            max_scrolled_percentage: 100 * parseInt(e.maxScrolledPercentage.toPrecision(4), 10)
                        }, a));
                        p.default.track(t, a)
                    };
                    return e
                }
                var o = r.prototype;
                o.componentDidMount = function() {
                    this.mountedAt = Date.now();
                    this.maxScrolledPercentage = 0;
                    this.track(K.rMx.CHANGE_LOG_OPENED, {}, !0)
                };
                o.componentWillUnmount = function() {
                    this.track(K.rMx.CHANGE_LOG_CLOSED)
                };
                o.render = function() {
                    return (0, n.jsx)(ce, pe(fe({}, this.props), {
                        onClose: this.close,
                        onScroll: this.handleScroll,
                        track: this.track
                    }))
                };
                return r
            }(o.PureComponent);

            function Oe(e) {
                var t = function() {
                        var e = (0, a.ZP)([l.default], (function() {
                                return l.default.locale
                            })),
                            t = (0, a.ZP)([s.Z], (function() {
                                return s.Z.latestChangelogId()
                            })),
                            r = (0, a.ZP)([s.Z], (function() {
                                return s.Z.getConfig()
                            })),
                            n = null != r && 0 === Object.keys(r).length,
                            o = null != r && Object.keys(r).length > 0 && null == t,
                            c = (0, a.ZP)([s.Z], (function() {
                                return s.Z.overrideId()
                            })),
                            i = f(t, e),
                            u = i.changelog,
                            p = i.loaded,
                            d = f(c, e),
                            g = d.changelog,
                            b = d.loaded;
                        return null == c || null == g && b ? {
                            id: t,
                            changelog: u,
                            loaded: !!n || p,
                            clientTooOld: o
                        } : {
                            id: c,
                            changelog: g,
                            loaded: b,
                            clientTooOld: !1
                        }
                    }(),
                    r = t.changelog,
                    u = t.loaded,
                    p = t.clientTooOld;
                o.useEffect((function() {
                    if (u && null != r) return function() {
                        return i.Z.markChangelogAsSeen(r.id, r.date)
                    }
                }), [u, r]);
                return p ? (0, n.jsx)(c.ModalRoot, {
                    transitionState: e.transitionState,
                    children: (0, n.jsx)("div", {
                        className: ae().empty,
                        children: (0, n.jsx)(c.Heading, {
                            variant: "heading-lg/medium",
                            children: Q.Z.Messages.CHANGE_LOG_CLIENT_OLD
                        })
                    })
                }) : null == r ? u ? (0, n.jsx)(c.ModalRoot, {
                    transitionState: e.transitionState,
                    children: (0, n.jsx)("div", {
                        className: ae().empty,
                        children: (0, n.jsx)(c.Text, {
                            variant: "text-md/semibold",
                            children: Q.Z.Messages.CHANGE_LOG_FAILED_TO_LOAD
                        })
                    })
                }) : (0, n.jsx)(c.ModalRoot, {
                    transitionState: e.transitionState,
                    children: (0,
                        n.jsx)("div", {
                        className: ae().empty,
                        children: (0, n.jsx)(c.Spinner, {
                            type: c.Spinner.Type.WANDERING_CUBES
                        })
                    })
                }) : (0, n.jsx)(c.ModalRoot, {
                    transitionState: e.transitionState,
                    children: (0, n.jsx)(ye, {
                        changeLog: r
                    })
                })
            }
        },
        367299: (e, t, r) => {
            "use strict";
            r.d(t, {
                Z: () => i
            });
            var n = r(785893),
                o = (r(667294), r(318715)),
                c = r(785085);

            function i(e) {
                var t = e.children,
                    r = e.confettiLocation;
                return (0, o.ZP)([c.Z], (function() {
                    return c.Z.isEnabled({
                        confettiLocation: r
                    })
                })) ? (0, n.jsx)(n.Fragment, {
                    children: t
                }) : null
            }
        },
        919031: (e, t, r) => {
            "use strict";
            r.d(t, {
                Z: () => l
            });
            var n = r(667294),
                o = r(318715),
                c = r(255875),
                i = r(785085);

            function a(e, t, r) {
                t in e ? Object.defineProperty(e, t, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[t] = r;
                return e
            }

            function l() {
                var e = n.useContext(c.E).createMultipleConfettiAt,
                    t = (0, o.ZP)([i.Z], (function() {
                        return i.Z.getState()
                    })),
                    r = n.useCallback((function(e) {
                        return {
                            size: {
                                type: "static-random",
                                minValue: e.confettiSize - 7,
                                maxValue: e.confettiSize + 7
                            }
                        }
                    }), []),
                    l = n.useMemo((function() {
                        return {
                            fire: function(n, o, c) {
                                var i, l, s = null != (null == c ? void 0 : c.settings) ? function(e) {
                                        for (var t = 1; t < arguments.length; t++) {
                                            var r = null != arguments[t] ? arguments[t] : {},
                                                n = Object.keys(r);
                                            "function" == typeof Object.getOwnPropertySymbols && (n = n.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                                                return Object.getOwnPropertyDescriptor(r, e).enumerable
                                            }))));
                                            n.forEach((function(t) {
                                                a(e, t, r[t])
                                            }))
                                        }
                                        return e
                                    }({}, t, c.settings) : t,
                                    u = r(s);
                                e(n, o, u, (null !== (i = null == c ? void 0 : c.count) && void 0 !== i ? i : s.confettiCount) * (null !== (l = null == c ? void 0 : c.countMultiplier) && void 0 !== l ? l : 1), {
                                    sprite: null == c ? void 0 : c.sprite
                                })
                            }
                        }
                    }), [e, r, t]);
                return l
            }
        },
        319463: (e, t, r) => {
            "use strict";
            r.d(t, {
                Z: () => l
            });
            var n = r(785893),
                o = (r(667294), r(633878));

            function c(e, t, r) {
                t in e ? Object.defineProperty(e, t, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[t] = r;
                return e
            }

            function i(e, t) {
                t = null != t ? t : {};
                Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : function(e, t) {
                    var r = Object.keys(e);
                    if (Object.getOwnPropertySymbols) {
                        var n = Object.getOwnPropertySymbols(e);
                        t && (n = n.filter((function(t) {
                            return Object.getOwnPropertyDescriptor(e, t).enumerable
                        })));
                        r.push.apply(r, n)
                    }
                    return r
                }(Object(t)).forEach((function(r) {
                    Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r))
                }));
                return e
            }

            function a(e, t) {
                if (null == e) return {};
                var r, n, o = function(e, t) {
                    if (null == e) return {};
                    var r, n, o = {},
                        c = Object.keys(e);
                    for (n = 0; n < c.length; n++) {
                        r = c[n];
                        t.indexOf(r) >= 0 || (o[r] = e[r])
                    }
                    return o
                }(e, t);
                if (Object.getOwnPropertySymbols) {
                    var c = Object.getOwnPropertySymbols(e);
                    for (n = 0; n < c.length; n++) {
                        r = c[n];
                        t.indexOf(r) >= 0 || Object.prototype.propertyIsEnumerable.call(e, r) && (o[r] = e[r])
                    }
                }
                return o
            }

            function l(e) {
                var t = e.width,
                    r = void 0 === t ? 16 : t,
                    l = e.height,
                    s = void 0 === l ? 16 : l,
                    u = e.color,
                    f = void 0 === u ? "currentColor" : u,
                    p = e.className,
                    d = e.foreground,
                    g = a(e, ["width", "height", "color", "className", "foreground"]);
                return (0, n.jsx)("svg", i(function(e) {
                    for (var t = 1; t < arguments.length; t++) {
                        var r = null != arguments[t] ? arguments[t] : {},
                            n = Object.keys(r);
                        "function" == typeof Object.getOwnPropertySymbols && (n = n.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                            return Object.getOwnPropertyDescriptor(r, e).enumerable
                        }))));
                        n.forEach((function(t) {
                            c(e, t, r[t])
                        }))
                    }
                    return e
                }({
                    className: p,
                    width: r,
                    height: s,
                    viewBox: "0 0 16 16"
                }, (0, o.Z)(g)), {
                    children: (0, n.jsxs)("g", {
                        fill: "none",
                        fillRule: "evenodd",
                        children: [(0, n.jsx)("path", {
                            className: d,
                            fill: f,
                            d: "M0,1.99406028 C0,0.892771196 0.894513756,0 1.99406028,0 L14.0059397,0 C15.1072288,0 16,0.894513756 16,1.99406028 L16,14.0059397 C16,15.1072288 15.1054862,16 14.0059397,16 L1.99406028,16 C0.892771196,16 0,15.1054862 0,14.0059397 L0,1.99406028 Z M8.23182341,16 L10.3991764,16 L10.3991764,9.93141161 L12.5663127,9.93141161 L13,7.76405862 L10.3991764,7.76405862 L10.3246195,6.3468265 C10.3246195,5.66107601 10.5049432,5.17342158 11.4698488,5.17342158 L12.974642,5.17385505 L12.974642,3.12202197 C12.7618079,3.09319618 12.3142495,3 11.4644304,3 C9.69001851,3 8.18500859,4.20353112 8.18500859,6.23043964 L8.23182341,7.76405862 L6.06425368,7.76405862 L6.06425368,9.93141161 L8.23182341,9.93141161 L8.23182341,16 Z"
                        }), (0, n.jsx)("rect", {
                            width: "16",
                            height: "16"
                        })]
                    })
                }))
            }
        },
        750349: (e, t, r) => {
            "use strict";
            r.d(t, {
                Z: () => l
            });
            var n = r(785893),
                o = (r(667294), r(633878));

            function c(e, t, r) {
                t in e ? Object.defineProperty(e, t, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[t] = r;
                return e
            }

            function i(e, t) {
                t = null != t ? t : {};
                Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : function(e, t) {
                    var r = Object.keys(e);
                    if (Object.getOwnPropertySymbols) {
                        var n = Object.getOwnPropertySymbols(e);
                        t && (n = n.filter((function(t) {
                            return Object.getOwnPropertyDescriptor(e, t).enumerable
                        })));
                        r.push.apply(r, n)
                    }
                    return r
                }(Object(t)).forEach((function(r) {
                    Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r))
                }));
                return e
            }

            function a(e, t) {
                if (null == e) return {};
                var r, n, o = function(e, t) {
                    if (null == e) return {};
                    var r, n, o = {},
                        c = Object.keys(e);
                    for (n = 0; n < c.length; n++) {
                        r = c[n];
                        t.indexOf(r) >= 0 || (o[r] = e[r])
                    }
                    return o
                }(e, t);
                if (Object.getOwnPropertySymbols) {
                    var c = Object.getOwnPropertySymbols(e);
                    for (n = 0; n < c.length; n++) {
                        r = c[n];
                        t.indexOf(r) >= 0 || Object.prototype.propertyIsEnumerable.call(e, r) && (o[r] = e[r])
                    }
                }
                return o
            }

            function l(e) {
                var t = e.width,
                    r = void 0 === t ? 16 : t,
                    l = e.height,
                    s = void 0 === l ? 16 : l,
                    u = e.color,
                    f = void 0 === u ? "currentColor" : u,
                    p = e.className,
                    d = e.foreground,
                    g = a(e, ["width", "height", "color", "className", "foreground"]);
                return (0, n.jsx)("svg", i(function(e) {
                    for (var t = 1; t < arguments.length; t++) {
                        var r = null != arguments[t] ? arguments[t] : {},
                            n = Object.keys(r);
                        "function" == typeof Object.getOwnPropertySymbols && (n = n.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                            return Object.getOwnPropertyDescriptor(r, e).enumerable
                        }))));
                        n.forEach((function(t) {
                            c(e, t, r[t])
                        }))
                    }
                    return e
                }({
                    className: p,
                    xmlns: "http://www.w3.org/2000/svg",
                    width: r,
                    height: s,
                    viewBox: "0 0 16 16"
                }, (0, o.Z)(g)), {
                    children: (0, n.jsxs)("g", {
                        fill: "none",
                        fillRule: "evenodd",
                        children: [(0, n.jsx)("path", {
                            className: d,
                            fill: f,
                            d: "M4.70012698,0.0531746 C3.84888888,0.092 3.2675238,0.22720635 2.7588254,0.42492063 C2.23292063,0.62926983 1.78692063,0.90273016 1.3422857,1.34733333 C0.89768254,1.79196825 0.62422222,2.23796825 0.41987302,2.76387303 C0.22215872,3.27257143 0.08695238,3.85393651 0.04812698,4.7051746 C0.00920635,5.55819048 0,5.83053968 0,8.00253968 C0,10.1745079 0.00920635,10.4468571 0.04812698,11.299873 C0.08695238,12.1511111 0.22215873,12.7324762 0.41987302,13.2411746 C0.62422222,13.7670794 0.89768254,14.2130794 1.34228572,14.6577143 C1.78692062,15.1023175 2.23292062,15.3757778 2.7588254,15.5801587 C3.2675238,15.7778413 3.8488889,15.9130476 4.70012698,15.951873 C5.55314286,15.9907937 5.82549206,16 7.99749206,16 C10.1694603,16 10.4418095,15.9907937 11.2948254,15.951873 C12.1460635,15.9130476 12.7274286,15.7778413 13.236127,15.5801587 C13.7620317,15.3757778 14.2080317,15.1023175 14.6526667,14.6577143 C15.0972698,14.2130794 15.3707302,13.7670794 15.5751111,13.2411746 C15.7727937,12.7324762 15.908,12.1511111 15.9468254,11.299873 C15.985746,10.4468571 15.9949524,10.1745079 15.9949524,8.00253968 C15.9949524,5.83053968 15.985746,5.55819048 15.9468254,4.7051746 C15.908,3.8539365 15.7727937,3.27257143 15.5751111,2.76387302 C15.3707302,2.23796825 15.0972698,1.79196825 14.6526667,1.34733332 C14.2080317,0.90273016 13.7620317,0.62926984 13.236127,0.42492064 C12.7274286,0.22720634 12.1460635,0.092 11.2948254,0.0531746 C10.4418095,0.01425397 10.1694603,0.00504762 7.99749206,0.00504762 C5.82549206,0.00504762 5.55314286,0.01425397 4.70012698,0.0531746 L4.70012698,0.0531746 Z M8.00001453,2.00504762 C9.95416635,2.00504762 10.185649,2.01251386 10.9573741,2.04772432 C11.6709381,2.08026206 12.0584565,2.19948958 12.3163471,2.29971739 C12.6579641,2.43248289 12.9017646,2.59107525 13.157854,2.84719363 C13.4139723,3.10328295 13.5725647,3.34708347 13.7053302,3.68870053 C13.805558,3.94659105 13.9247856,4.33410953 13.9573233,5.04767346 C13.9925338,5.8193986 14,6.05088127 14,8.00506213 C14,9.95921396 13.9925338,10.1906966 13.9573233,10.9624217 C13.9247856,11.6759857 13.805558,12.0635042 13.7053302,12.3213947 C13.5725647,12.6630117 13.4139723,12.9068123 13.157854,13.1629016 C12.9017646,13.41902 12.6579641,13.5776123 12.3163471,13.7103779 C12.0584565,13.8106057 11.6709381,13.9298332 10.9573741,13.9623709 C10.1857652,13.9975814 9.9543116,14.0050476 8.00001453,14.0050476 C6.04568839,14.0050476 5.81426383,13.9975814 5.04262587,13.9623709 C4.32906195,13.9298332 3.94154347,13.8106057 3.68365294,13.7103779 C3.34203588,13.5776123 3.09823536,13.41902 2.84214604,13.1629016 C2.58605671,12.9068123 2.42743531,12.6630117 2.29466977,12.3213947 C2.19444197,12.0635042 2.07521444,11.6759857 2.04267671,10.9624217 C2.00746628,10.1906966 2,9.95921395 2,8.00506212 C2,6.05088125 2.00746625,5.81939858 2.0426767,5.04767347 C2.07521444,4.33410953 2.19444196,3.94659104 2.29466977,3.68870052 C2.42743531,3.34708346 2.58602767,3.10328294 2.84214604,2.84719362 C3.09823536,2.59107524 3.34203588,2.43248288 3.68365294,2.29971735 C3.94154346,2.19948953 4.32906194,2.08026201 5.04262587,2.04772428 C5.81435097,2.01251381 6.04583365,2.00504758 8.00001453,2.00504758 L8.00001453,2.00504762 Z"
                        }), (0, n.jsx)("path", {
                            className: d,
                            fill: f,
                            d: "M8.0000119,10 C6.89542535,10 6,9.10457466 6,8.0000119 C6,6.89542535 6.89542534,6 8.0000119,6 C9.10457467,6 10,6.89542534 10,8.0000119 C10,9.10457467 9.10457466,10 8.0000119,10 L8.0000119,10 Z M8.00001546,4 C5.7908468,4 4,5.7908468 4,8.00001546 C4,10.2091532 5.7908468,12 8.00001546,12 C10.2091532,12 12,10.2091532 12,8.00001546 C12,5.7908468 10.2091532,4 8.00001546,4 L8.00001546,4 Z M13,4.00001654 C13,4.55230644 12.5522734,5 11.9999835,5 C11.4477266,5 11,4.55230645 11,4.00001654 C11,3.44772664 11.4477266,3 11.9999835,3 C12.5522734,3 13,3.44772663 13,4.00001654 L13,4.00001654 Z"
                        }), (0, n.jsx)("rect", {
                            width: "16",
                            height: "16"
                        })]
                    })
                }))
            }
        },
        313197: (e, t, r) => {
            "use strict";
            r.d(t, {
                Z: () => l
            });
            var n = r(785893),
                o = (r(667294), r(633878));

            function c(e, t, r) {
                t in e ? Object.defineProperty(e, t, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[t] = r;
                return e
            }

            function i(e, t) {
                t = null != t ? t : {};
                Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : function(e, t) {
                    var r = Object.keys(e);
                    if (Object.getOwnPropertySymbols) {
                        var n = Object.getOwnPropertySymbols(e);
                        t && (n = n.filter((function(t) {
                            return Object.getOwnPropertyDescriptor(e, t).enumerable
                        })));
                        r.push.apply(r, n)
                    }
                    return r
                }(Object(t)).forEach((function(r) {
                    Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r))
                }));
                return e
            }

            function a(e, t) {
                if (null == e) return {};
                var r, n, o = function(e, t) {
                    if (null == e) return {};
                    var r, n, o = {},
                        c = Object.keys(e);
                    for (n = 0; n < c.length; n++) {
                        r = c[n];
                        t.indexOf(r) >= 0 || (o[r] = e[r])
                    }
                    return o
                }(e, t);
                if (Object.getOwnPropertySymbols) {
                    var c = Object.getOwnPropertySymbols(e);
                    for (n = 0; n < c.length; n++) {
                        r = c[n];
                        t.indexOf(r) >= 0 || Object.prototype.propertyIsEnumerable.call(e, r) && (o[r] = e[r])
                    }
                }
                return o
            }

            function l(e) {
                var t = e.width,
                    r = void 0 === t ? 20 : t,
                    l = e.height,
                    s = void 0 === l ? 16 : l,
                    u = e.color,
                    f = void 0 === u ? "currentColor" : u,
                    p = e.className,
                    d = e.foreground,
                    g = a(e, ["width", "height", "color", "className", "foreground"]);
                return (0, n.jsx)("svg", i(function(e) {
                    for (var t = 1; t < arguments.length; t++) {
                        var r = null != arguments[t] ? arguments[t] : {},
                            n = Object.keys(r);
                        "function" == typeof Object.getOwnPropertySymbols && (n = n.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                            return Object.getOwnPropertyDescriptor(r, e).enumerable
                        }))));
                        n.forEach((function(t) {
                            c(e, t, r[t])
                        }))
                    }
                    return e
                }({
                    className: p,
                    width: r,
                    height: s,
                    viewBox: "0 0 20 16"
                }, (0, o.Z)(g)), {
                    children: (0, n.jsxs)("g", {
                        fill: "none",
                        fillRule: "evenodd",
                        children: [(0, n.jsx)("path", {
                            className: d,
                            fill: f,
                            d: "M1,14.1538462 L1.95,14.1538462 C3.73125,14.1538462 5.5125,13.5384615 6.81875,12.4307692 C5.15625,12.4307692 3.73125,11.2 3.1375,9.6 C3.375,9.6 3.6125,9.72307692 3.85,9.72307692 C4.20625,9.72307692 4.5625,9.72307692 4.91875,9.6 C3.1375,9.23076923 1.7125,7.63076923 1.7125,5.66153846 C2.1875,5.90769231 2.78125,6.15384615 3.49375,6.15384615 C2.425,5.41538462 1.83125,4.18461538 1.83125,2.70769231 C1.83125,1.96923077 2.06875,1.23076923 2.30625,0.615384615 C4.20625,3.07692308 7.05625,4.67692308 10.38125,4.8 C10.2625,4.67692308 10.2625,4.30769231 10.2625,4.06153846 C10.2625,1.84615385 12.04375,0 14.18125,0 C15.25,0 16.31875,0.492307692 17.03125,1.23076923 C17.8625,1.10769231 18.8125,0.738461538 19.525,0.246153846 C19.2875,1.23076923 18.575,1.96923077 17.8625,2.46153846 C18.575,2.46153846 19.2875,2.21538462 20,1.84615385 C19.525,2.70769231 18.8125,3.32307692 18.1,3.93846154 L18.1,4.43076923 C18.1,9.84615385 14.18125,16 6.9375,16 C4.68125,16 2.6625,15.3846154 1,14.1538462 Z"
                        }), (0,
                            n.jsx)("rect", {
                            width: "20",
                            height: "16"
                        })]
                    })
                }))
            }
        },
        641596: (e, t, r) => {
            "use strict";
            r.r(t);
            r.d(t, {
                baseRules: () => x,
                customRules: () => P
            });
            var n = r(785893),
                o = r(667294),
                c = r(294184),
                i = r.n(c),
                a = r(120053),
                l = r.n(a),
                s = r(882723),
                u = r(734691),
                f = r(883064),
                p = r(652591),
                d = r(2590),
                g = r(907784);

            function b(e, t) {
                (null == t || t > e.length) && (t = e.length);
                for (var r = 0, n = new Array(t); r < t; r++) n[r] = e[r];
                return n
            }

            function h(e, t, r) {
                t in e ? Object.defineProperty(e, t, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[t] = r;
                return e
            }

            function y(e) {
                for (var t = 1; t < arguments.length; t++) {
                    var r = null != arguments[t] ? arguments[t] : {},
                        n = Object.keys(r);
                    "function" == typeof Object.getOwnPropertySymbols && (n = n.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                        return Object.getOwnPropertyDescriptor(r, e).enumerable
                    }))));
                    n.forEach((function(t) {
                        h(e, t, r[t])
                    }))
                }
                return e
            }

            function O(e, t) {
                t = null != t ? t : {};
                Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(t)) : function(e, t) {
                    var r = Object.keys(e);
                    if (Object.getOwnPropertySymbols) {
                        var n = Object.getOwnPropertySymbols(e);
                        t && (n = n.filter((function(t) {
                            return Object.getOwnPropertyDescriptor(e, t).enumerable
                        })));
                        r.push.apply(r, n)
                    }
                    return r
                }(Object(t)).forEach((function(r) {
                    Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(t, r))
                }));
                return e
            }

            function m(e) {
                return function(e) {
                    if (Array.isArray(e)) return b(e)
                }(e) || function(e) {
                    if ("undefined" != typeof Symbol && null != e[Symbol.iterator] || null != e["@@iterator"]) return Array.from(e)
                }(e) || function(e, t) {
                    if (!e) return;
                    if ("string" == typeof e) return b(e, t);
                    var r = Object.prototype.toString.call(e).slice(8, -1);
                    "Object" === r && e.constructor && (r = e.constructor.name);
                    if ("Map" === r || "Set" === r) return Array.from(r);
                    if ("Arguments" === r || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r)) return b(e, t)
                }(e) || function() {
                    throw new TypeError("Invalid attempt to spread non-iterable instance.\\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.")
                }()
            }
            var j = l().defaultRules.link,
                v = {
                    section: d.jXE.SETTINGS_CHANGELOG
                };

            function C(e, t) {
                return null == t ? [] : t.split(" ").map((function(t) {
                    return e[t]
                }))
            }
            var w = function(e) {
                    var t = e.level,
                        r = e.children,
                        n = e.className,
                        c = void 0 === n ? null : n,
                        a = e.styleSheet,
                        l = void 0 === a ? {} : a,
                        u = (0, s.usePrivateHeadingLevel)(),
                        f = parseInt(t, 10),
                        p = u + (isNaN(f) ? 1 : f) - 1;
                    return o.createElement("h".concat(p), {
                        className: i().apply(void 0, m(C(l, c)))
                    }, r)
                },
                x = f.Z.defaultRules,
                P = {
                    link: {
                        parse: function(e, t, r) {
                            var n, o = e[2],
                                c = o.startsWith("https://discordapp.com/nitro") || o.startsWith("https://discord.com/nitro"),
                                i = o.startsWith("https://discordapp.com/hypesquad") || o.startsWith("https://discord.com/hypesquad");
                            n = c || i ? function(e) {
                                p.default.track(d.rMx.PREMIUM_PROMOTION_OPENED, {
                                    location: v
                                });
                                c ? u.Z.open(d.oAB.PREMIUM) : i && u.Z.open(d.oAB.HYPESQUAD_ONLINE);
                                r.changeLog.track(d.rMx.CHANGE_LOG_CTA_CLICKED, {
                                    cta_type: "nitro"
                                });
                                (0, s.closeModal)(g.Xd);
                                e.preventDefault()
                            } : function() {
                                r && "function" == typeof r.onLinkClick && r.onLinkClick(o);
                                r.changeLog.track(d.rMx.CHANGE_LOG_CTA_CLICKED, y({}, p.default.getCampaignParams(o)))
                            };
                            return O(y({}, j.parse(e, t, r)), {
                                callToAction: n
                            })
                        },
                        react: function(e, t, r) {
                            return (0, n.jsx)(s.Anchor, {
                                href: l().sanitizeUrl(e.target),
                                title: e.title,
                                onClick: e.callToAction,
                                target: "_blank",
                                className: e.callToAction ? "cta" : void 0,
                                children: t(e.content, r)
                            }, r.key)
                        }
                    },
                    lheading: function(e) {
                        return {
                            react: function(t, r, o) {
                                return (0, n.jsx)(w, {
                                    level: t.level,
                                    className: t.className,
                                    styleSheet: e,
                                    children: r(t.content, o)
                                }, o.key)
                            }
                        }
                    },
                    heading: {
                        react: function(e, t, r) {
                            return (0, n.jsx)(w, {
                                level: e.level,
                                children: t(e.content, r)
                            }, r.key)
                        }
                    },
                    image: {
                        react: function(e, t, o) {
                            var c = r(478717)("./".concat(e.target));
                            return (0, n.jsx)("img", {
                                alt: e.alt,
                                src: c
                            }, o.key)
                        }
                    },
                    blockQuote: {
                        react: x.blockQuote.react
                    },
                    paragraph: {
                        react: function(e, t, r) {
                            return (0, n.jsx)("p", {
                                children: t(e.content, r)
                            }, r.key)
                        }
                    }
                }
        }
    }
]);
//# sourceMappingURL=0fa8cdf24b0307f2405d.js.map