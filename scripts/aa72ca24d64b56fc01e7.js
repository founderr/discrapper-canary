"use strict";
(this.webpackChunkdiscord_app = this.webpackChunkdiscord_app || []).push([
    [74544], {
        383495: (e, n, r) => {
            r.d(n, {
                Q: () => I,
                Z: () => P
            });
            var t = r(785893),
                i = r(667294),
                a = r(294184),
                o = r.n(a),
                l = r(202351),
                c = r(70418),
                s = r(296916),
                u = r(344832),
                d = r(787193),
                f = r(682776),
                h = r(124251),
                p = r(746103),
                v = r(599467),
                y = r(619584),
                b = r(951687),
                m = r(699296),
                g = r(220109),
                x = r(26601),
                C = r(173764),
                E = r(3155),
                N = r(2590),
                w = r(473708),
                k = r(919569),
                S = r.n(k);

            function j(e, n, r) {
                n in e ? Object.defineProperty(e, n, {
                    value: r,
                    enumerable: !0,
                    configurable: !0,
                    writable: !0
                }) : e[n] = r;
                return e
            }

            function A(e) {
                for (var n = 1; n < arguments.length; n++) {
                    var r = null != arguments[n] ? arguments[n] : {},
                        t = Object.keys(r);
                    "function" == typeof Object.getOwnPropertySymbols && (t = t.concat(Object.getOwnPropertySymbols(r).filter((function(e) {
                        return Object.getOwnPropertyDescriptor(r, e).enumerable
                    }))));
                    t.forEach((function(n) {
                        j(e, n, r[n])
                    }))
                }
                return e
            }

            function T(e, n) {
                n = null != n ? n : {};
                Object.getOwnPropertyDescriptors ? Object.defineProperties(e, Object.getOwnPropertyDescriptors(n)) : function(e, n) {
                    var r = Object.keys(e);
                    if (Object.getOwnPropertySymbols) {
                        var t = Object.getOwnPropertySymbols(e);
                        n && (t = t.filter((function(n) {
                            return Object.getOwnPropertyDescriptor(e, n).enumerable
                        })));
                        r.push.apply(r, t)
                    }
                    return r
                }(Object(n)).forEach((function(r) {
                    Object.defineProperty(e, r, Object.getOwnPropertyDescriptor(n, r))
                }));
                return e
            }
            var I;
            ! function(e) {
                e[e.BANNER = 0] = "BANNER";
                e[e.THUMBNAIL = 1] = "THUMBNAIL"
            }(I || (I = {}));

            function _(e) {
                var n = e.channel,
                    r = e.onClick,
                    a = (0, d.XJ)(n).canManageAllEvents,
                    h = (0, l.e7)([f.Z], (function() {
                        return !n.isGuildVocal() || f.Z.can(N.Plq.CONNECT, n)
                    }), [n]),
                    p = i.useMemo((function() {
                        return (0, s.Z)(n)
                    }), [n]),
                    v = (0, u.KS)(n);
                return (0, t.jsx)(c.Tooltip, {
                    text: w.Z.Messages.GUILD_EVENT_CANNOT_CONNECT,
                    shouldShow: !h && null != r,
                    children: function(e) {
                        var i;
                        return (0, t.jsxs)(c.Clickable, T(A({}, e), {
                            className: o()(S().inline, S().channelContainer, (i = {}, j(i, S().channelContainerEnabled, h && null != r),
                                j(i, S().channelContainerDisabled, !h && null != r), i)),
                            onClick: r,
                            children: [(0, t.jsx)(c.Tooltip, {
                                text: w.Z.Messages.GUILD_EVENT_PRIVATE_CHANNEL_TOOLTIP,
                                shouldShow: a && p && h && null != r,
                                children: function(e) {
                                    return null != v ? (0, t.jsx)(v, T(A({}, e), {
                                        width: 20,
                                        height: 20,
                                        className: S().icon
                                    })) : null
                                }
                            }), (0, t.jsx)(c.Text, {
                                variant: "text-sm/normal",
                                color: "none",
                                className: S().channelLocation,
                                children: n.name
                            })]
                        }))
                    }
                })
            }

            function O(e) {
                var n = e.guild,
                    r = e.channel,
                    i = e.onJoinClick,
                    a = e.handleLocationClick,
                    l = e.location,
                    s = e.isExternal;
                return e.isHub ? null == n ? (0, t.jsx)("div", {}) : (0, t.jsxs)("div", {
                    className: S().inline,
                    children: [(0, t.jsx)(h.Z, {
                        className: S().guildIcon,
                        size: h.Z.Sizes.MINI,
                        active: !0,
                        guild: n
                    }), (0, t.jsx)(c.Text, {
                        variant: "text-sm/normal",
                        color: "header-secondary",
                        children: null == n ? void 0 : n.name
                    })]
                }) : null != r ? (0, t.jsx)(_, {
                    channel: r,
                    onClick: i
                }) : (0, t.jsxs)(c.Clickable, {
                    className: S().inline,
                    onClick: a,
                    children: [(0, t.jsx)(p.Z, {
                        height: 20,
                        width: 20,
                        className: o()(S().channelContainer, S().icon)
                    }), (0, t.jsx)(c.Text, {
                        className: s ? S().externalLocation : S().channelLocation,
                        variant: "text-sm/normal",
                        children: (0, v.m)(l, !0)
                    })]
                })
            }

            function P(e) {
                var n, r = e.className,
                    i = e.guild,
                    a = e.channel,
                    l = e.creator,
                    s = e.name,
                    u = e.entityType,
                    d = e.description,
                    f = e.imageLocation,
                    h = void 0 === f ? I.BANNER : f,
                    p = e.imageSource,
                    v = e.isActive,
                    N = e.isUserLurking,
                    w = e.isJoined,
                    k = void 0 !== w && w,
                    A = e.isMember,
                    T = void 0 !== A && A,
                    _ = e.isHub,
                    P = void 0 !== _ && _,
                    L = e.speakers,
                    Z = e.speakerCount,
                    M = e.rsvped,
                    G = e.canInvite,
                    R = e.location,
                    U = e.truncate,
                    B = e.onContextMenu,
                    V = e.onJoinClick,
                    D = e.onJoinGuildClick,
                    X = e.onGoToGuildClick,
                    W = e.onRsvpClick,
                    H = e.onStartClick,
                    J = e.onInviteClick,
                    Y = e.onEndClick,
                    K = e.onClick,
                    z = e.isNew,
                    q = e.guildEventId,
                    F = e.eventPreview,
                    $ = (0, y.Q)(a, u),
                    Q = u === E.WX.EXTERNAL,
                    ee = Q ? function(e) {
                        return e.stopPropagation()
                    } : void 0;
                return (0, t.jsxs)(c.ClickableContainer, {
                    "aria-label": s,
                    onClick: K,
                    onContextMenu: B,
                    className: o()(S().card, (n = {}, j(n, S().joined, k), j(n, S().lurking, N), n), r),
                    children: [h === I.BANNER && (0, t.jsx)(m.Z, {
                        source: p
                    }), (0, t.jsx)(g.ZP, {
                        creator: l,
                        name: s,
                        description: d,
                        imageSource: h === I.THUMBNAIL ? p : null,
                        truncate: U,
                        guildId: null == i ? void 0 : i.id,
                        isHub: P,
                        isNew: z,
                        guildEventId: q,
                        eventPreview: F
                    }), v && null != i && null != L && Z > 0 && (0, t.jsx)(C.Z, {
                        guild: i,
                        speakers: L,
                        speakerCount: Z,
                        className: S().spacing
                    }), (0, t.jsx)("hr", {
                        className: S().divider
                    }), (0, t.jsxs)("div", {
                        className: o()(S().inline, S().footer),
                        children: [(0, t.jsx)(O, {
                            guild: i,
                            channel: a,
                            onJoinClick: V,
                            handleLocationClick: ee,
                            location: R,
                            isExternal: Q,
                            isHub: P
                        }), P ? (0, t.jsx)(x.Z, {
                            isActive: v,
                            isUserLurking: N,
                            isMember: T,
                            rsvped: M,
                            onRsvpClick: W,
                            onJoinGuildClick: D,
                            onGoToGuildClick: X,
                            guildName: null == i ? void 0 : i.name,
                            canInvite: G,
                            isChannelPublic: $,
                            onInviteClick: J
                        }) : (0, t.jsx)(b.ZP, {
                            entityType: u,
                            isJoined: k,
                            isActive: v,
                            isUserLurking: N,
                            rsvped: M,
                            canInvite: G,
                            isChannelPublic: $,
                            onContextMenu: B,
                            onJoinClick: V,
                            onRsvpClick: W,
                            onStartClick: H,
                            onInviteClick: J,
                            onEndClick: Y
                        })]
                    })]
                })
            }
        },
        173764: (e, n, r) => {
            r.d(n, {
                Z: () => d
            });
            var t = r(785893),
                i = (r(667294), r(294184)),
                a = r.n(i),
                o = r(70418),
                l = r(961241),
                c = r(473708),
                s = r(811948),
                u = r.n(s);

            function d(e) {
                var n = e.guild,
                    r = e.speakers,
                    i = e.speakerCount,
                    s = e.className,
                    d = r.slice(0, 5),
                    f = d.map((function(e) {
                        var r, i;
                        return (0, t.jsxs)("div", {
                            className: u().speakerContainer,
                            children: [(0, t.jsx)(o.Avatar, {
                                src: null == e || null === (r = e.user) || void 0 === r ? void 0 : r.getAvatarURL(n.id, 20),
                                size: o.AvatarSizes.SIZE_20,
                                className: u().avatar,
                                "aria-label": "".concat(null == e ? void 0 : e.userNick, "-avatar")
                            }), (0, t.jsx)("div", {
                                className: u().textInGridContainer,
                                children: (0, t.jsx)(o.Text, {
                                    color: "header-secondary",
                                    variant: "text-sm/normal",
                                    className: u().textInGrid,
                                    children: null == e ? void 0 : e.userNick
                                })
                            })]
                        }, null == e || null === (i = e.user) || void 0 === i ? void 0 : i.id)
                    })),
                    h = i - d.length;
                return (0, t.jsxs)("div", {
                    className: a()(u().grid, s),
                    children: [f, h > 0 && (0, t.jsxs)("div", {
                        className: u().speakerContainer,
                        children: [(0, t.jsx)("div", {
                            className: u().iconMicrophone,
                            children: (0, t.jsx)(l.Z, {
                                height: 12
                            })
                        }), (0, t.jsx)(o.Text, {
                            color: "header-secondary",
                            variant: "text-sm/normal",
                            className: u().textInGrid,
                            children: c.Z.Messages.STAGE_DISCOVERY_LIVE_STAGE_TILE_SPEAKER_SUMMARY_OVERFLOW.format({
                                count: h
                            })
                        })]
                    })]
                })
            }
        },
        874544: (e, n, r) => {
            r.r(n);
            r.d(n, {
                default: () => le
            });
            var t = r(785893),
                i = r(667294),
                a = r(294184),
                o = r.n(a),
                l = r(202351),
                c = r(70418),
                s = r(944522),
                u = r(787193),
                d = r(299239),
                f = r(61209),
                h = r(567403),
                p = r(443812),
                v = r(755284),
                y = r(347365),
                b = r(441143),
                m = r.n(b),
                g = r(116404),
                x = r(771575),
                C = r(915840),
                E = r(563367),
                N = r(3155);

            function w(e, n, r, t, i, a, o) {
                try {
                    var l = e[a](o),
                        c = l.value
                } catch (e) {
                    r(e);
                    return
                }
                l.done ? n(c) : Promise.resolve(c).then(t, i)
            }

            function k(e) {
                return function() {
                    var n = this,
                        r = arguments;
                    return new Promise((function(t, i) {
                        var a = e.apply(n, r);

                        function o(e) {
                            w(a, t, i, o, l, "next", e)
                        }

                        function l(e) {
                            w(a, t, i, o, l, "throw", e)
                        }
                        o(void 0)
                    }))
                }
            }
            var S = function(e, n) {
                var r, t, i, a, o = {
                    label: 0,
                    sent: function() {
                        if (1 & i[0]) throw i[1];
                        return i[1]
                    },
                    trys: [],
                    ops: []
                };
                return a = {
                    next: l(0),
                    throw: l(1),
                    return: l(2)
                }, "function" == typeof Symbol && (a[Symbol.iterator] = function() {
                    return this
                }), a;

                function l(a) {
                    return function(l) {
                        return function(a) {
                            if (r) throw new TypeError("Generator is already executing.");
                            for (; o;) try {
                                if (r = 1, t && (i = 2 & a[0] ? t.return : a[0] ? t.throw || ((i = t.return) && i.call(t), 0) : t.next) && !(i = i.call(t, a[1])).done) return i;
                                (t = 0, i) && (a = [2 & a[0], i.value]);
                                switch (a[0]) {
                                    case 0:
                                    case 1:
                                        i = a;
                                        break;
                                    case 4:
                                        o.label++;
                                        return {
                                            value: a[1], done: !1
                                        };
                                    case 5:
                                        o.label++;
                                        t = a[1];
                                        a = [0];
                                        continue;
                                    case 7:
                                        a = o.ops.pop();
                                        o.trys.pop();
                                        continue;
                                    default:
                                        if (!(i = o.trys, i = i.length > 0 && i[i.length - 1]) && (6 === a[0] || 2 === a[0])) {
                                            o = 0;
                                            continue
                                        }
                                        if (3 === a[0] && (!i || a[1] > i[0] && a[1] < i[3])) {
                                            o.label = a[1];
                                            break
                                        }
                                        if (6 === a[0] && o.label < i[1]) {
                                            o.label = i[1];
                                            i = a;
                                            break
                                        }
                                        if (i && o.label < i[2]) {
                                            o.label = i[2];
                                            o.ops.push(a);
                                            break
                                        }
                                        i[2] && o.ops.pop();
                                        o.trys.pop();
                                        continue
                                }
                                a = n.call(e, o)
                            } catch (e) {
                                a = [6, e];
                                t = 0
                            } finally {
                                r = i = 0
                            }
                            if (5 & a[0]) throw a[1];
                            return {
                                value: a[0] ? a[1] : void 0,
                                done: !0
                            }
                        }([a, l])
                    }
                }
            };

            function j(e, n) {
                return A.apply(this, arguments)
            }

            function A() {
                return (A = k((function(e, n) {
                    var r, t, i, a;
                    return S(this, (function(n) {
                        switch (n.label) {
                            case 0:
                                switch (e.entity_type) {
                                    case N.WX.STAGE_INSTANCE:
                                        return [3, 1];
                                    case N.WX.VOICE:
                                        return [3, 6]
                                }
                                return [3, 7];
                            case 1:
                                r = E.Z.getChannelId();
                                t = f.Z.getChannel(e.channel_id);
                                m()(null != t, "could not find channel");
                                return r === t.id ? [3, 4] : [4, C.TM(t, !0)];
                            case 2:
                                n.sent();
                                return [4, C.R5(t, null, !1)];
                            case 3:
                                n.sent();
                                n.label = 4;
                            case 4:
                                return [4, (0, x.RK)(t, !1, !0)];
                            case 5:
                                n.sent();
                                return [3, 8];
                            case 6:
                                i = E.Z.getChannelId();
                                a = f.Z.getChannel(e.channel_id);
                                m()(null != a, "could not find channel");
                                i !== a.id && g.default.selectVoiceChannel(a.id);
                                return [3, 8];
                            case 7:
                                return [3, 8];
                            case 8:
                                return [2]
                        }
                    }))
                }))).apply(this, arguments)
            }

            function T(e, n) {
                switch (e.entity_type) {
                    case N.WX.STAGE_INSTANCE:
                    case N.WX.VOICE:
                    case N.WX.EXTERNAL:
                        null == n || n()
                }
                return Promise.resolve()
            }
            var I = r(985462),
                _ = r(341971),
                O = r(382060),
                P = r(242735),
                L = r(2590);

            function Z(e, n) {
                (null == n || n > e.length) && (n = e.length);
                for (var r = 0, t = new Array(n); r < n; r++) t[r] = e[r];
                return t
            }

            function M(e, n, r, t, i, a, o) {
                try {
                    var l = e[a](o),
                        c = l.value
                } catch (e) {
                    r(e);
                    return
                }
                l.done ? n(c) : Promise.resolve(c).then(t, i)
            }

            function G(e) {
                return function() {
                    var n = this,
                        r = arguments;
                    return new Promise((function(t, i) {
                        var a = e.apply(n, r);

                        function o(e) {
                            M(a, t, i, o, l, "next", e)
                        }

                        function l(e) {
                            M(a, t, i, o, l, "throw", e)
                        }
                        o(void 0)
                    }))
                }
            }

            function R(e) {
                return function(e) {
                    if (Array.isArray(e)) return Z(e)
                }(e) || function(e) {
                    if ("undefined" != typeof Symbol && null != e[Symbol.iterator] || null != e["@@iterator"]) return Array.from(e)
                }(e) || function(e, n) {
                    if (!e) return;
                    if ("string" == typeof e) return Z(e, n);
                    var r = Object.prototype.toString.call(e).slice(8, -1);
                    "Object" === r && e.constructor && (r = e.constructor.name);
                    if ("Map" === r || "Set" === r) return Array.from(r);
                    if ("Arguments" === r || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r)) return Z(e, n)
                }(e) || function() {
                    throw new TypeError("Invalid attempt to spread non-iterable instance.\\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.")
                }()
            }
            var U = function(e, n) {
                var r, t, i, a, o = {
                    label: 0,
                    sent: function() {
                        if (1 & i[0]) throw i[1];
                        return i[1]
                    },
                    trys: [],
                    ops: []
                };
                return a = {
                        next: l(0),
                        throw: l(1),
                        return: l(2)
                    },
                    "function" == typeof Symbol && (a[Symbol.iterator] = function() {
                        return this
                    }), a;

                function l(a) {
                    return function(l) {
                        return function(a) {
                            if (r) throw new TypeError("Generator is already executing.");
                            for (; o;) try {
                                if (r = 1, t && (i = 2 & a[0] ? t.return : a[0] ? t.throw || ((i = t.return) && i.call(t), 0) : t.next) && !(i = i.call(t, a[1])).done) return i;
                                (t = 0, i) && (a = [2 & a[0], i.value]);
                                switch (a[0]) {
                                    case 0:
                                    case 1:
                                        i = a;
                                        break;
                                    case 4:
                                        o.label++;
                                        return {
                                            value: a[1], done: !1
                                        };
                                    case 5:
                                        o.label++;
                                        t = a[1];
                                        a = [0];
                                        continue;
                                    case 7:
                                        a = o.ops.pop();
                                        o.trys.pop();
                                        continue;
                                    default:
                                        if (!(i = o.trys, i = i.length > 0 && i[i.length - 1]) && (6 === a[0] || 2 === a[0])) {
                                            o = 0;
                                            continue
                                        }
                                        if (3 === a[0] && (!i || a[1] > i[0] && a[1] < i[3])) {
                                            o.label = a[1];
                                            break
                                        }
                                        if (6 === a[0] && o.label < i[1]) {
                                            o.label = i[1];
                                            i = a;
                                            break
                                        }
                                        if (i && o.label < i[2]) {
                                            o.label = i[2];
                                            o.ops.push(a);
                                            break
                                        }
                                        i[2] && o.ops.pop();
                                        o.trys.pop();
                                        continue
                                }
                                a = n.call(e, o)
                            } catch (e) {
                                a = [6, e];
                                t = 0
                            } finally {
                                r = i = 0
                            }
                            if (5 & a[0]) throw a[1];
                            return {
                                value: a[0] ? a[1] : void 0,
                                done: !0
                            }
                        }([a, l])
                    }
                }
            };

            function B() {
                B = G((function(e, n) {
                    var r, t, i, a, o = arguments;
                    return U(this, (function(l) {
                        switch (l.label) {
                            case 0:
                                r = o.length > 2 && void 0 !== o[2] ? o[2] : [];
                                (t = i = []).push.apply(t, R(r));
                                return [4, I.Z.createChannel({
                                    guildId: e.id,
                                    type: L.d4z.GUILD_STAGE_VOICE,
                                    name: n.substring(0, 100),
                                    permissionOverwrites: i
                                })];
                            case 1:
                                if (null == (a = l.sent()) || 201 !== a.status) throw new Error("Can't create channel for event");
                                return [2, (0, O.q_)(a.body)]
                        }
                    }))
                }));
                return B.apply(this, arguments)
            }

            function V(e, n) {
                var r = e.guild_id,
                    t = e.channel_id,
                    i = h.Z.getGuild(r);
                if (null == i) return Promise.resolve(null);
                var a = f.Z.getChannel(t);
                return null == a ? function(e, n) {
                    return B.apply(this, arguments)
                }(i, e.name, n) : Promise.resolve(a)
            }

            function D(e, n) {
                return X.apply(this, arguments)
            }

            function X() {
                return (X = G((function(e, n) {
                    var r;
                    return U(this, (function(t) {
                        switch (t.label) {
                            case 0:
                                return e.entity_type === N.WX.STAGE_INSTANCE ? [3, 1] : [3, 3];
                            case 1:
                                return [4, V(e, n)];
                            case 2:
                                r = t.sent();
                                m()(null != r, "could not find or create channel");
                                return [3, 4];
                            case 3:
                                return [3, 4];
                            case 4:
                                return [2]
                        }
                    }))
                }))).apply(this, arguments)
            }

            function W(e) {
                return H.apply(this, arguments)
            }

            function H() {
                H = G((function(e) {
                    var n, r, t, i, a, o, l = arguments;
                    return U(this, (function(c) {
                        switch (c.label) {
                            case 0:
                                n = l.length > 1 && void 0 !== l[1] && l[1];
                                r = e.channel_id, t = e.entity_type, i = e.name, a = e.id, o = e.guild_id;
                                switch (t) {
                                    case N.WX.STAGE_INSTANCE:
                                        return [3, 1];
                                    case N.WX.VOICE:
                                        return [3, 3];
                                    case N.WX.EXTERNAL:
                                        return [3, 5]
                                }
                                return [3, 7];
                            case 1:
                                m()(null != r, "channel_id is required");
                                return [4, (0, _.me)(r, i, N.j8.GUILD_ONLY, n, a)];
                            case 2:
                            case 4:
                            case 6:
                                c.sent();
                                return [3, 8];
                            case 3:
                                m()(null != r, "channel_id is required");
                                return [4, P.Z.startEvent(a, o)];
                            case 5:
                                return [4, P.Z.startEvent(a, o)];
                            case 7:
                                return [3, 8];
                            case 8:
                                return [2]
                        }
                    }))
                }));
                return H.apply(this, arguments)
            }

            function J(e, n) {
                (null == n || n > e.length) && (n = e.length);
                for (var r = 0, t = new Array(n); r < n; r++) t[r] = e[r];
                return t
            }

            function Y(e, n, r, t, i, a, o) {
                try {
                    var l = e[a](o),
                        c = l.value
                } catch (e) {
                    r(e);
                    return
                }
                l.done ? n(c) : Promise.resolve(c).then(t, i)
            }

            function K(e) {
                return function() {
                    var n = this,
                        r = arguments;
                    return new Promise((function(t, i) {
                        var a = e.apply(n, r);

                        function o(e) {
                            Y(a, t, i, o, l, "next", e)
                        }

                        function l(e) {
                            Y(a, t, i, o, l, "throw", e)
                        }
                        o(void 0)
                    }))
                }
            }

            function z(e, n) {
                return function(e) {
                    if (Array.isArray(e)) return e
                }(e) || function(e, n) {
                    var r = null == e ? null : "undefined" != typeof Symbol && e[Symbol.iterator] || e["@@iterator"];
                    if (null != r) {
                        var t, i, a = [],
                            o = !0,
                            l = !1;
                        try {
                            for (r = r.call(e); !(o = (t = r.next()).done); o = !0) {
                                a.push(t.value);
                                if (n && a.length === n) break
                            }
                        } catch (e) {
                            l = !0;
                            i = e
                        } finally {
                            try {
                                o || null == r.return || r.return()
                            } finally {
                                if (l) throw i
                            }
                        }
                        return a
                    }
                }(e, n) || function(e, n) {
                    if (!e) return;
                    if ("string" == typeof e) return J(e, n);
                    var r = Object.prototype.toString.call(e).slice(8, -1);
                    "Object" === r && e.constructor && (r = e.constructor.name);
                    if ("Map" === r || "Set" === r) return Array.from(r);
                    if ("Arguments" === r || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r)) return J(e, n)
                }(e, n) || function() {
                    throw new TypeError("Invalid attempt to destructure non-iterable instance.\\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.")
                }()
            }
            var q = function(e, n) {
                    var r, t, i, a, o = {
                        label: 0,
                        sent: function() {
                            if (1 & i[0]) throw i[1];
                            return i[1]
                        },
                        trys: [],
                        ops: []
                    };
                    return a = {
                        next: l(0),
                        throw: l(1),
                        return: l(2)
                    }, "function" == typeof Symbol && (a[Symbol.iterator] = function() {
                        return this
                    }), a;

                    function l(a) {
                        return function(l) {
                            return function(a) {
                                if (r) throw new TypeError("Generator is already executing.");
                                for (; o;) try {
                                    if (r = 1, t && (i = 2 & a[0] ? t.return : a[0] ? t.throw || ((i = t.return) && i.call(t), 0) : t.next) && !(i = i.call(t, a[1])).done) return i;
                                    (t = 0, i) && (a = [2 & a[0], i.value]);
                                    switch (a[0]) {
                                        case 0:
                                        case 1:
                                            i = a;
                                            break;
                                        case 4:
                                            o.label++;
                                            return {
                                                value: a[1], done: !1
                                            };
                                        case 5:
                                            o.label++;
                                            t = a[1];
                                            a = [0];
                                            continue;
                                        case 7:
                                            a = o.ops.pop();
                                            o.trys.pop();
                                            continue;
                                        default:
                                            if (!(i = o.trys, i = i.length > 0 && i[i.length - 1]) && (6 === a[0] || 2 === a[0])) {
                                                o = 0;
                                                continue
                                            }
                                            if (3 === a[0] && (!i || a[1] > i[0] && a[1] < i[3])) {
                                                o.label = a[1];
                                                break
                                            }
                                            if (6 === a[0] && o.label < i[1]) {
                                                o.label = i[1];
                                                i = a;
                                                break
                                            }
                                            if (i && o.label < i[2]) {
                                                o.label = i[2];
                                                o.ops.push(a);
                                                break
                                            }
                                            i[2] && o.ops.pop();
                                            o.trys.pop();
                                            continue
                                    }
                                    a = n.call(e, o)
                                } catch (e) {
                                    a = [6, e];
                                    t = 0
                                } finally {
                                    r = i = 0
                                }
                                if (5 & a[0]) throw a[1];
                                return {
                                    value: a[0] ? a[1] : void 0,
                                    done: !0
                                }
                            }([a, l])
                        }
                    }
                },
                F = {
                    onSuccess: function() {},
                    permissionOverwrites: []
                };
            var $ = r(205316),
                Q = r(383495),
                ee = r(473708),
                ne = r(351391),
                re = r.n(ne);

            function te(e, n) {
                (null == n || n > e.length) && (n = e.length);
                for (var r = 0, t = new Array(n); r < n; r++) t[r] = e[r];
                return t
            }

            function ie(e, n, r, t, i, a, o) {
                try {
                    var l = e[a](o),
                        c = l.value
                } catch (e) {
                    r(e);
                    return
                }
                l.done ? n(c) : Promise.resolve(c).then(t, i)
            }

            function ae(e, n) {
                return function(e) {
                    if (Array.isArray(e)) return e
                }(e) || function(e, n) {
                    var r = null == e ? null : "undefined" != typeof Symbol && e[Symbol.iterator] || e["@@iterator"];
                    if (null != r) {
                        var t, i, a = [],
                            o = !0,
                            l = !1;
                        try {
                            for (r = r.call(e); !(o = (t = r.next()).done); o = !0) {
                                a.push(t.value);
                                if (n && a.length === n) break
                            }
                        } catch (e) {
                            l = !0;
                            i = e
                        } finally {
                            try {
                                o || null == r.return || r.return()
                            } finally {
                                if (l) throw i
                            }
                        }
                        return a
                    }
                }(e, n) || function(e, n) {
                    if (!e) return;
                    if ("string" == typeof e) return te(e, n);
                    var r = Object.prototype.toString.call(e).slice(8, -1);
                    "Object" === r && e.constructor && (r = e.constructor.name);
                    if ("Map" === r || "Set" === r) return Array.from(r);
                    if ("Arguments" === r || /^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(r)) return te(e, n)
                }(e, n) || function() {
                    throw new TypeError("Invalid attempt to destructure non-iterable instance.\\nIn order to be iterable, non-array objects must have a [Symbol.iterator]() method.")
                }()
            }
            var oe = function(e, n) {
                var r, t, i, a, o = {
                    label: 0,
                    sent: function() {
                        if (1 & i[0]) throw i[1];
                        return i[1]
                    },
                    trys: [],
                    ops: []
                };
                return a = {
                    next: l(0),
                    throw: l(1),
                    return: l(2)
                }, "function" == typeof Symbol && (a[Symbol.iterator] = function() {
                    return this
                }), a;

                function l(a) {
                    return function(l) {
                        return function(a) {
                            if (r) throw new TypeError("Generator is already executing.");
                            for (; o;) try {
                                if (r = 1, t && (i = 2 & a[0] ? t.return : a[0] ? t.throw || ((i = t.return) && i.call(t), 0) : t.next) && !(i = i.call(t, a[1])).done) return i;
                                (t = 0, i) && (a = [2 & a[0], i.value]);
                                switch (a[0]) {
                                    case 0:
                                    case 1:
                                        i = a;
                                        break;
                                    case 4:
                                        o.label++;
                                        return {
                                            value: a[1], done: !1
                                        };
                                    case 5:
                                        o.label++;
                                        t = a[1];
                                        a = [0];
                                        continue;
                                    case 7:
                                        a = o.ops.pop();
                                        o.trys.pop();
                                        continue;
                                    default:
                                        if (!(i = o.trys,
                                                i = i.length > 0 && i[i.length - 1]) && (6 === a[0] || 2 === a[0])) {
                                            o = 0;
                                            continue
                                        }
                                        if (3 === a[0] && (!i || a[1] > i[0] && a[1] < i[3])) {
                                            o.label = a[1];
                                            break
                                        }
                                        if (6 === a[0] && o.label < i[1]) {
                                            o.label = i[1];
                                            i = a;
                                            break
                                        }
                                        if (i && o.label < i[2]) {
                                            o.label = i[2];
                                            o.ops.push(a);
                                            break
                                        }
                                        i[2] && o.ops.pop();
                                        o.trys.pop();
                                        continue
                                }
                                a = n.call(e, o)
                            } catch (e) {
                                a = [6, e];
                                t = 0
                            } finally {
                                r = i = 0
                            }
                            if (5 & a[0]) throw a[1];
                            return {
                                value: a[0] ? a[1] : void 0,
                                done: !0
                            }
                        }([a, l])
                    }
                }
            };

            function le(e) {
                var n = e.transitionState,
                    r = e.event,
                    a = e.onSuccess,
                    b = e.onClose,
                    m = (0, p.Dt)(),
                    g = r.guild_id,
                    x = r.privacy_level,
                    C = (0, l.e7)([f.Z], (function() {
                        return f.Z.getChannel(r.channel_id)
                    }), [r]),
                    E = (0, l.e7)([h.Z], (function() {
                        return h.Z.getGuild(g)
                    }), [g]),
                    w = (0, (0, u.XJ)(null != C ? C : E).canManageGuildEvent)(r),
                    k = (0, l.e7)([s.Z], (function() {
                        return s.Z.isLurking(g)
                    }), [g]),
                    S = r.entity_type === N.WX.STAGE_INSTANCE,
                    A = ae(i.useState(S), 2),
                    I = A[0],
                    _ = A[1],
                    O = ae(function() {
                        var e = z(i.useState(!1), 2),
                            n = e[0],
                            r = e[1],
                            t = z(i.useState(null), 2),
                            a = t[0],
                            o = t[1];

                        function l() {
                            l = K((function(e, n) {
                                var t, i, a, l, c, s, u, d = arguments;
                                return q(this, (function(f) {
                                    switch (f.label) {
                                        case 0:
                                            t = d.length > 2 && void 0 !== d[2] ? d[2] : F, i = t.onSuccess, a = void 0 === i ? F.onSuccess : i, l = t.permissionOverwrites, c = void 0 === l ? F.permissionOverwrites : l;
                                            r(!0);
                                            f.label = 1;
                                        case 1:
                                            f.trys.push([1, 6, , 7]);
                                            return [4, D(e, c)];
                                        case 2:
                                            f.sent();
                                            return [4, W(e, n)];
                                        case 3:
                                            f.sent();
                                            return [4, j(e, a)];
                                        case 4:
                                            f.sent();
                                            return [4, T(e, a)];
                                        case 5:
                                            f.sent();
                                            r(!1);
                                            return [3, 7];
                                        case 6:
                                            s = f.sent();
                                            u = new y.Hx(s);
                                            o(u);
                                            r(!1);
                                            return [3, 7];
                                        case 7:
                                            r(!1);
                                            return [2]
                                    }
                                }))
                            }));
                            return l.apply(this, arguments)
                        }
                        return [function(e, n) {
                            return l.apply(this, arguments)
                        }, {
                            loading: n,
                            error: a
                        }]
                    }(), 2),
                    P = O[0],
                    L = O[1],
                    Z = L.loading,
                    M = L.error;
                if (!w) return null;
                var G, R, U, B = x === N.j8.PUBLIC ? ee.Z.Messages.STAGE_INSTANCE_PRIVACY_TYPE_PUBLIC_LABEL : ee.Z.Messages.STAGE_INSTANCE_PRIVACY_TYPE_PRIVATE_LABEL,
                    V = function() {
                        null == a || a();
                        b();
                        (0, d.Ku)(!1)
                    },
                    X = (R = (G = function() {
                        return oe(this, (function(e) {
                            switch (e.label) {
                                case 0:
                                    return [4, P(r, I, {
                                        onSuccess: V
                                    })];
                                case 1:
                                    e.sent();
                                    return [2]
                            }
                        }))
                    }, function() {
                        var e = this,
                            n = arguments;
                        return new Promise((function(r, t) {
                            var i = G.apply(e, n);

                            function a(e) {
                                ie(i, r, t, a, o, "next", e)
                            }

                            function o(e) {
                                ie(i, r, t, a, o, "throw", e)
                            }
                            a(void 0)
                        }))
                    }), function() {
                        return R.apply(this, arguments)
                    });
                return (0, t.jsx)(c.ModalRoot, {
                    transitionState: n,
                    "aria-labelledby": m,
                    children: (0, t.jsxs)(c.ModalContent, {
                        className: re().content,
                        children: [(0, t.jsx)("div", {
                            className: re().previewCard,
                            children: (0, t.jsx)(Q.Z, {
                                guild: E,
                                channel: C,
                                name: r.name,
                                description: null !== (U = r.description) && void 0 !== U ? U : void 0,
                                imageSource: (0, $.Z)(r),
                                isActive: !1,
                                isUserLurking: k,
                                speakers: [],
                                speakerCount: 0,
                                rsvped: !0,
                                guildEventId: r.id
                            })
                        }), (0, t.jsx)(c.Text, {
                            color: "header-secondary",
                            className: re().privacyLevel,
                            variant: "text-sm/normal",
                            children: ee.Z.Messages.START_EVENT_CONFIRMATION.format({
                                privacyLevel: B,
                                privacyLevelHook: function(e, n) {
                                    return x !== N.j8.PUBLIC ? null : (0, t.jsxs)("div", {
                                        className: re().privacyLevel,
                                        children: [(0, t.jsx)(v.Z, {
                                            width: 16,
                                            height: 16,
                                            className: re().publicIcon
                                        }), (0, t.jsx)(c.Text, {
                                            variant: "text-sm/normal",
                                            children: e
                                        })]
                                    }, n)
                                }
                            })
                        }), (0, t.jsx)(c.Heading, {
                            variant: "heading-xl/semibold",
                            className: re().header,
                            children: r.name
                        }), S && (0, t.jsx)(c.Checkbox, {
                            className: re().verticalSpacing,
                            type: c.Checkbox.Types.INVERTED,
                            value: I,
                            onChange: function(e) {
                                var n = e.currentTarget;
                                return _(n.checked)
                            },
                            children: (0, t.jsx)(c.Text, {
                                variant: "text-sm/normal",
                                children: ee.Z.Messages.GUILD_NOTIFY_MEMBERS_DESKTOP
                            })
                        }), (0, t.jsxs)("div", {
                            className: o()(re().inline, re().buttons, re().verticalSpacing),
                            children: [(0, t.jsx)(c.Button, {
                                color: c.Button.Colors.PRIMARY,
                                onClick: function() {
                                    b()
                                },
                                className: o()(re().button, re().spacing),
                                children: ee.Z.Messages.STAGE_BLOCKED_USERS_CANCEL
                            }), (0, t.jsx)(c.Button, {
                                color: c.Button.Colors.GREEN,
                                onClick: X,
                                submitting: Z,
                                className: re().button,
                                children: ee.Z.Messages.START_EVENT
                            })]
                        }), null != M && null != M.getAnyErrorMessage() ? (0, t.jsx)(c.Text, {
                            color: "text-danger",
                            variant: "text-sm/normal",
                            className: re().errorMessage,
                            children: M.getAnyErrorMessage()
                        }) : null]
                    })
                })
            }
        }
    }
]);